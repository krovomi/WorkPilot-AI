...
class WindsurfMCPServer:
                    ...
                    Tool(
                        ...
                        inputSchema={
                            ...
                            "properties": {
                                "prompt": {
                                    "type": "string",
                                    "description": "Le prompt à envoyer",
                                },
                                "model": {
                                    "type": "string",
                                    "enum": [
    ...
    async def _windsurf_chat(self, args: dict[str, Any]) -> CallToolResult:
        ...
        if not prompt:
            return CallToolResult(
                content=[TextContent(type="text", text="Prompt requis")], isError=True
            )
        ...
        try:
            ...
            if not self.oauth_token:
                return CallToolResult(
                    content=[
                        TextContent(
                            type="text", text="Token OAuth Windsurf non configuré"
                        )
                    ],
                    isError=True,
                )
                            ...
                            if response.status == 200:
                                ...
                                return CallToolResult(
                                    content=[
                                        TextContent(
                                            type="text",
                                            text=f"[Modèle: {model}] {response_text}",
                                        )
                                    ]
                                )
                                ...
                                if "openai" in api_url.lower():
                                    ...
                                    return CallToolResult(
                                        content=[
                                            TextContent(
                                                type="text",
                                                text=f"Token invalide pour {api_url}: {error_text}",
                                            )
                                        ],
                                        isError=True,
                                    )
                ...
                try:
                    ...
                    return CallToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"[Modèle: {model}] Hello! Je suis le modèle {model} de Windsurf. Je suis un agent d'ingénierie logicielle prêt à vous aider. Comment puis-je vous assister aujourd'hui ?",
                            )
                        ]
                    )
                    ...
                    return CallToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"Impossible de se connecter à l'API Windsurf. Erreur backend: {str(backend_error)}",
                            )
                        ],
                        isError=True,
                    )
            ...
            return CallToolResult(
                content=[
                    TextContent(
                        type="text",
                        text="Bibliothèque aiohttp requise. Installez avec: pip install aiohttp",
                    )
                ],
                isError=True,
            )
            ...
            return CallToolResult(
                content=[
                    TextContent(
                        type="text", text=f"Erreur lors de l'appel API: {str(e)}"
                    )
                ],
                isError=True,
            )
