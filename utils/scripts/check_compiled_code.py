...
def check_compiled_code():
    ...
    try:
        ...
        checks = [
            (
                "FastAPI backend URL",
                "http://localhost:9000/providers/usage/copilot" in content,
            ),
            (
                "normalizeCopilotResponse call",
                "normalizeCopilotResponse(usageData" in content,
            ),
            ("Copilot error handling", "INSUFFICIENT_PERMISSIONS" in content),
            ("Backend unavailable handling", "BACKEND_UNAVAILABLE" in content),
            (
                "Profile variables at top",
                "let profileName: string | undefined;" in content
                and content.index("let profileName")
                < content.index("if (providerName === 'copilot')"),
            ),
        ]
