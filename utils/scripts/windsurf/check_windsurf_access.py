    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.get(
                "https://server.codeium.com/api/v1/user", headers=headers, timeout=10
            )
...
async def _test_chat_endpoints(
    chat_endpoints: list, headers: dict, chat_payload: dict
) -> None:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    endpoint, headers=headers, json=chat_payload, timeout=10
                )
                print(f"\n💬 Chat test - {endpoint}")
                print(f"   Status: {response.status_code}")
