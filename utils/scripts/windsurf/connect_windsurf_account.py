...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "..", "..", "apps", "backend")
)
    ...
    for port in ports:
        try:
            response = requests.get(
                f"http://localhost:{port}/providers/status", timeout=5
            )
            if response.status_code == 200:
                data = response.json()
    ...
    if os.path.exists(config_file):
        ...
        for provider in providers_list:
            ...
            if isinstance(provider, dict) and provider.get("label") == "Windsurf AI":
                windsurf_found = True
                print(
                    f"✅ Provider Windsurf dans config: {provider.get('description')}"
                )
                break
