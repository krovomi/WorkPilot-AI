...
class BMADMemorySystem:
    ...
    def search(
        self, query: str, agent_type: str = None, limit: int = 10
    ) -> list[MemoryEntry]:
    ...
    def get_similar_performance_patterns(
        self, scenario: str, threshold: float = 0.8
    ) -> list[PerformanceMemory]:
    ...
    def get_architecture_decisions(
        self, component: str = None, agent_type: str = None
        ...
        if component:
            decisions = [
                d for d in decisions if component.lower() in d.component.lower()
            ]
    ...
    def get_workflow_history(
        self, workflow_type: str = None, days_back: int = 30
    ) -> list[WorkflowMemory]:
    ...
    def get_memory_statistics(self) -> dict[str, Any]:
        ...
        if self.entries:
            ...
            stats["average_quality_score"] = sum(
                e.quality_score for e in self.entries
            ) / len(self.entries)
    ...
    def cleanup_old_entries(self, days_to_keep: int = 90):
        ...
        self.performance_patterns = [
            p for p in self.performance_patterns if p.timestamp >= cutoff_date
        ]
        self.architecture_decisions = [
            d for d in self.architecture_decisions if d.timestamp >= cutoff_date
        ]
        self.workflow_history = [
            w for w in self.workflow_history if w.timestamp >= cutoff_date
        ]
