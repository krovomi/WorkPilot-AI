...
def create_projects(base_dir, project_name, project_type, framework, database):
    ...
    # Domain
    if not run_command(
        f"dotnet new classlib -n {domain_project} -f {framework}", src_dir
    ):
        return False

    # Application
    if not run_command(
        f"dotnet new classlib -n {application_project} -f {framework}", src_dir
    ):
        return False

    # Infrastructure
    if not run_command(
        f"dotnet new classlib -n {infrastructure_project} -f {framework}", src_dir
    ):
        return False
    ...
    if project_type == "webapi":
        if not run_command(
            f"dotnet new webapi -n {api_project} -f {framework} --minimal", src_dir
        ):
            return False
    elif project_type == "blazor":
        if not run_command(
            f"dotnet new blazorserver -n {api_project} -f {framework}", src_dir
        ):
            return False
    elif project_type == "console":
        if not run_command(
            f"dotnet new console -n {api_project} -f {framework}", src_dir
        ):
            return False
    elif project_type == "worker":
        if not run_command(
            f"dotnet new worker -n {api_project} -f {framework}", src_dir
        ):
            return False
    else:
    ...
    if not run_command(
        f"dotnet new xunit -n {unit_tests_project} -f {framework}", tests_dir
    ):
        return False
    ...
    if not run_command(
        f"dotnet new xunit -n {integration_tests_project} -f {framework}", tests_dir
    ):
        return False
...
def setup_project_references(base_dir, project_name, project_type):
    ...
    # Application -> Domain
    if not run_command(
        f"dotnet add {src_dir}/{application_project} reference {src_dir}/{domain_project}",
        src_dir,
    ):
        return False
    ...
    if not run_command(
        f"dotnet add {src_dir}/{infrastructure_project} reference {src_dir}/{domain_project} {src_dir}/{application_project}",
        src_dir,
    ):
        return False
    ...
    if not run_command(
        f"dotnet add {src_dir}/{api_project} reference {src_dir}/{application_project} {src_dir}/{infrastructure_project}",
        src_dir,
    ):
        return False
    ...
    if not run_command(
        f"dotnet add {tests_dir}/{unit_tests_project} reference {src_dir}/{application_project}",
        tests_dir,
    ):
        return False

    if not run_command(
        f"dotnet add {tests_dir}/{integration_tests_project} reference {src_dir}/{api_project}",
        tests_dir,
    ):
        return False
...
def add_nuget_packages(base_dir, project_name, database):
    ...
    for package in packages_app:
        if not run_command(
            f"dotnet add {src_dir}/{application_project} package {package}", src_dir
        ):
            return False
    ...
    for package in db_packages:
        if not run_command(
            f"dotnet add {src_dir}/{infrastructure_project} package {package}", src_dir
        ):
            return False
    ...
    for package in test_packages:
        if not run_command(
            f"dotnet add {tests_dir}/{unit_tests_project} package {package}", tests_dir
        ):
            return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Créer un nouveau projet .NET avec Clean Architecture"
    )
    parser.add_argument("--name", required=True, help="Nom du projet")
    parser.add_argument(
        "--type",
        choices=["webapi", "blazor", "console", "worker"],
        default="webapi",
        help="Type de projet",
    )
    parser.add_argument(
        "--framework",
        choices=["net10.0", "net9.0", "net8.0"],
        default="net10.0",
        help="Framework .NET",
    )
    parser.add_argument(
        "--database",
        choices=["sqlserver", "postgresql", "sqlite", "cosmosdb"],
        default="sqlserver",
        help="Base de données cible",
    )
    ...
    # Créer la structure
    base_dir = create_solution_structure(
        args.name, args.type, args.framework, args.database
    )

    # Créer les projets
    if not create_projects(
        base_dir, args.name, args.type, args.framework, args.database
    ):
