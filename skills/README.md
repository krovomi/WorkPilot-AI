...
The BMAD pack is the reason this exists, and it still needs it after the pack stopped
being wrappers. Every v6 skill resolves its customization through
`_bmad/scripts/resolve_customization.py`, a tree BMAD's own installer writes **into the
project being built** — per project, so no checkout of this repository can make it true
for the next one. Before the gate, all of them appeared in the Kanban command palette on
a fresh clone and every one failed on invocation.

The gate is applied twice, because the two readings answer different questions.
`skills-cli build` applies it against *this* repository and decides what
`.agents/skills/` holds. `workflows/runner.py` applies it again, against the **project
directory a phase is about to run in**, before starting the session — the build cannot
answer that question on a machine it will never see.
