...
def main():
    parser = argparse.ArgumentParser(
        description="Exécuter une commande avec un ou plusieurs serveurs"
    )
    parser.add_argument(
        "--server",
        action="append",
        dest="servers",
        required=True,
        help="Commande du serveur (peut être répétée)",
    )
    parser.add_argument(
        "--port",
        action="append",
        ...
        type=int,
        required=True,
        help="Port pour chaque serveur (doit correspondre au nombre de --server)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=30,
        help="Timeout en secondes par serveur (défaut: 30)",
    )
    parser.add_argument(
        "--shell",
        action="store_true",
        help="Interpréter --server via le shell (nécessaire pour 'cd && cmd'). Sans ce flag, la commande est tokenisée par shlex.",
    )
    parser.add_argument(
        "command",
        nargs=argparse.REMAINDER,
        help="Commande à exécuter après que les serveurs soient prêts",
    )
    ...
    try:
        ...
        for i, server in enumerate(servers):
            ...
            if not is_server_ready(server["port"], timeout=args.timeout):
                raise RuntimeError(
                    f"Le serveur a échoué à démarrer sur le port {server['port']} dans les {args.timeout}s"
                )
