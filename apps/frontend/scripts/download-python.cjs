...
const CHECKSUMS = {
	"darwin-arm64":
		"abe1de2494bb8b243fd507944f4d50292848fa00685d5288c858a72623a16635",
	"darwin-x64":
		"867c1af10f204224b571f8f2593fc9eb580fe0c2376224d1096ebe855ad8c722",
	"win32-x64":
		"1a702b3463cf87ec0d2e33902a47e95456053b0178fe96bd673c1dbb554f5d15",
	"linux-x64":
		"698e53b264a9bcd35cfa15cd680c4d78b0878fa529838844b5ffd0cd661d6bc2",
	"linux-arm64":
		"fb983ec85952513f5f013674fcbf4306b1a142c50fcfd914c2c3f00c61a874b0",
};
...
function getDownloadInfo(platform, arch) {
	...
	if (!config) {
		throw new Error(
			`Unsupported platform/arch combination: ${key}. Supported: ${Object.keys(configs).join(", ")}`,
		);
	}
	...
	return new Promise((resolve, reject) => {
		...
		const request = (urlString) => {
			...
			currentRequest.on("timeout", () => {
				...
				reject(
					new Error(
						`Download timeout after ${DOWNLOAD_TIMEOUT / 1000} seconds`,
					),
				);
			});
		};
...
function extractTarGz(archivePath, destDir) {
	...
	if (isWindows) {
		...
		if (result.status !== 0) {
			throw new Error(
				`Failed to extract archive: Windows tar exited with code ${result.status}`,
			);
		}
	} else {
		...
		if (result.status !== 0) {
			throw new Error(
				`Failed to extract archive: tar exited with code ${result.status}`,
			);
		}
	}
}
...
function verifyPythonBinary(pythonBin) {
	...
	if (result.status !== 0) {
		throw new Error(
			`Python verification failed with exit code ${result.status}`,
		);
	}
...
function hasPackage(sitePackagesDir, pkg) {
	...
	return (
		(fs.existsSync(pkgPath) && fs.existsSync(initPath)) ||
...
function isCriticalPackageMissing(sitePackagesDir, pkg) {
	if (pkg === "pydantic_core") {
		return (
			!hasPackage(sitePackagesDir, pkg) ||
			!hasPydanticCoreBinary(sitePackagesDir)
		);
	}
...
function stripSitePackages(sitePackagesDir) {
	...
	if (STRIP_PATTERNS.packagePaths) {
		for (const pkgPath of STRIP_PATTERNS.packagePaths) {
			...
			try {
				...
				if (err.code !== "ENOENT") {
					console.warn(
						`[download-python] Failed to remove ${pkgPath}: ${err.message}`,
					);
				}
			}
		}
	...
	function shouldRemoveFile(name) {
		...
		if (
			STRIP_PATTERNS.files.includes(name) ||
			STRIP_PATTERNS.files.includes(lowerName)
		) {
			return true;
		}
	...
	const _savedPercent = (((sizeBefore - sizeAfter) / sizeBefore) * 100).toFixed(
		1,
	);
}
...
function checkForBlockedPackages(requirementsPath) {
	...
	if (blocked.length > 0) {
		...
		for (const pkg of blocked) {
			console.warn(
				`[download-python]    - ${pkg} (consider making this an on-demand install)`,
			);
		}
		console.warn(
			`[download-python] These packages may significantly increase app size.\n`,
		);
	}
...
function fixPywin32(sitePackagesDir) {
	...
	for (const pyModule of pyModules) {
		...
		if (fs.existsSync(srcPath)) {
			try {
				...
				console.warn(
					`[download-python] Failed to copy ${pyModule}: ${err.message}`,
				);
			}
		}
	}
	...
	const sysloaderFiles = fs
		.readdirSync(win32Dir)
		.filter((f) => f.startsWith("_win32sysloader") && f.endsWith(".pyd"));
	for (const sysloader of sysloaderFiles) {
		...
		try {
			...
			console.warn(
				`[download-python] Failed to copy ${sysloader}: ${err.message}`,
			);
		}
	}
...
`;
		...
		if (err.code !== "EEXIST") {
			console.warn(
				`[download-python] Failed to create __init__.py: ${err.message}`,
			);
		}
	}
	...
	for (const dll of dllFiles) {
		...
		try {
			...
			console.warn(
				`[download-python] Failed to copy ${dll} to win32/: ${err.message}`,
			);
		}
	}
...
function installPackages(pythonBin, requirementsPath, targetSitePackages) {
	...
	if (!hasPydanticCoreBinary(targetSitePackages)) {
		console.warn(
			"[download-python] pydantic_core binary missing after strip; reinstalling pydantic-core...",
		);
		const pinnedVersion = getPinnedPydanticCoreVersion(targetSitePackages);
		const coreSpec = pinnedVersion
			? `pydantic-core==${pinnedVersion}`
			: "pydantic-core";
		// biome-ignore lint/suspicious/noEmptyBlockStatements: intentionally empty
		if (pinnedVersion) {
		} else {
			console.warn(
				"[download-python] Unable to determine pydantic-core pin; reinstalling latest",
			);
		}
		const pipArgs = [
			"-m",
		...
		if (result.error) {
			throw new Error(
				`Failed to reinstall pydantic-core: ${result.error.message}`,
			);
		}
		if (result.status !== 0) {
			throw new Error(
				`pydantic-core reinstall failed with exit code ${result.status}`,
			);
		}
...
async function downloadPython(targetPlatform, targetArch, options = {}) {
	...
	const { skipPackages = false, requirementsPath: customRequirementsPath } =
		options;
	...
	const requirementsPath =
		...
		path.join(frontendDir, "..", "backend", "requirements.txt");
		...
		try {
			...
			if (missingPackages.length > 0) {
				...
				return {
					...
					sitePackagesPath: sitePackagesDir,
				};
			}
		} catch (err) {
	...
	if (!skipPackages) {
		if (!fs.existsSync(requirementsPath)) {
			console.warn(
				`[download-python] Warning: requirements.txt not found at ${requirementsPath}`,
			);
			...
			if (postInstallMissing.length > 0) {
				throw new Error(
					`Package installation failed - missing critical packages: ${postInstallMissing.join(", ")}`,
				);
			}
...
async function downloadAllPlatforms() {
	...
	for (const { platform, arch } of platforms) {
		try {
			...
			console.error(
				`[download-python] Failed for ${platform}-${arch}: ${error.message}`,
			);
			throw error;
		}
	}
...
function validateInput(value, validValues, name) {
	...
	if (!validValues.includes(sanitized)) {
		throw new Error(
			`Invalid ${name}: "${sanitized}". Valid values: ${validValues.join(", ")}`,
		);
	}
