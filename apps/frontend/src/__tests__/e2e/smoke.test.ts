...
function createTestSettings(
	overrides: Partial<TestSettingsData> = {},
): TestSettingsData {
	return {
		theme: "system",
		telemetry: true,
...
describe("E2E Smoke Tests", { timeout: 30000 }, () => {
	...
	describe("Project Creation Flow", () => {
		...
		it("should handle project creation with custom settings", async () => {
			...
			const updateResult = await updateProjectSettings(
				"project-001",
				newSettings,
			);
	...
	describe("Task Creation and Execution Flow", () => {
		...
		it("should handle task update with partial data", async () => {
			...
			const result = await updateTask("task-001", {
		...
		it("should handle theme toggle cycle (system -> light -> dark -> system)", async () => {
			...
			for (const nextTheme of themeProgression) {
				...
				const result = await saveSettings(
					createTestSettings({ theme: nextTheme }),
				);
