...
// Mock xterm.js and its addons - use class syntax for constructability with `new`
vi.mock("@xterm/xterm", () => {
	class MockTerminal {
		constructor() {
...
vi.mock("@xterm/addon-serialize", () => {
	class MockSerializeAddon {
		constructor() {
			Object.assign(this, { serialize: vi.fn(() => ""), dispose: vi.fn() });
		}
	}
...
describe("Terminal copy/paste integration", () => {
	...
	beforeEach(() => {
		vi.clearAllMocks();
		// Reset terminal factory
		terminalFactory = null;

		// Mock ResizeObserver with constructable class
	...
	describe("xterm.js selection API integration with clipboard write", () => {
		it("should integrate xterm.hasSelection() with clipboard write", async () => {
			const { useXterm } = await import(
				"../../renderer/components/terminal/useXterm"
			);

			let keyEventHandler: ((event: KeyboardEvent) => boolean) | null = null;
			const mockHasSelection = vi.fn(() => true);
			const mockGetSelection = vi.fn(() => "selected terminal text");

			// Override Terminal factory for this test
			terminalFactory = () => ({
				open: vi.fn(),
				loadAddon: vi.fn(),
				attachCustomKeyEventHandler: vi.fn(
			...
			// Addon factories are handled by the class-based mocks
	...
	describe("clipboard error handling without breaking terminal", () => {
		it("should continue terminal operation after clipboard error", async () => {
			...
			mockClipboard.readText = vi
				.fn()
				.mockRejectedValue(new Error("Clipboard denied"));

			// Mock window.electronAPI with sendTerminalInput
			(
				window as unknown as { electronAPI: { sendTerminalInput: Mock } }
			).electronAPI = {
