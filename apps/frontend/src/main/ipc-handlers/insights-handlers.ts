...
export function registerInsightsHandlers(
	...
	ipcMain.handle(
		...
		async (
			...
			if (!project.autoBuildPath) {
				return {
					success: false,
					error: "WorkPilot AI not initialized for this project",
				};
			}
	...
	ipcMain.handle(
		...
		async (
			...
			const session = insightsService.switchSession(
				projectId,
				project.path,
				sessionId,
			);
	...
	ipcMain.handle(
		...
		async (_, projectId: string, sessionId: string): Promise<IPCResult> => {
			...
			const success = insightsService.deleteSession(
				projectId,
				project.path,
				sessionId,
			);
	...
	ipcMain.handle(
		...
		async (
			...
			const success = insightsService.renameSession(
				project.path,
				sessionId,
				newTitle,
			);
	...
	insightsService.on(
		"session-updated",
		(projectId: string, session: unknown) => {
			safeSendToRenderer(
				getMainWindow,
				IPC_CHANNELS.INSIGHTS_SESSION_UPDATED,
				projectId,
				session,
			);
		},
