...
function getRunnerPath(): string {
	return path.join(
		...
		"backend",
		"runners",
		"cross_language_translation_runner.py",
	);
}
		...
		proc.on("close", (code) => {
			...
			if (code === 0) {
				try {
					...
					resolve({ success: true, data: { translated_code: out } });
				}
			} else {
...
export function registerCrossLanguageTranslationHandlers(): void {
	...
	ipcMain.handle(
		"crossLangTranslation:translate",
		async (
			event,
			params: {
				projectDir: string;
			...
			if (params.code && !params.filePath) {
				const ext =
					SUPPORTED_LANGUAGES.find((l) => l.id === params.sourceLang)
						?.extension ?? ".txt";
				tempFile = path.join(
					params.projectDir,
					`.workpilot`,
			...
			const idx = history.findIndex(
				(h: unknown) => (h as { id: string }).id === historyEntry.id,
			);
			if (idx >= 0) {
				(history[idx] as Record<string, unknown>).status = result.success
					? "complete"
					: "failed";
				(history[idx] as Record<string, unknown>).completed_at =
					new Date().toISOString();
				fs.writeFileSync(
					getHistoryPath(params.projectDir),
	...
	ipcMain.handle(
		"crossLangTranslation:getHistory",
		(_event, projectDir: string, limit: number = 50) => {
			try {
	...
	ipcMain.handle(
		"crossLangTranslation:clearHistory",
