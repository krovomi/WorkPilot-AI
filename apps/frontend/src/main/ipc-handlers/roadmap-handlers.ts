		...
		async (_, projectId: string): Promise<IPCResult<Roadmap | null>> => {
			...
			try {
				...
				if (existsSync(competitorAnalysisPath)) {
					try {
						const competitorContent = readFileSync(
						...
						competitorAnalysis = {
							projectContext: {
								...
								targetAudience:
									rawCompetitor.project_context?.target_audience || "",
							},
							competitors: (rawCompetitor.competitors || []).map(
								(c: Record<string, unknown>) => ({
									...
									painPoints: (
										(c.pain_points as Array<Record<string, unknown>>) || []
									).map((p) => ({
										id: p.id,
							...
							marketGaps: (rawCompetitor.market_gaps || []).map(
								(g: Record<string, unknown>) => ({
									id: g.id,
									description: g.description,
									affectedCompetitors:
										(g.affected_competitors as string[]) || [],
							...
							insightsSummary: {
								topPainPoints:
									rawCompetitor.insights_summary?.top_pain_points || [],
								differentiatorOpportunities:
									rawCompetitor.insights_summary
										?.differentiator_opportunities || [],
								marketTrends:
									rawCompetitor.insights_summary?.market_trends || [],
							},
							researchMetadata: {
								searchQueriesUsed:
									rawCompetitor.research_metadata?.search_queries_used || [],
								sourcesConsulted:
									rawCompetitor.research_metadata?.sources_consulted || [],
				...
				const roadmap: Roadmap = {
					...
					phases: (rawRoadmap.phases || []).map(
						(phase: Record<string, unknown>) => ({
							...
							milestones: (
								(phase.milestones as Array<Record<string, unknown>>) || []
							).map((m) => ({
								id: m.id,
								title: m.title,
								...
								targetDate: m.target_date
									? new Date(m.target_date as string)
									: undefined,
							})),
						}),
					),
					features: (rawRoadmap.features || []).map(
				...
				return {
					success: false,
					error:
						error instanceof Error ? error.message : "Failed to read roadmap",
				};
			}
		},
	);
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				const feature = roadmap.features?.find(
...
`;
				...
				writeFileSync(
					path.join(specDir, AUTO_BUILD_PATHS.SPEC_FILE),
					taskDescription,
					"utf-8",
				);
				...
				return {
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to convert feature to spec",
				};
			}
		},
	);
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				debugLog("[Roadmap Handler] Saved progress checkpoint:", {
					projectId,
					phase: progressData.phase,
				});
		...
		async (
			...
			try {
				...
				if (
					...
					!validPhases.includes(rawData.phase)
				) {
					debugLog(
						"[Roadmap Handler] Invalid progress file structure or phase, ignoring:",
						{ projectId, phase: rawData.phase },
					);
					return { success: true, data: null };
				}
				...
				debugLog("[Roadmap Handler] Loaded progress checkpoint:", {
					projectId,
					phase: progressData.phase,
				});
		...
		async (_, projectId: string): Promise<IPCResult> => {
			...
			try {
				if (existsSync(progressPath)) {
					...
					debugLog("[Roadmap Handler] Cleared progress checkpoint:", {
						projectId,
					});
				}
				return { success: true };
