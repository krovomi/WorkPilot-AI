...
export function setupConflictPredictorHandlers() {
	...
	ipcMain.handle(
		...
		async (event, { projectId }: ConflictPredictorRequest) => {
			return new Promise((resolve, reject) => {
				...
				const spawnedProcess = spawn(
					pythonCommand,
					[...pythonBaseArgs, runnerPath, "--project-path", project.path],
					{
						cwd: backendPath,
						env: {
							...process.env,
						...
						if (line.startsWith("CONFLICT_PREDICTOR_EVENT:")) {
							try {
								const eventData = JSON.parse(
									line.substring("CONFLICT_PREDICTOR_EVENT:".length),
								);
							...
							try {
								result = JSON.parse(
									line.substring("CONFLICT_PREDICTOR_RESULT:".length),
								);
							} catch (e) {
