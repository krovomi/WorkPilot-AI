...
import {
	...
	type PythonEnvManager,
	pythonEnvManager as pythonEnvManagerSingleton,
...
export function validateWorktreeBranch(
	...
	if (
		detectedBranch.startsWith("workpilot/") &&
		detectedBranch.length > "workpilot/".length
	) {
		return {
			branchToDelete: detectedBranch,
			usedFallback: false,
...
function getUtilitySettings(): {
	...
	try {
		if (existsSync(settingsPath)) {
			...
			const thinkingLevel =
				featureThinking.utility || DEFAULT_FEATURE_THINKING.utility;
			...
			return {
				...
				thinkingBudget:
					thinkingLevel in THINKING_BUDGET_MAP
						? THINKING_BUDGET_MAP[thinkingLevel]
						: THINKING_BUDGET_MAP.low,
			};
		}
	} catch (error) {
...
function fixMisconfiguredBareRepo(projectPath: string): boolean {
	try {
				...
				if (directoryFiles === null) {
					try {
						...
						if (allFiles.length > MAX_FILES_TO_CHECK) {
							console.warn(
								`[GIT] Directory has ${allFiles.length} entries, checking only first ${MAX_FILES_TO_CHECK}`,
							);
						}
					} catch (error) {
						// Log the error for debugging instead of silently swallowing
						console.warn(
							`[GIT] Failed to read directory ${projectPath}:`,
		...
		console.warn(
			"[GIT] Detected misconfigured bare repository with source files. Auto-fixing by unsetting core.bare...",
		);
		...
		console.warn(
			"[GIT] Fixed: core.bare has been unset. Git operations should now work correctly.",
		);
		return true;
	} catch {
		return false;
...
> = {
	...
	vscodium: {
		...
		paths: {
			...
			win32: [
				...
				"C:\\Users\\%USERNAME%\\AppData\\Local\\Programs\\VSCodium\\VSCodium.exe",
			],
	...
	void: {
		...
		paths: {
			...
			win32: [
				"C:\\Users\\%USERNAME%\\AppData\\Local\\Programs\\Void\\Void.exe",
			],
			linux: ["/usr/bin/void"],
		},
	...
	fleet: {
		...
		paths: {
			...
			win32: [
				"C:\\Users\\%USERNAME%\\AppData\\Local\\JetBrains\\Toolbox\\apps\\Fleet\\ch-0\\*\\Fleet.exe",
			],
	...
	netbeans: {
		...
		paths: {
			darwin: [
				"/Applications/NetBeans.app",
				"/Applications/Apache NetBeans.app",
			],
...
const TERMINAL_DETECTION: Partial<
	Record<
		SupportedTerminal,
		{
			name: string;
...
> = {
	...
	ghostty: {
		name: "Ghostty",
		paths: {
			darwin: ["/Applications/Ghostty.app"],
			win32: [],
			linux: ["/usr/bin/ghostty"],
		},
		commands: {
			darwin: ["open", "-a", "Ghostty"],
			win32: [],
			linux: ["ghostty"],
		},
	},
	...
	powershell: {
		...
		commands: {
			darwin: [],
			win32: ["powershell.exe", "-NoExit", "-Command", "cd"],
			linux: [],
		},
	},
	cmd: {
	...
	cmder: {
		...
		paths: {
			darwin: [],
			win32: ["C:\\cmder\\Cmder.exe", "C:\\tools\\cmder\\Cmder.exe"],
			linux: [],
		},
	...
	gitbash: {
		...
		paths: {
			darwin: [],
			win32: ["C:\\Program Files\\Git\\git-bash.exe"],
			linux: [],
		},
	...
	xfce4terminal: {
		...
		commands: {
			darwin: [],
			win32: [],
			linux: ["xfce4-terminal", "--working-directory="],
		},
	},
	"mate-terminal": {
		name: "MATE Terminal",
		...
		commands: {
			darwin: [],
			win32: [],
			linux: ["mate-terminal", "--working-directory="],
		},
	},
	...
	terminator: {
		...
		commands: {
			darwin: [],
			win32: [],
			linux: ["terminator", "--working-directory="],
		},
	},
	tilix: {
		name: "Tilix",
	...
	alacritty: {
		...
		paths: {
			...
			win32: [
				...
				"C:\\Users\\%USERNAME%\\scoop\\apps\\alacritty\\current\\alacritty.exe",
			],
	...
	hyper: {
		...
		paths: {
			...
			win32: [
				"C:\\Users\\%USERNAME%\\AppData\\Local\\Programs\\Hyper\\Hyper.exe",
			],
	...
	tabby: {
		...
		paths: {
			...
			win32: [
				"C:\\Users\\%USERNAME%\\AppData\\Local\\Programs\\Tabby\\Tabby.exe",
			],
	...
	st: {
		...
		paths: {
			darwin: [],
			win32: [],
			linux: ["/usr/local/bin/st", "/usr/bin/st"],
		},
	...
	coolretroterm: {
		...
		paths: {
			darwin: ["/Applications/cool-retro-term.app"],
			win32: [],
			linux: ["/usr/bin/cool-retro-term"],
		},
		commands: {
			darwin: ["open", "-a", "cool-retro-term"],
			win32: [],
...
async function detectMacApps(): Promise<Set<string>> {
	...
	try {
		...
		const { stdout } = await execAsync(
			'mdfind -onlyin /Applications "kMDItemKind == Application" 2>/dev/null | head -500',
			{ timeout: 10000 },
		);
...
function isAppInstalled(
	...
	for (const checkPath of specificPaths) {
		...
		if (
			...
			(basePath !== expandedPath && existsSync(basePath))
		) {
...
async function detectInstalledTools(): Promise<DetectedTools> {
	...
	for (const [id, config] of Object.entries(IDE_DETECTION)) {
		...
		const { installed, foundPath } = isAppInstalled(
			searchNames,
			paths,
			platform,
		);
		...
		if (!finalInstalled && config.commands[platform]) {
			try {
				if (platform === "win32") {
					await execAsync(`where ${config.commands[platform]}`, {
						timeout: 2000,
					});
				} else {
					await execAsync(`which ${config.commands[platform]}`, {
						timeout: 2000,
					});
				}
				finalInstalled = true;
...
async function openInIDE(
	...
	try {
		...
		if (!command) {
			return {
				success: false,
				error: `IDE ${ide} is not supported on ${platform}`,
			};
		}
		...
		if (platform === "darwin") {
          ...
          tell current session of current window
			...
			if (terminal === "system") {
				...
				spawn(commands[0], [...commands.slice(1), dirPath], {
					detached: true,
					stdio: "ignore",
				}).unref();
			}
			...
			if (terminal === "system") {
				...
				try {
					await execFileAsync("x-terminal-emulator", [
						"--working-directory",
						dirPath,
						"-e",
						"bash",
...
function getEffectiveBaseBranch(
	...
	if (!projectPath || typeof projectPath !== "string") {
		console.error(
			"[getEffectiveBaseBranch] projectPath is undefined or not a string",
		);
		return "main";
	}
	if (!specId || typeof specId !== "string") {
		console.error(
			"[getEffectiveBaseBranch] specId is undefined or not a string",
		);
		return "main";
	}
...
function parsePRJsonOutput(stdout: string): ParsedPRResult | null {
	...
	try {
		...
		const validatedPrUrl =
			rawPrUrl && isValidGitHubUrl(rawPrUrl) ? rawPrUrl : undefined;

		return {
			success: typeof parsed.success === "boolean" ? parsed.success : false,
			prUrl: validatedPrUrl,
			alreadyExists:
				typeof parsed.already_exists === "boolean"
					? parsed.already_exists
					: typeof parsed.alreadyExists === "boolean"
...
async function updateTaskStatusAfterPRCreation(
	...
	debug(
		"Main project metadata updated with prUrl:",
	...
	if (worktreePath) {
		...
		result.worktreeMetadata = updateTaskMetadataPrUrl(
...
function buildCreatePRArgs(
	...
	if (options?.title) {
		...
		if (options.title.length > MAX_PR_TITLE_LENGTH) {
			return {
				args: [],
				validationError: `PR title exceeds maximum length of ${MAX_PR_TITLE_LENGTH} characters`,
			};
		}
		if (!PRINTABLE_CHARS_REGEX.test(options.title)) {
			return {
				args: [],
				validationError: "PR title contains invalid characters",
			};
		}
...
export function registerWorktreeHandlers(
		...
		async (_, taskId: string): Promise<IPCResult<WorktreeStatus>> => {
			try {
				...
				try {
					...
					try {
						currentProjectBranch = execFileSync(
							getToolPath("git"),
							["rev-parse", "--abbrev-ref", "HEAD"],
							{
								cwd: project.path,
		...
		async (
			...
			try {
				...
				if (!pythonEnvManager.isEnvReady()) {
					...
					if (autoBuildSource) {
						...
						if (!status.ready) {
							return {
								success: false,
								error: `Python environment not ready: ${status.error || "Unknown error"}`,
							};
						}
					} else {
						return {
							success: false,
							error:
								"Python environment not ready and WorkPilot AI source not found",
						};
					}
				}
				...
				if (isGitWorkTree(project.path)) {
					try {
						const gitStatusBefore = execFileSync(
							getToolPath("git"),
							["status", "--short"],
							{ cwd: project.path, encoding: "utf-8" },
						);
						debug(
							"Git status BEFORE merge in main project:\n",
					...
					debug(
						"Project is a bare repository - skipping pre-merge git status check",
					);
				}
				...
				return new Promise((resolve) => {
					...
					const [pythonCommand, pythonBaseArgs] =
						parsePythonCommand(pythonPath);
						...
						[...pythonBaseArgs, ...args],
						{
							...
							env: {
								...
								UTILITY_THINKING_BUDGET:
									utilitySettings.thinkingBudget === null
										? ""
										: utilitySettings.thinkingBudget?.toString() || "",
							},
					...
					timeoutId = setTimeout(() => {
						if (!resolved) {
							debug(
								"TIMEOUT: Merge process exceeded",
								MERGE_TIMEOUT_MS,
								"ms, killing...",
							);
							resolved = true;
							...
							if (mainWindow) {
								mainWindow.webContents.send(
									IPC_CHANNELS.TASK_MERGE_PROGRESS,
									taskId,
									{
										type: "progress",
										stage: "error",
							...
							const mayHaveSucceeded =
								stdout.includes("staged") ||
								stdout.includes("Successfully merged") ||
								stdout.includes("Changes from");

							if (mayHaveSucceeded) {
								debug(
									"TIMEOUT: Process hung but merge may have succeeded based on output",
								);
								...
								resolve({
									...
									data: {
										success: true,
										message:
											"Changes staged (process timed out but merge appeared successful)",
										staged: isStageOnly,
								...
								resolve({
									success: false,
									error:
										"Merge process timed out. Check git status to see if merge completed.",
								});
							}
						}
					}, MERGE_TIMEOUT_MS);
					...
					const handleProcessExit = async (
						...
						if (isGitWorkTree(project.path)) {
							try {
								const gitStatusAfter = execFileSync(
									getToolPath("git"),
									["status", "--short"],
									{ cwd: project.path, encoding: "utf-8" },
								);
								debug(
									"Git status AFTER merge in main project:\n",
									gitStatusAfter || "(clean)",
								);
								const gitDiffStaged = execFileSync(
									getToolPath("git"),
									["diff", "--staged", "--stat"],
									{ cwd: project.path, encoding: "utf-8" },
								);
							...
							debug(
								"Project is a bare repository - skipping git status check (this is normal for worktree-based projects)",
							);
						}
						...
						if (code === 0) {
							...
							if (isStageOnly) {
								...
								if (isGitWorkTree(project.path)) {
									try {
										...
										debug(
											"Stage-only verification: hasActualStagedChanges:",
											hasActualStagedChanges,
										);
										...
										if (!hasActualStagedChanges) {
											...
											try {
												...
												debug(
													"Merge already committed check:",
													mergeAlreadyCommitted,
												);
											} catch {
												// Exit code 1 means not merged, or branch may not exist
												mergeAlreadyCommitted = false;
												debug(
													"Could not check merge status, assuming not merged",
												);
											}
										}
									} catch (e) {
									...
									debug(
										"Project is a bare repository - skipping staged changes verification",
									);
								}
							}
							...
							if (
								...
								planStatus = "review";
								message =
									"Changes were already merged and committed. You can mark this task as complete when ready.";
								staged = false;
								debug(
									"Stage-only requested but merge already committed. Keeping in human_review for user to confirm completion.",
								);
								...
								planStatus = "review";
								message =
									"No changes to stage. The worktree may have no differences from the current branch.";
								staged = false;
							...
							const reviewReason =
								newStatus === "human_review" ? "completed" : undefined;
								...
								try {
									if (existsSync(commitMsgPath)) {
										...
										debug(
											...
											suggestedCommitMessage?.substring(0, 100),
										);
									}
								} catch (e) {
							...
							const updatePlanWithRetry = async (
								planPath: string,
								isMain: boolean,
							): Promise<boolean> => {
								...
								try {
									await withRetry(
										async () => {
											const planContent = await fsPromises.readFile(
												planPath,
												"utf-8",
											);
											...
											const verifyContent = await fsPromises.readFile(
												planPath,
												"utf-8",
											);
											const verifyPlan = JSON.parse(verifyContent);
											if (
												verifyPlan.status !== newStatus ||
												verifyPlan.planStatus !== planStatus
											) {
												throw new Error(
													"Write verification failed - status mismatch",
												);
											}
										},
										{
									...
									if (isMain) {
										console.error(
											"Failed to persist task status to main plan after retries:",
											err,
										);
									} else {
										debug(
											"Failed to persist task status to worktree plan (non-critical):",
											err,
										);
									}
									return false;
							...
							const updatePlans = async () => {
								...
								if (!results[0]) {
									console.warn(
										"Background plan update: main plan write may not have persisted",
									);
								}
							};
							...
							const hasConflicts = conflictPatterns.some((pattern) =>
	...
	ipcMain.handle(
		...
		async (_, taskId: string): Promise<IPCResult<WorktreeMergeResult>> => {
			console.warn(
				"[IPC] TASK_WORKTREE_MERGE_PREVIEW called with taskId:",
				taskId,
			);
			try {
				// Ensure Python environment is ready
				if (!pythonEnvManager.isEnvReady()) {
					...
					if (autoBuildSource) {
						...
						if (!status.ready) {
							console.error(
								"[IPC] Python environment failed to initialize:",
								status.error,
							);
							return {
								success: false,
				...
				if (isGitWorkTree(project.path)) {
					...
					console.warn(
						"[IPC] Project is a bare repository - skipping uncommitted changes check",
					);
				}
				...
				if (effectiveBaseBranch) {
					args.push("--base-branch", effectiveBaseBranch);
					console.warn(
						"[IPC] Using base branch for preview:",
						effectiveBaseBranch,
						`(source: ${taskBaseBranch ? "task metadata" : "project settings"})`,
					);
				}
					...
					previewProcess.on("close", (code: number) => {
						...
						if (code === 0) {
							try {
								...
								console.error(
									"[IPC] Failed to parse preview result:",
									parseError,
								);
	...
	ipcMain.handle(
		...
		): Promise<IPCResult<WorktreeDiscardResult>> => {
			try {
				const { task, project } = findTaskAndProject(taskId);
				if (!task || !project) {
					return { success: false, error: "Task not found" };
				...
				if (!cleanupResult.success) {
					console.error(
						"[TASK_WORKTREE_DISCARD] Cleanup failed:",
						cleanupResult.warnings,
					);
					return {
						success: false,
				...
				if (cleanupResult.warnings.length > 0) {
					console.warn(
						"[TASK_WORKTREE_DISCARD] Cleanup warnings:",
						cleanupResult.warnings,
					);
				}
				if (cleanupResult.autoCommitted) {
					console.warn(
						"[TASK_WORKTREE_DISCARD] Auto-committed uncommitted work before discard",
					);
				}
		...
		async (
			...
			try {
				...
				if (!projectId || typeof projectId !== "string") {
					console.error(
						"discardOrphanedWorktree: Invalid projectId:",
						projectId,
					);
				...
				if (!project.path || typeof project.path !== "string") {
					console.error(
						"discardOrphanedWorktree: Project path is invalid:",
						project.path,
					);
				...
				return {
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to discard orphaned worktree",
				};
			}
		},
	);
	...
	ipcMain.handle(
		...
		async (_, projectId: string): Promise<IPCResult<WorktreeListResult>> => {
			try {
				...
				if (!project.path || typeof project.path !== "string") {
					console.error(
						"listWorktrees: Project path is invalid:",
						project.path,
					);
					return { success: false, error: "Project path is invalid" };
				}
				...
				const taskLookupSuccessful =
					tasks.length > 0 || !existsSync(mainSpecsDir);

				// Helper to process a single worktree entry (async)
				const processWorktreeEntry = async (
					entry: string,
					entryPath: string,
					...
					try {
						...
						try {
							...
							commitCount =
								parseInt((countResult.stdout as string).trim(), 10) || 0;
						} catch {
							commitCount = 0;
						}
						...
						const hasTask = tasks.some((t) => t.specId === entry);
						console.warn(
							`[Worktree] Git commands failed for ${entry}, hasTask=${hasTask}:`,
							gitError,
						);
				...
				const worktrees = results.filter(
					(w): w is WorktreeListItem => w !== null,
				);
			...
			try {
				...
				return {
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to detect installed tools",
				};
			}
		},
	);
		...
		async (_, taskId: string): Promise<IPCResult<{ cleared: boolean }>> => {
			try {
				...
				if (worktreePath) {
					...
					try {
						const worktreePlanContent = await fsPromises.readFile(
							worktreePlanPath,
							"utf-8",
						);
						...
						await fsPromises.writeFile(
							worktreePlanPath,
							JSON.stringify(worktreePlan, null, 2),
							"utf-8",
						);
					} catch (e) {
						...
						if (!isFileNotFound(e)) {
							console.warn(
								"[CLEAR_STAGED_STATE] Failed to update worktree plan:",
								e,
							);
						}
					}
			...
			try {
				...
				return new Promise((resolve) => {
					...
					timeoutId = setTimeout(() => {
						if (!resolved) {
							debug(
								"TIMEOUT: Create PR process exceeded",
							...
							resolve({
								success: false,
								error:
									"PR creation timed out. Check if the PR was created on GitHub.",
							});
						}
					}, PR_CREATION_TIMEOUT_MS);
					 ...
					 */
					const handleCreatePRProcessExit = async (
						code: number | null,
						eventSource: "close" | "exit",
					): Promise<void> => {
						if (resolved) return;
						resolved = true;
						if (timeoutId) clearTimeout(timeoutId);
						...
						if (code === 0) {
							...
							if (result) {
								...
								resolve({
									success: false,
									error: stripAnsiCodes(
										stdout || stderr || "Failed to create PR",
									),
								});
							}
						}
	...
	ipcMain.handle(
		...
		async (
			...
			IPCResult<import("../../../shared/types").WorktreeSyncResult>
		> => {
			try {
				const { task, project } = findTaskAndProject(taskId);
				if (!task || !project) {
				...
				if (!worktreePath) {
					return {
						success: false,
						error:
							"No worktree found for this task. The task must be running or in review.",
					};
				}
				...
				if (!sourceBranch || !GIT_BRANCH_REGEX.test(sourceBranch)) {
					return {
						success: false,
						error: `Invalid branch name: ${sourceBranch}`,
					};
				}
				...
				return {
					success: false,
					error:
						stripAnsiCodes(output.trim()) ||
						`${strategy} failed with exit code ${result.status}`,
				};
			} catch (error) {
