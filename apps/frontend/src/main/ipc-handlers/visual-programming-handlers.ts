...
export function registerVisualProgrammingHandlers(
	_getMainWindow: () => import("electron").BrowserWindow | null,
): void {
	ipcMain.handle(
		"visualProgramming:run",
		async (_event, request: VisualProgrammingRequest) => {
			try {
	...
	ipcMain.handle(
		"visualProgramming:configure",
		async (_event, config: { pythonPath?: string; sourcePath?: string }) => {
			try {
				visualProgrammingService.configure(
					config.pythonPath,
					config.sourcePath,
				);
				return { success: true };
...
export function setupVisualProgrammingEventForwarding(
