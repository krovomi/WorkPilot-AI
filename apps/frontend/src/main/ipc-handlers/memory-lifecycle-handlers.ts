		...
		proc.on("close", (code) => {
			...
			if (code === 0) {
				try {
					...
					resolve({ success: true, data: { message: out } });
				}
			} else {
...
export function registerMemoryLifecycleHandlers(): void {
			...
			if (fs.existsSync(storageDir)) {
				for (const f of fs.readdirSync(storageDir)) {
					if (f.endsWith(".json")) {
						episodeCount++;
						try {
							diskUsageBytes += fs.statSync(path.join(storageDir, f)).size;
						} catch {
							/* ignore */
						}
					}
	...
	ipcMain.handle(
		...
		(_event, projectDir: string, page: number = 0, pageSize: number = 50) => {
			try {
				const storageDir = path.join(projectDir, ".workpilot", "memory");
				if (!fs.existsSync(storageDir))
				...
				const items = slice.map((f) => {
					try {
						const raw = JSON.parse(
							fs.readFileSync(path.join(storageDir, f), "utf-8"),
						);
	...
	ipcMain.handle(
		...
		async (
			...
			return runRunner(projectDir, [
				...
				...(options.max_age_days === undefined
					? []
					: ["--max-age-days", String(options.max_age_days)]),
				...(options.max_count === undefined
					? []
					: ["--max-count", String(options.max_count)]),
	...
	ipcMain.handle(
		...
		(_event, projectDir: string, episodeId: string) => {
			try {
	...
	ipcMain.handle(
		...
		(_event, projectDir: string, outputPath: string) => {
			try {
				const storageDir = path.join(projectDir, ".workpilot", "memory");
				...
				if (fs.existsSync(storageDir)) {
					for (const f of fs
						...
						try {
							memories.push(
								JSON.parse(fs.readFileSync(path.join(storageDir, f), "utf-8")),
							);
						} catch {
							/* skip */
						}
				...
				return {
					success: true,
					data: { count: memories.length, path: outputPath },
				};
			} catch (err) {
				return { success: false, error: String(err) };
			}
