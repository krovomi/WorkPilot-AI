...
export function registerSelfHealingHandlers(
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_GET_DASHBOARD,
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_GET_INCIDENTS,
		async (_, projectPath: string, mode?: string) => {
			try {
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_GET_OPERATIONS,
		async (_, projectPath: string) => {
			try {
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_GET_FRAGILITY,
		async (_, projectPath: string) => {
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_CICD_ENABLE,
		async (_, projectPath: string) => {
			try {
				const config = await updateModeConfig(projectPath, "cicd", {
					enabled: true,
				});
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_CICD_DISABLE,
		async (_, projectPath: string) => {
			try {
				const config = await updateModeConfig(projectPath, "cicd", {
					enabled: false,
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_CICD_CONFIG,
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_PRODUCTION_CONNECT,
		async (_, projectPath: string, sourceConfig: Record<string, unknown>) => {
			try {
				const source =
					typeof sourceConfig.source === "string" ? sourceConfig.source : "";
				const result = await runSelfHealingCommand(projectPath, [
					"connect",
					"--source",
					source,
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_PRODUCTION_DISCONNECT,
		async (_, projectPath: string, source: string) => {
			try {
				const result = await runSelfHealingCommand(projectPath, [
					"disconnect",
					"--source",
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_PRODUCTION_CONFIG,
		async (_, projectPath: string, config: Record<string, unknown>) => {
			try {
				const merged = await updateModeConfig(
					projectPath,
					"production",
					config,
				);
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_PROACTIVE_SCAN,
		async (_, projectPath: string) => {
			...
			mainWindow?.webContents.send(
				IPC_CHANNELS.SELF_HEALING_OPERATION_COMPLETE,
				{
					mode: "proactive",
					success: true,
				},
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_PROACTIVE_CONFIG,
		async (_, projectPath: string, config: Record<string, unknown>) => {
			try {
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_TRIGGER_FIX,
		async (_, projectPath: string, incidentId: string) => {
			const mainWindow = getMainWindow();
			try {
				await runSelfHealingCommandStreaming(
					...
					(progress) => {
						mainWindow?.webContents.send(
							IPC_CHANNELS.SELF_HEALING_OPERATION_PROGRESS,
							{
								incidentId,
								...progress,
							},
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_CANCEL_OPERATION,
		async (_, projectPath: string, operationId: string) => {
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_DISMISS_INCIDENT,
		async (_, projectPath: string, incidentId: string) => {
			try {
				const result = await runSelfHealingCommand(projectPath, [
					"dismiss",
	...
	ipcMain.handle(
		IPC_CHANNELS.SELF_HEALING_RETRY_INCIDENT,
		async (_, projectPath: string, incidentId: string) => {
			const mainWindow = getMainWindow();
			try {
				await runSelfHealingCommandStreaming(
 ...
 */
async function runBuiltinFragilityScan(
	projectPath: string,
): Promise<FragilityReportData[]> {
	...
	return results
		.filter(
			(r): r is FragilityReportData => r !== null && r.risk_score >= threshold,
		)
...
async function getEmptyDashboardWithPersistedData(projectPath: string) {
	const dataDir = path.join(projectPath, ".workpilot", "self-healing");
	const incidents = (await readJsonFile(
		path.join(dataDir, "incidents.json"),
	)) as Record<string, unknown>[];
	const operations = (await readJsonFile(
		path.join(dataDir, "operations.json"),
	)) as Record<string, unknown>[];
	const fragilityReports = await readJsonFile(
		path.join(dataDir, "fragility_reports.json"),
	);
	...
	return {
		...
		proactiveSummary: {
			scanned: fragilityReports.length > 0,
			files_at_risk: fragilityReports.length,
			avg_risk: 0,
			max_risk: 0,
			top_files: [],
		},
...
function runSelfHealingCommand(
	projectPath: string,
	args: string[],
): Promise<string> {
	return new Promise((resolve, reject) => {
		...
		const proc = spawn(
			pythonPath,
			[getRunnerPath(), "--project", projectPath, ...args],
			{
				cwd: backendDir,
				stdio: ["ignore", "pipe", "pipe"],
				timeout: 120000,
		...
		proc.on("close", (code: number | null) => {
			if (code === 0) {
				...
				reject(
					new Error(`Self-healing runner exited with code ${code}: ${stderr}`),
				);
			}
		});
