...
vi.mock("../utils/subprocess-runner", () => ({
	...
	validateGitHubModule: (...args: unknown[]) =>
		mockValidateGitHubModule(...args),
...
function createProject(): Project {
	const projectPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "github-env-test-"),
	);
...
describe("GitHub runner env usage", () => {
	...
	it("passes runner env to autofix analyze preview subprocess", async () => {
		...
		const { AgentManager: MockedAgentManager } = await import(
			"../../../agent/agent-manager"
		);
