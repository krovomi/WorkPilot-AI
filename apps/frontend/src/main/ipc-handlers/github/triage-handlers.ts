			...
			if (file.startsWith("triage_") && file.endsWith(".json")) {
				try {
					const data = JSON.parse(
						fs.readFileSync(path.join(issuesDir, file), "utf-8"),
					);
					results.push({
...
async function runTriage(
	...
	const additionalArgs = issueNumbers
		? issueNumbers.map((n) => n.toString())
		: [];
	if (applyLabels) {
...
export function registerTriageHandlers(
	...
	ipcMain.handle(
		...
		async (_, projectId: string, config: TriageConfig): Promise<boolean> => {
			debugLog("saveTriageConfig handler called", {
				projectId,
	...
	ipcMain.handle(
		...
		async (_, projectId: string, issueNumbers: number[]): Promise<boolean> => {
			...
			const applyResult = await withProjectOrNull(
				projectId,
				async (project) => {
					...
					try {
						for (const issueNumber of issueNumbers) {
							...
							if (result && result.labelsToAdd.length > 0) {
								debugLog("Applying labels to issue", {
									issueNumber,
								...
								if (safeLabels.length !== result.labelsToAdd.length) {
									debugLog(
										"Some labels were filtered due to invalid characters",
										{
