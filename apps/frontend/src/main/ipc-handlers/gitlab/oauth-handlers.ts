...
function _redactSensitiveData(data: unknown): unknown {
	if (typeof data === "string") {
		...
		return data
			...
			.replace(
				/private[_-]?token[=:]\s*["']?[A-Za-z0-9_-]+["']?/gi,
...
export function registerCheckGlabAuth(): void {
	ipcMain.handle(
		...
		async (
			...
			const hostname = instanceUrl
				? getHostnameFromUrl(instanceUrl)
				: "gitlab.com";
...
export function registerStartGlabAuth(): void {
	ipcMain.handle(
		...
		async (
			_event,
			instanceUrl?: string,
		): Promise<IPCResult<GitLabAuthStartResult>> => {
					...
					glabProcess.on("close", (code) => {
						...
						if (code === 0) {
							...
							resolve({
								success: false,
								error:
									errorOutput || `Authentication failed with exit code ${code}`,
								data: {
									deviceCode: "",
									verificationUrl: deviceUrl,
									userCode: "",
...
export function registerGetGlabToken(): void {
	ipcMain.handle(
		...
		async (
			_event,
			instanceUrl?: string,
		): Promise<IPCResult<{ token: string }>> => {
...
export function registerDetectGitLabProject(): void {
	ipcMain.handle(
		...
		async (
			_event,
			projectPath: string,
		): Promise<IPCResult<{ project: string; instanceUrl: string }>> => {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				const args = [
					"api",
					`projects/${encodedProject}/repository/branches`,
					"--paginate",
					"--jq",
					".[].name",
				];
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				const webUrl = urlMatch
					? urlMatch[0]
					: `https://${hostname}/${options.namespace || ""}/${projectName}`;
				const pathWithNamespace = options.namespace
...
export function registerAddGitLabRemote(): void {
	ipcMain.handle(
		...
		async (
			...
			debugLog("addGitLabRemote handler called", {
				projectPath,
				projectFullPath,
				instanceUrl,
			});
		...
		> => {
			...
			try {
				const args = [
					...
					"groups",
					"--jq",
					".[] | {id: .id, name: .name, path: .path, fullPath: .full_path}",
				];
				if (hostname !== "gitlab.com") {
					args.push("--hostname", hostname);
				}
