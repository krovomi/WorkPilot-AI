...
function extractPeerInfo(
	...
	teamId: string,
	memberId: string,
): { peers: string[]; localEpisodeCount: number; lastExport: string | null } {
	const teamDir = path.join(syncPath, teamId);
	const result: {
		peers: string[];
		localEpisodeCount: number;
		lastExport: string | null;
	} = { peers: [], localEpisodeCount: 0, lastExport: null };
	...
	try {
		...
		result.peers = fs
			...
			.filter(
				(f) =>
					f.endsWith("_snapshot.json") && f !== `${memberId}_snapshot.json`,
			)
...
export function registerTeamSyncHandlers(): void {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				return await runPythonRunner(projectDir, [
					"--push",
					"--project",
					projectDir,
					"--output-json",
				...
				for (const f of fs.readdirSync(teamDir)) {
					...
					try {
						const snap = JSON.parse(
							fs.readFileSync(path.join(teamDir, f), "utf-8"),
						);
						peers.push({
						...
						peers.push({
							member_id: f.replace("_snapshot.json", ""),
							exported_at: null,
							episode_count: 0,
							project: "",
							is_self: false,
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				_serverProcess.on("exit", () => {
					_serverProcess = null;
				});
				_serverProcess.stderr?.on("data", (d: Buffer) =>
					console.warn("[TeamSync Server]", d.toString().trim()),
				);
				// Give it a moment to start
				await new Promise((r) => setTimeout(r, 800));
				const sender = event.sender;
				sender.send(IPC_CHANNELS.TEAM_SYNC_SERVER_STATUS, {
					running: true,
					port: _serverPort,
				});
				return { success: true, port: _serverPort };
	...
	ipcMain.handle(
		...
		(event): { success: boolean } => {
			...
			event.sender.send(IPC_CHANNELS.TEAM_SYNC_SERVER_STATUS, {
				running: false,
				port: _serverPort,
			});
			return { success: true };
