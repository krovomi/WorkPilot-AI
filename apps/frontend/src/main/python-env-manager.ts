...
export class PythonEnvManager extends EventEmitter {
	...
	private hasBundledPackages(): boolean {
		...
		const packageExists = (pkg: string): boolean => {
			...
			return (
				(existsSync(pkgPath) && existsSync(initPath)) || existsSync(moduleFile)
			);
		};

		const missingPackages = criticalPackages.filter(
			(pkg) => !packageExists(pkg),
		);
		const missingOptional = optionalPackages.filter(
			(pkg) => !packageExists(pkg),
		);
	...
	private findSystemPython(): string | null {
		...
		try {
			...
			console.error(
				`[PythonEnvManager] Failed to get Python path for ${pythonCmd}:`,
				err,
			);
			return null;
		}
	...
	private async createVenv(): Promise<boolean> {
		...
		const venvPath = this.getVenvBasePath()!;
		console.warn(
			"[PythonEnvManager] Creating venv at:",
			venvPath,
			"with:",
			systemPython,
		);
		...
		return new Promise((resolve) => {
			...
			const timeoutId = setTimeout(() => {
				if (!resolved) {
					resolved = true;
					console.error(
						"[PythonEnvManager] Venv creation timed out after",
						PythonEnvManager.VENV_CREATION_TIMEOUT_MS,
						"ms",
					);
					this.emit(
						"error",
						"Virtual environment creation timed out. This may indicate a system issue.",
					);
					try {
						proc.kill();
					} catch {
			...
			proc.on("error", (err) => {
				...
				this.emit(
					"error",
					`Failed to create virtual environment: ${err.message}`,
				);
				resolve(false);
			});
		});
	...
	private async installDeps(): Promise<boolean> {
		...
		console.warn(
			"[PythonEnvManager] Installing dependencies from:",
			requirementsPath,
		);
		...
		return new Promise((resolve) => {
			...
			proc.on("close", (code) => {
				if (code === 0) {
					console.warn(
						"[PythonEnvManager] Dependencies installed successfully",
					);
					this.emit("status", "Dependencies installed successfully");
					resolve(true);
				} else {
					console.error(
						"[PythonEnvManager] Failed to install deps:",
						stderr || stdout,
					);
					this.emit(
						"error",
	...
	async initialize(autoBuildSourcePath: string): Promise<PythonEnvStatus> {
		...
		if (this.initializationPromise) {
			console.warn(
				"[PythonEnvManager] Initialization already in progress, waiting...",
			);
	...
	private async _doInitialize(
		...
		console.warn(
			"[PythonEnvManager] Initializing with path:",
			autoBuildSourcePath,
		);
		...
		try {
			...
			if (app.isPackaged && this.hasBundledPackages()) {
				...
				if (bundledPython && bundledSitePackages) {
					...
					console.warn(
						"[PythonEnvManager] Ready with bundled Python:",
						this.pythonPath,
					);
					console.warn(
						"[PythonEnvManager] Using bundled site-packages:",
			...
			console.warn(
				"[PythonEnvManager] Using venv-based setup (development mode or bundled packages missing)",
			);
			...
			if (!depsInstalled) {
				console.warn(
					"[PythonEnvManager] Dependencies not installed, installing...",
				);
			...
			this.emit("ready", this.pythonPath);
			console.warn(
				"[PythonEnvManager] Ready with Python path:",
				this.pythonPath,
			);
	...
	getPythonEnv(): Record<string, string> {
		...
		if (this.sitePackagesPath && isWindows()) {
			...
			pythonPath = [this.sitePackagesPath, win32Path, win32LibPath].join(
				pathSep,
			);
		}
		...
		if (this.sitePackagesPath && isWindows()) {
			const pywin32System32 = path.join(
				this.sitePackagesPath,
				"pywin32_system32",
			);
			...
			const pathKey = Object.keys(baseEnv).find(
