...
import type { Idea, IdeationConfig } from "../../shared/types";
...
function formatStatusMessage(log: string): string {
	if (!log) return "";
	return stripAnsiCodes(log.trim())
		.split("\n")[0]
		.substring(0, STATUS_MESSAGE_MAX_LENGTH);
}
...
export class AgentQueueManager {
		...
		try {
			...
			if (activeProfile?.configDir) {
				...
				if (tokenResult.wasRefreshed) {
					...
					debugLog(
						"[Agent Queue] Force-refreshed OAuth token before spawn for profile:",
						activeProfile.name,
					);
				} else if (
					...
					debugLog(
						"[Agent Queue] PERMANENT TOKEN ERROR:",
						tokenResult.error,
			...
			debugLog(
				"[Agent Queue] Pre-spawn token refresh failed (non-fatal):",
	...
	async startRoadmapGeneration(
		...
		const roadmapRunnerPath = path.join(
			autoBuildSource,
			"runners",
			"roadmap_runner.py",
		);
		...
		if (!existsSync(roadmapRunnerPath)) {
			debugError(
				"[Agent Queue] Roadmap runner not found at:",
				roadmapRunnerPath,
			);
			this.emitter.emit(
				"roadmap-error",
				projectId,
				`Roadmap runner not found at: ${roadmapRunnerPath}`,
			);
			return;
		}
	...
	async startIdeationGeneration(
		...
		const ideationRunnerPath = path.join(
			autoBuildSource,
			"runners",
			"ideation_runner.py",
		);
		...
		if (!existsSync(ideationRunnerPath)) {
			debugError(
				"[Agent Queue] Ideation runner not found at:",
				ideationRunnerPath,
			);
			this.emitter.emit(
				"ideation-error",
				projectId,
				`Ideation runner not found at: ${ideationRunnerPath}`,
			);
			return;
		}
	...
	private async spawnIdeationProcess(
		...
		debugLog("[Agent Queue] Spawning ideation process:", {
			projectId,
			projectPath,
		});
		...
		childProcess.stdout?.on("data", (data: Buffer) => {
			...
			if (typeCompleteMatch) {
				...
				loadIdeationType().catch((err: unknown) => {
					debugError(
						"[Agent Queue] Unhandled error in ideation type handler (event already emitted):",
						{
							ideationType,
							projectId,
							typeFilePath,
		...
		childProcess.on("exit", (code: number | null) => {
			debugLog("[Agent Queue] Ideation process exited:", {
				projectId,
				code,
				spawnId,
			});
			...
			if (wasIntentionallyStopped) {
				debugLog(
					"[Agent Queue] Ideation process was intentionally stopped, ignoring exit",
				);
			...
			if (code === 0) {
				...
				if (storedProjectPath) {
					try {
						...
						debugLog(
							"[Agent Queue] Loading ideation session from:",
							ideationFilePath,
						);
						if (existsSync(ideationFilePath)) {
							const loadSession = async (): Promise<void> => {
								try {
									...
									debugError(
										"[Ideation] Failed to load ideation session:",
										err,
									);
									this.emitter.emit(
										"ideation-error",
							...
							loadSession().catch((err: unknown) => {
								debugError(
									"[Agent Queue] Unhandled error loading ideation session:",
									err,
								);
							});
						} else {
							debugError(
								"[Ideation] ideation.json not found at:",
								ideationFilePath,
							);
							this.emitter.emit(
								"ideation-error",
						...
						debugError(
							"[Ideation] Unexpected error in ideation completion:",
							err,
						);
						this.emitter.emit(
							"ideation-error",
				...
				debugError("[Agent Queue] Ideation generation failed:", {
					projectId,
					code,
				});
				this.emitter.emit(
	...
	private async spawnRoadmapProcess(
		...
		debugLog("[Agent Queue] Spawning roadmap process:", {
			projectId,
			projectPath,
		});
		...
		if (wasKilled) {
			debugLog(
				"[Agent Queue] Killed existing roadmap process for project:",
				projectId,
			);
		}
		...
		childProcess.stdout?.on("data", (data: Buffer) => {
			...
			// Parse progress using AgentEvents
			const progressUpdate = this.events.parseRoadmapProgress(
				log,
				progressPhase,
				progressPercent,
			);
		...
		// Handle process exit
		childProcess.on("exit", (code: number | null) => {
			debugLog("[Agent Queue] Roadmap process exited:", {
				projectId,
				code,
				spawnId,
			});
			...
			if (wasIntentionallyStopped) {
				debugLog(
					"[Agent Queue] Roadmap process was intentionally stopped, ignoring exit",
				);
				this.state.clearKilledSpawn(spawnId);
			...
			if (code === 0) {
				...
				if (storedProjectPath) {
					try {
						...
						if (existsSync(roadmapFilePath)) {
							...
							loadRoadmap().catch((err: unknown) => {
								debugError(
									"[Agent Queue] Unhandled error loading roadmap:",
									err,
								);
							});
						} else {
							debugError(
								"[Roadmap] roadmap.json not found at:",
								roadmapFilePath,
							);
							this.emitter.emit(
								"roadmap-error",
						...
						debugError(
							"[Roadmap] Unexpected error in roadmap completion:",
							err,
						);
						this.emitter.emit(
							"roadmap-error",
					...
					debugError(
						"[Roadmap] No project path available for roadmap completion",
					);
					this.emitter.emit(
						"roadmap-error",
						projectId,
						"Roadmap completed but project path not found.",
					);
				}
			} else {
	...
	stopIdeation(projectId: string): boolean {
		...
		debugLog("[Agent Queue] Process running?", {
			projectId,
			isIdeation,
	...
	stopRoadmap(projectId: string): boolean {
		...
		debugLog("[Agent Queue] Roadmap process running?", {
			projectId,
			isRoadmap,
			processType: processInfo?.queueProcessType,
		});
