...
export class VisualProgrammingService extends EventEmitter {
	...
	private getSourcePath(): string | null {
		...
		if (
			existsSync(path.join(resolved, "runners", "visual_programming_runner.py"))
		) {
	...
	async run(request: VisualProgrammingRequest): Promise<void> {
		...
		if (!sourcePath) {
			this.emit(
				"error",
				"WorkPilot AI source not found. Cannot locate visual_programming_runner.py",
			);
			return;
		}
	...
	private async executeProcess(
				...
				if (line.startsWith("__VIS_PROG_RESULT__:")) {
					try {
						const payload = JSON.parse(
							line.substring("__VIS_PROG_RESULT__:".length),
						);
		...
		proc.on("close", (code) => {
			...
			if (code !== 0 && code !== null) {
				this.emit(
					"error",
					`Process exited with code ${code}. ${stderrOutput.slice(-500)}`,
				);
			}
		});
