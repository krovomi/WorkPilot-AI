	...
	try {
		...
		if (remoteConfig?.url) {
			try {
				...
				try {
					const existingRemote = execFileSync(
						git,
					...
					if (existingRemote !== remoteConfig.url) {
						execFileSync(
							git,
							["remote", "set-url", remoteName, remoteConfig.url],
							{
								cwd: projectPath,
								encoding: "utf-8",
								stdio: "pipe",
				...
				try {
					const _currentBranch = execFileSync(
						git,
						["branch", "--show-current"],
						{
							cwd: projectPath,
 ...
 */
const DATA_DIRECTORIES = ["specs", "ideation", "insights", "roadmap"];
