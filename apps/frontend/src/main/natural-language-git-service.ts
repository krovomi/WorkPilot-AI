...
export class NaturalLanguageGitService extends EventEmitter {
	...
	private getAutoBuildSourcePath(): string | null {
		...
		const validatePath = (p: string): boolean => {
			return (
				existsSync(p) &&
				existsSync(path.join(p, "runners", "natural_language_git_runner.py"))
			);
		};
	...
	async execute(request: NaturalLanguageGitRequest): Promise<void> {
		...
		if (!autoBuildSource) {
			this.emit(
				"error",
				"WorkPilot AI source not found. Cannot locate natural_language_git_runner.py",
			);
			return;
		}
		...
		const runnerPath = path.join(
			autoBuildSource,
			"runners",
			"natural_language_git_runner.py",
		);
		if (!existsSync(runnerPath)) {
			this.emit(
				"error",
				"natural_language_git_runner.py not found in auto-claude directory",
			);
			return;
		}
			...
			for (const line of lines) {
				...
				if (line.startsWith("__GIT_RESULT__:")) {
					try {
						...
						console.error(
							"[NaturalLanguageGit] Failed to parse result:",
							parseErr,
						);
					}
		...
		proc.on("close", (code) => {
			...
			if (code === 0 && gitResult) {
				...
				if (
					...
					this.emit(
						"error",
						`Command execution failed (exit code ${code}). ${stderrOutput.slice(-500)}`,
					);
				}
			} else {
				...
				if (fullOutput.trim()) {
					this.emit(
						"error",
						`Command completed but no structured result: ${fullOutput.trim()}`,
					);
				} else {
					this.emit(
						"error",
						"Command execution completed but produced no output.",
					);
				}
			}
		});
