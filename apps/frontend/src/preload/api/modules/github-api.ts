...
export interface GitHubAPI {
	...
	suggestReleaseVersion: (
		projectId: string,
	) => Promise<IPCResult<VersionSuggestion>>;
	...
	onAutoFixError: (
		callback: (
			projectId: string,
			error: { issueNumber: number; error: string },
		) => void,
	) => IpcListenerCleanup;
	...
	approveBatches: (
		projectId: string,
		approvedBatches: ProposedBatch[],
	) => Promise<{ success: boolean; batches?: IssueBatch[]; error?: string }>;
	...
	getWorkflowsAwaitingApproval: (
		projectId: string,
		prNumber: number,
	) => Promise<WorkflowsAwaitingApprovalResult>;
	...
	onPRReviewError: (
		callback: (
			projectId: string,
			error: { prNumber: number; error: string },
		) => void,
	) => IpcListenerCleanup;
}
...
export const createGitHubAPI = (): GitHubAPI => ({
	...
	onAnalyzePreviewProgress: (
		...
		createIpcListener(
			IPC_CHANNELS.GITHUB_AUTOFIX_ANALYZE_PREVIEW_PROGRESS,
			callback,
		),
	...
	onAnalyzePreviewComplete: (
		...
		createIpcListener(
			IPC_CHANNELS.GITHUB_AUTOFIX_ANALYZE_PREVIEW_COMPLETE,
			callback,
		),
	...
	onAnalyzePreviewError: (
		...
		createIpcListener(
			IPC_CHANNELS.GITHUB_AUTOFIX_ANALYZE_PREVIEW_ERROR,
			callback,
		),
