...
export const useIdeationStore = create<IdeationState>((set) => ({
	...
	deleteIdea: (ideaId) =>
		set((state) => {
			...
			const updatedIdeas = state.session.ideas.filter(
				(idea) => idea.id !== ideaId,
			);
...
export function generateIdeation(projectId: string): void {
	...
	const timeoutId = setTimeout(() => {
		...
		if (currentState.generationStatus.phase === "generating") {
			if (window.DEBUG) {
				console.warn(
					"[Ideation] Generation timed out after",
...
export async function dismissAllIdeasForProject(
	projectId: string,
): Promise<boolean> {
	const store = useIdeationStore.getState();
...
export async function archiveIdeaForProject(
	projectId: string,
	ideaId: string,
): Promise<boolean> {
	const store = useIdeationStore.getState();
...
export async function deleteIdeaForProject(
	projectId: string,
	ideaId: string,
): Promise<boolean> {
	const store = useIdeationStore.getState();
...
export async function deleteMultipleIdeasForProject(
	projectId: string,
	ideaIds: string[],
): Promise<boolean> {
	const store = useIdeationStore.getState();
	const result = await window.electronAPI.deleteMultipleIdeas(
		projectId,
		ideaIds,
	);
	if (result.success) {
...
export function getIdeationSummary(
	session: IdeationSession | null,
): IdeationSummary {
	if (!session) {
		return {
...
export function isCodeImprovementIdea(
	idea: Idea,
): idea is Idea & { type: "code_improvements" } {
	return idea.type === "code_improvements";
}

export function isUIUXIdea(
	idea: Idea,
): idea is Idea & { type: "ui_ux_improvements" } {
...
export function setupIdeationListeners(): () => void {
	...
	const unsubProgress = window.electronAPI.onIdeationProgress(
		(projectId, status) => {
	...
	const unsubComplete = window.electronAPI.onIdeationComplete(
		(projectId, session) => {
			// Only process events for the current project
			if (!isCurrentProject(projectId)) {
				if (window.DEBUG) {
