...
export function registerOutputCallback(
	...
	debugLog(
		`[TerminalStore] Registered output callback for terminal: ${terminalId}`,
	);
}
...
export function unregisterOutputCallback(terminalId: string): void {
	...
	debugLog(
		`[TerminalStore] Unregistered output callback for terminal: ${terminalId}`,
	);
}
...
export function writeToTerminal(terminalId: string, data: string): void {
	...
	if (callback) {
		try {
			...
			debugError(
				`[TerminalStore] Error writing to terminal ${terminalId}:`,
				error,
			);
		}
	}
...
export const useTerminalStore = create<TerminalState>((set, get) => ({
	...
	addRestoredTerminal: (session: TerminalSession) => {
		const state = get();
		debugLog(
			`[TerminalStore] addRestoredTerminal called for session: ${session.id}, title: "${session.title}", projectPath: ${session.projectPath}`,
		);
		...
		if (session.outputBuffer) {
			...
			debugLog(
				`[TerminalStore] Restored buffer for terminal ${session.id}, size: ${session.outputBuffer.length} chars`,
			);
		} else {
			debugLog(
				`[TerminalStore] No output buffer to restore for terminal ${session.id}`,
			);
		}
		...
		if (existingTerminal) {
			debugLog(
				`[TerminalStore] Terminal ${session.id} already exists in store, returning existing (buffer was still restored above)`,
			);
			...
			if (
				...
				!existingTerminal.pendingClaudeResume
			) {
				debugLog(
					`[TerminalStore] Updating pendingClaudeResume for existing terminal ${session.id}`,
				);
				set((state) => ({
		...
		debugLog(
			`[TerminalStore] Successfully added restored terminal ${session.id} to store, isRestored: true, claudeSessionId: ${session.claudeSessionId || "none"}, pendingClaudeResume: ${session.isClaudeMode === true}`,
		);
	...
	resumeAllPendingClaude: async () => {
		...
		const pendingTerminals = state.terminals.filter(
			(t) => t.pendingClaudeResume === true,
		);
		...
		debugLog(
			`[TerminalStore] Resuming ${pendingTerminals.length} pending Claude sessions with 500ms stagger`,
		);
		...
		for (let i = 0; i < pendingTerminals.length; i++) {
			...
			debugLog(
				`[TerminalStore] Activating deferred Claude resume for terminal: ${terminal.id}`,
			);
	...
	canAddTerminal: (projectPath?: string) => {
		const state = get();
		return (
			getActiveProjectTerminalCount(state.terminals, projectPath) <
			state.maxTerminals
		);
	},
...
export async function restoreTerminalSessions(
	...
	if (restoringProjects.has(projectPath)) {
		debugLog(
			"[TerminalStore] Already restoring terminals for this project, skipping",
		);
		return;
	}
	...
	try {
		...
		const projectTerminals = store.terminals.filter(
		...
		if (projectTerminals.length > 0) {
			...
			const aliveChecks = await Promise.all(
				projectTerminals.map(async (terminal) => {
					try {
						const result = await globalThis.electronAPI.checkTerminalPtyAlive(
							terminal.id,
						);
						return {
							terminal,
							alive: result.success && result.data?.alive === true,
						};
					} catch {
			...
			debugLog(
				`[TerminalStore] ${deadTerminals.length} terminals had dead PTY, will restore from disk`,
			);
		}

		// Restore from disk
		debugLog(
			`[TerminalStore] Fetching terminal sessions from disk for project: ${projectPath}`,
		);
		const result =
			await globalThis.electronAPI.getTerminalSessions(projectPath);
		if (!result.success || !result.data || result.data.length === 0) {
			debugLog(
				`[TerminalStore] No sessions found on disk for project: ${projectPath}, success: ${result.success}, sessionCount: ${result.data?.length || 0}`,
			);
			return;
		}
		debugLog(
			`[TerminalStore] Found ${result.data.length} sessions on disk for project: ${projectPath}`,
		);
		...
		debugLog(
			`[TerminalStore] Adding ${sortedSessions.length} sorted sessions to store`,
		);
		...
		debugLog(
			`[TerminalStore] Completed terminal session restoration for project: ${projectPath}`,
		);
	} catch (error) {
