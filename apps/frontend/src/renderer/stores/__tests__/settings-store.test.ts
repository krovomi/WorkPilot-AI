...
describe("Settings Store", () => {
	...
	describe("updateSettings", () => {
		it("should merge updates into existing settings", () => {
			...
			useSettingsStore
				.getState()
				.updateSettings({ autoBuildPath: "/new-path" } as Partial<AppSettings>);
		...
		it("should handle multiple sequential updates", () => {
			useSettingsStore
				.getState()
				.updateSettings({ autoBuildPath: "/path1" } as Partial<AppSettings>);
			useSettingsStore
				.getState()
				.updateSettings({ autoBuildPath: "/path2" } as Partial<AppSettings>);
	...
	describe("saveSettings (IPC)", () => {
		it("should save settings and update store on success", async () => {
			...
			expect(useSettingsStore.getState().settings.autoBuildPath).toBe(
				"/saved-path",
			);
		});
	...
	describe("Profile Management", () => {
		...
		describe("saveProfile", () => {
			it("should save a new profile and re-fetch from backend", async () => {
				mockSaveAPIProfile.mockResolvedValue({
					success: true,
					data: mockProfile,
				});
				mockGetAPIProfiles.mockResolvedValue({
			...
			it("should handle save profile failure", async () => {
				...
				expect(result).toBe(false);
				expect(useSettingsStore.getState().profilesError).toBe(
					"Duplicate name",
				);
			});
		});
		...
		describe("updateProfile", () => {
			it("should update existing profile", async () => {
				...
				mockUpdateAPIProfile.mockResolvedValue({
					success: true,
					data: updatedProfile,
				});
				...
				expect(result).toBe(true);
				expect(useSettingsStore.getState().profiles[0].name).toBe(
					"Updated Profile",
				);
			});
		...
		describe("deleteProfile", () => {
			...
			it("should handle delete failure", async () => {
				useSettingsStore.setState({ profiles: [mockProfile] });
				mockDeleteAPIProfile.mockResolvedValue({
					success: false,
		...
		describe("setActiveProfile", () => {
			...
			it("should handle set active profile failure", async () => {
				mockSetActiveAPIProfile.mockResolvedValue({
					success: false,
	...
	describe("Test Connection", () => {
		it("should test connection successfully", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.testConnection("https://api.test.com", "key-123");
		...
		it("should handle connection test failure from IPC", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.testConnection("https://api.test.com", "bad-key");
		...
		it("should handle connection test exception", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.testConnection("https://api.test.com", "key");
		...
		it("should set isTestingConnection during test", async () => {
			...
			const testCall = useSettingsStore
				.getState()
				.testConnection("https://api.test.com", "key");
	...
	describe("Model Discovery", () => {
		...
		it("should discover models successfully", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "sk-key");
		...
		it("should cache discovered models", async () => {
			...
			await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "sk-test-key");
		...
		it("should not cache across different API keys", async () => {
			...
			await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "sk-key1");
			await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "sk-key2");
		...
		it("should handle discovery failure", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "bad-key");
		...
		it("should handle discovery exception", async () => {
			...
			const result = await useSettingsStore
				.getState()
				.discoverModels("https://api.openai.com", "key");
	...
	describe("setProviderPriorityOrder", () => {
		it("should update provider priority order in settings", () => {
			...
			expect(
				useSettingsStore.getState().settings.providerPriorityOrder,
			).toEqual(order);
		});
