...
function validatePlanData(plan: ImplementationPlan): boolean {
	...
	if (!plan.phases || !Array.isArray(plan.phases)) {
		console.warn(
			"[validatePlanData] Invalid plan: missing or invalid phases array",
		);
		return false;
	}
	...
	for (let i = 0; i < plan.phases.length; i++) {
		...
		if (!phase?.subtasks || !Array.isArray(phase.subtasks)) {
			console.warn(
				`[validatePlanData] Invalid phase ${i}: missing or invalid subtasks array`,
			);
			return false;
		}
		...
		for (let j = 0; j < phase.subtasks.length; j++) {
			...
			if (!subtask || typeof subtask !== "object") {
				console.warn(
					`[validatePlanData] Invalid subtask at phase ${i}, index ${j}: not an object`,
				);
				return false;
			}
			...
			if (
				...
				console.warn(
					`[validatePlanData] Invalid subtask at phase ${i}, index ${j}: missing or empty description`,
				);
				return false;
			}
		}
...
export const useTaskStore = create<TaskState>((set, get) => ({
	...
	setTasks: (tasks) => {
		...
		tasks.forEach((task) => {
			if (task.logs && task.logs.length > 0) {
				debugLog(
					`[TaskStore.setTasks] Task ${task.id} has ${task.logs.length} logs:`,
					{
		...
		set((state) => {
			...
			if (taskOrder) {
				...
				if (newTaskOrder[status]) {
					...
					newTaskOrder[status] = newTaskOrder[status].filter(
						(id) => id !== task.id,
					);
					// Add to top (index 0)
	...
	updateTaskStatus: (taskId, status, reviewReason) => {
		...
		debugLog("[updateTaskStatus] START:", {
			...
			oldStatus,
			newStatus: status,
			allInProgress: state.tasks
				.filter((t) => t.status === "in_progress" && !t.metadata?.archivedAt)
				.map((t) => t.id),
		});
	...
	updateTaskFromPlan: (taskId, plan) =>
		set((state) => {
			...
			debugLog("[updateTaskFromPlan] called with plan:", {
				...
				totalSubtasks:
					plan.phases?.reduce((acc, p) => acc + (p.subtasks?.length || 0), 0) ||
					0,
			...
			if (!validatePlanData(plan)) {
				console.error(
					"[updateTaskFromPlan] Invalid plan data, skipping update:",
					{
						taskId,
						plan,
					},
	...
	updateExecutionProgress: (taskId, progress) => {
		...
		const handleSequenceValidation = (
			incomingSeq: number,
			currentSeq: number,
			taskId: string,
			progress: Partial<ExecutionProgress>,
			existingProgress: ExecutionProgress,
		) => {
			const normalizedCurrentSeq = isValidSequence(currentSeq) ? currentSeq : 0;

			if (
				incomingSeq > 0 &&
				normalizedCurrentSeq > 0 &&
				incomingSeq < normalizedCurrentSeq
			) {
				console.warn(
					"[updateExecutionProgress] Dropping out-of-order update:",
					{
						taskId,
						incomingSeq,
		...
		set((state) => {
			...
			return {
				tasks: updateTaskAtIndex(state.tasks, index, (t) => {
					...
					const phaseChanged =
						progress.phase && progress.phase !== existingProgress.phase;
	...
	batchAppendLogs: (taskId, logs) => {
		...
		return set((state) => {
			if (logs.length === 0) {
				debugLog(
					"[TaskStore.batchAppendLogs] No logs to append for task:",
					taskId,
				);
				return state;
			}
	...
	moveTaskToColumnTop: (taskId, targetStatus, sourceStatus) => {
		set((state) => {
			...
			if (sourceStatus && newTaskOrder[sourceStatus]) {
				newTaskOrder[sourceStatus] = newTaskOrder[sourceStatus].filter(
					(id) => id !== taskId,
				);
			}
			...
			if (newTaskOrder[targetStatus]) {
				...
				newTaskOrder[targetStatus] = newTaskOrder[targetStatus].filter(
					(id) => id !== taskId,
				);
				// Add to top (index 0)
	...
	loadTaskOrder: (projectId) => {
		...
		const validateAndMergeOrder = (
			parsed: Record<string, unknown>,
		): TaskOrderState => {
			const emptyOrder = createEmptyTaskOrder();

			return {
				backlog: isValidColumnArray(parsed.backlog)
					? parsed.backlog
					: emptyOrder.backlog,
				queue: isValidColumnArray(parsed.queue)
					? parsed.queue
					: emptyOrder.queue,
				in_progress: isValidColumnArray(parsed.in_progress)
					? parsed.in_progress
					: emptyOrder.in_progress,
				ai_review: isValidColumnArray(parsed.ai_review)
					? parsed.ai_review
					: emptyOrder.ai_review,
				...
				pr_created: isValidColumnArray(parsed.pr_created)
					? parsed.pr_created
					: emptyOrder.pr_created,
				error: isValidColumnArray(parsed.error)
					? parsed.error
					: emptyOrder.error,
			};
		};
		...
		const handleParsedData = (parsed: unknown) => {
			...
			if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
				debugWarn(
					"[TaskStore.loadTaskOrder] Invalid task order data in localStorage, resetting to empty",
				);
				set({ taskOrder: createEmptyTaskOrder() });
				return;
			}

			const validatedOrder = validateAndMergeOrder(
				parsed as Record<string, unknown>,
			);

			debugLog("[TaskStore.loadTaskOrder] Loaded task order:", {
				projectId,
				columnCounts: Object.entries(validatedOrder).map(([col, ids]) => ({
					col,
					count: ids.length,
				})),
			});
		...
		try {
			...
			debugLog("[TaskStore.loadTaskOrder] Loading task order:", {
				projectId,
				key,
			});
			...
			if (stored) {
				...
				debugLog(
					"[TaskStore.loadTaskOrder] No stored task order found, using empty order",
				);
				set({ taskOrder: createEmptyTaskOrder() });
			}
		} catch (error) {
	...
	try {
		...
		if (result.success && result.data) {
			debugLog("[TaskStore.loadTasks] Tasks loaded successfully:", {
				...
				tasksWithLogs: result.data.filter((t) => t.logs && t.logs.length > 0)
					.length,
				totalLogCount: result.data.reduce(
					(sum, t) => sum + (t.logs?.length || 0),
					0,
				),
			});
...
export async function submitReview(
	...
	try {
		const result = await globalThis.electronAPI.submitReview(
			taskId,
			approved,
			feedback,
			images,
...
export async function persistTaskStatus(
	...
	try {
		...
		const result = await globalThis.electronAPI.updateTaskStatus(
			taskId,
			status,
			options,
		);
...
export async function recoverStuckTask(
	...
	try {
		const result = await globalThis.electronAPI.recoverStuckTask(taskId, {
			autoRestart: true,
			...options,
		});
...
export async function restoreTask(
	...
	try {
		...
		if (!projectId) {
			return {
				success: false,
				error: "No project ID found for task restoration",
			};
		}
 ...
 */
export function getDeletedTaskBackup(
	taskId: string,
): { task: Task; timestamp: number } | undefined {
...
export async function deleteTasks(
	...
	try {
		...
		store.setTasks(
			store.tasks.filter(
				(t) => !deletedIds.has(t.id) && !deletedIds.has(t.specId || ""),
			),
		);
...
export async function archiveTasks(
	...
	try {
		const result = await globalThis.electronAPI.archiveTasks(
			projectId,
			taskIds,
			version,
		);
