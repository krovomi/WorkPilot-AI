...
export async function startAppEmulator(projectDir: string): Promise<void> {
	...
	try {
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const result = await (globalThis as any).electronAPI.detectAppProject(
			projectDir,
		);
		...
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const startResult = await (globalThis as any).electronAPI.startAppEmulator(
			config,
		);
		if (!startResult.success) {
			...
			if (
				startResult.error !== "Already starting — ignoring duplicate request"
			) {
...
export function setupAppEmulatorListeners(): () => void {
	...
	if (_listenerRefCount === 1) {
		...
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const unsubStatus = (globalThis as any).electronAPI.onAppEmulatorStatus(
			(status: string) => {
				store().setStatus(status);
			},
		);
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const unsubReady = (globalThis as any).electronAPI.onAppEmulatorReady(
			(url: string) => {
				store().setUrl(url);
		...
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const unsubOutput = (globalThis as any).electronAPI.onAppEmulatorOutput(
			(line: string) => {
				store().appendOutput(line);
			},
		);
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const unsubError = (globalThis as any).electronAPI.onAppEmulatorError(
			(error: string) => {
		...
		// biome-ignore lint/suspicious/noExplicitAny: TODO: type this properly
		const unsubConfig = (globalThis as any).electronAPI.onAppEmulatorConfig(
