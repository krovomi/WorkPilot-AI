...
function queueUpdate(taskId: string, update: BatchedUpdate): void {
	...
	if (update.progress?.phase && storeActionsRef) {
		...
		if (update.progress.phase !== currentPhase) {
			...
			if (window.DEBUG) {
				console.warn(
					`[IPC Batch] Phase change detected: ${currentPhase} → ${update.progress.phase}, applying immediately`,
				);
			}
...
export function useIpcListeners(): void {
	...
	const updateExecutionProgress = useTaskStore(
	...
	useEffect(() => {
		...
		const showSDKRateLimitModal =
			useRateLimitStore.getState().showSDKRateLimitModal;
		...
		const showAuthFailureModal =
			useAuthFailureStore.getState().showAuthFailureModal;
...
export function useAppSettings() {
	...
	const saveSettings = async (
		settings: Parameters<typeof window.electronAPI.saveSettings>[0],
	) => {
