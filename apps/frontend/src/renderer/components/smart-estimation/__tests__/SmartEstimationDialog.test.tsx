...
describe("SmartEstimationDialog", () => {
	...
	describe("Dialog Rendering", () => {
		...
		it("should display task description input", () => {
			...
			expect(
				screen.getByText("smartEstimation:task.label"),
			).toBeInTheDocument();
		});
		...
		it("should show project warning when no project selected", () => {
			...
			expect(
				screen.getByText("smartEstimation:errors.noProject"),
			).toBeInTheDocument();
		});
	});
	...
	describe("Estimation Actions", () => {
		it("should enable estimate button with valid input and project", async () => {
			...
			await waitFor(() => {
				const estimateButton = screen.getByText(
					"smartEstimation:actions.estimate",
				);
	...
	describe("Analysis State", () => {
		...
		it("should show streaming output when available", () => {
			...
			expect(
				screen.getByText(
					(content) =>
						content.includes("Analysis step 1") &&
						content.includes("Analysis step 2"),
				),
			).toBeInTheDocument();
			expect(
				screen.getByText("smartEstimation:result.analysisProgress"),
			).toBeInTheDocument();
		});
		...
		it("should show try again button on error", () => {
			...
			expect(
				screen.getByText("smartEstimation:status.error"),
			).toBeInTheDocument();
			expect(screen.getByText("Something went wrong")).toBeInTheDocument();
			expect(
				screen.getByText("smartEstimation:actions.tryAgain"),
			).toBeInTheDocument();
		});
		...
		it("should reset and try again when try again clicked", async () => {
			...
			const tryAgainButton = screen.getByText(
				"smartEstimation:actions.tryAgain",
			);
	...
	describe("Results Display", () => {
		...
		it("should display reasoning", () => {
			...
			expect(
				screen.getByText("smartEstimation:result.reasoning"),
			).toBeInTheDocument();
		...
		it("should display risk factors", () => {
			...
			expect(
				screen.getByText("smartEstimation:result.riskFactors"),
			).toBeInTheDocument();
		...
		it("should display recommendations", () => {
			...
			expect(
				screen.getByText("smartEstimation:result.recommendations"),
			).toBeInTheDocument();
		...
		it("should display similar tasks", () => {
			...
			expect(
				screen.getByText("smartEstimation:result.similarTasks"),
			).toBeInTheDocument();
			expect(screen.getByText("Similar task")).toBeInTheDocument();
			expect(
				screen.getByText(
					(content) => content.includes("80") && content.includes("% similar"),
				),
			).toBeInTheDocument();
		});
	...
	describe("Edge Cases", () => {
		it("should handle missing optional result fields", () => {
			...
			expect(
				screen.getByText(
					(content) => content.includes("70") && content.includes("confidence"),
				),
			).toBeInTheDocument();
		...
		it("should handle empty similar tasks array", () => {
			...
			expect(
				screen.getByText(
					(content) => content.includes("60") && content.includes("confidence"),
				),
			).toBeInTheDocument();
			// Should not show similar tasks section
			expect(
				screen.queryByText("smartEstimation:result.similarTasks"),
