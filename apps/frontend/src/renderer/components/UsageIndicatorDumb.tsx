...
import { Activity, AlertCircle, LogIn, TrendingUp } from "lucide-react";
...
export function UsageIndicatorDumb({
	selectedProvider: propSelectedProvider,
}: UsageIndicatorDumbProps = {}) {
	 ...
	 */
	const _handleProviderSwitch = useCallback(
		async (
			e: React.MouseEvent,
			provider: string,
			type: "oauth" | "api_key",
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				console.error(
					"[UsageIndicatorDumb] Failed to load initial data:",
					error,
				);
				setIsLoading(false);
		...
		const handleCredentialUpdated = (
			newCredential: CredentialConfig | null,
		) => {
				...
				>
					...
					{usageData.usage.needsReauthentication ? (
						...
						<div className="flex items-center gap-0.5 text-xs font-semibold font-mono">
							<span
								className={sessionColorClass}
								title={t("common:usage.sessionShort")}
							>
						...
						<div className="relative">
							<div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-primary/10">
								<span className="text-xs font-semibold text-primary">
									{getInitials(
										credential?.credentials?.profileName ||
