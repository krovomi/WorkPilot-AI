						...
						<Tabs defaultValue="overview" className="flex-1">
							<TabsList className="grid w-full grid-cols-4">
								<TabsTrigger value="overview">
									{t("conflictPredictor:tabs.overview")}
								</TabsTrigger>
								<TabsTrigger value="conflicts">
									{t("conflictPredictor:tabs.conflicts")}
								</TabsTrigger>
								<TabsTrigger value="files">
									{t("conflictPredictor:tabs.files")}
								</TabsTrigger>
								<TabsTrigger value="recommendations">
									{t("conflictPredictor:tabs.recommendations")}
								</TabsTrigger>
							</TabsList>
							...
							<TabsContent value="overview" className="mt-4">
								<div className="space-y-4">
									...
									<Card>
										...
										<CardContent>
											<div
												className={`text-lg font-semibold ${getAssessmentColor(result.summary.risk_assessment)}`}
											>
										...
										<Card>
											<CardContent className="pt-6">
												<div className="text-2xl font-bold">
													{result.total_worktrees}
												</div>
												<p className="text-xs text-muted-foreground">
													{t("conflictPredictor:result.worktrees")}
												</p>
											</CardContent>
										</Card>
										<Card>
											<CardContent className="pt-6">
												...
												<p className="text-xs text-muted-foreground">
													{t("conflictPredictor:result.conflicts")}
												</p>
											</CardContent>
										</Card>
										<Card>
											<CardContent className="pt-6">
												<div className="text-2xl font-bold text-orange-600">
													{result.summary.critical_conflicts +
														result.summary.high_conflicts}
												</div>
												<p className="text-xs text-muted-foreground">
													{t("conflictPredictor:stats.highRisk")}
												</p>
											</CardContent>
										</Card>
										<Card>
											<CardContent className="pt-6">
												<div className="text-2xl font-bold text-green-600">
													{result.modified_files.length}
												</div>
												<p className="text-xs text-muted-foreground">
													{t("conflictPredictor:stats.filesModified")}
												</p>
											</CardContent>
										</Card>
									</div>
									...
									<div className="space-y-3">
										{result.conflicts_detected.length === 0 ? (
											...
											result.conflicts_detected.map((conflict) => (
												<Card
													key={`${conflict.worktree1}-${conflict.worktree2}-${conflict.file_path}`}
												>
													<CardHeader>
														<div className="flex items-center justify-between">
															...
															<Badge
																variant={getRiskLevelColor(conflict.risk_level)}
													...
													<CardContent>
														<div className="space-y-2">
															<div className="flex items-center gap-2 text-sm">
																...
																<span>
																	{conflict.worktree1} vs {conflict.worktree2}
																</span>
															</div>
															...
															<div className="mt-2 p-2 bg-muted rounded text-sm">
																<strong>
																	{t("conflictPredictor:result.resolution")}:
																</strong>{" "}
								...
								<ScrollArea className="h-[400px]">
									<div className="space-y-2">
										{result.modified_files.map((file) => (
											<div
												key={`${file.worktree_name}-${file.file_path}`}
										...
										<CardContent>
											<div className="space-y-3">
												{result.recommendations.map((rec, index) => (
													<div
														key={rec.substring(0, 20).replaceAll(/\s+/g, "-")}
						...
						<Card className="mt-4">
							<CardHeader>
								<CardTitle className="text-sm">
									{t("conflictPredictor:result.streamingOutput")}
								</CardTitle>
							</CardHeader>
							<CardContent>
								<pre
					...
					{phase === "complete" && result && (
						<>
							<Button variant="outline" onClick={handleCopyResults}>
								...
								{copied
									? t("conflictPredictor:actions.copied")
									: t("conflictPredictor:actions.copy")}
							</Button>
