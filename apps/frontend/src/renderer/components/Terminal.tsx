...
export const Terminal = forwardRef<TerminalHandle, TerminalProps>(
	function Terminal(
		...
		const pendingWorktreeConfigRef = useRef<TerminalWorktreeConfig | null>(
			null,
		);
		...
		const lastPtyDimensionsRef = useRef<{ cols: number; rows: number } | null>(
			null,
		);
		...
		const postCreationTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
			null,
		);
		...
		const setAssociatedTask = useTerminalStore(
			(state) => state.setAssociatedTask,
		);
		const setWorktreeConfig = useTerminalStore(
			(state) => state.setWorktreeConfig,
		);
		...
		const showFileDropOverlay =
			(isOver && !isDraggingTerminal) || isNativeDragOver;
		...
		const [readyDimensions, setReadyDimensions] = useState<{
			cols: number;
			rows: number;
		} | null>(null);
		 ...
		 */
		const resizePtyWithTracking = useCallback(
			(cols: number, rows: number, context: string) => {
				...
				globalThis.electronAPI
					...
					.then((result) => {
						...
						if (sequence !== resizeSequenceRef.current) {
							debugLog(
								`[Terminal ${id}] ${context}: Ignoring stale resize result (sequence ${sequence} vs current ${resizeSequenceRef.current})`,
							);
							return;
						}
						...
						if (result.success) {
							...
							debugLog(
								`[Terminal ${id}] ${context} resize failed: ${result.error || "unknown error"}`,
							);
						}
					})
					.catch((error) => {
		...
		const handleDimensionsReady = useCallback(
			(cols: number, rows: number) => {
				...
				if (cols >= MIN_COLS && rows >= MIN_ROWS) {
					debugLog(
						`[Terminal ${id}] handleDimensionsReady: cols=${cols}, rows=${rows} - setting readyDimensions`,
					);
					setReadyDimensions({ cols, rows });
				} else {
					debugLog(
						`[Terminal ${id}] handleDimensionsReady: dimensions below minimum: cols=${cols} (min=${MIN_COLS}), rows=${rows} (min=${MIN_ROWS})`,
					);
				}
			},
			[id],
		...
		const checkDimensionMismatch = useCallback(
			(
				...
				if (colsMismatch || rowsMismatch) {
					...
					const timeSinceAutoCorrect =
						Date.now() - lastAutoCorrectionTimeRef.current;
					if (
						autoCorrect &&
						...
						const now = Date.now();
						if (
							now - autoCorrectionWindowStartRef.current >=
							AUTO_CORRECTION_WINDOW_MS
						) {
							// Log warning if previous window had excessive corrections
							if (
								autoCorrectionCountRef.current >=
								AUTO_CORRECTION_WARNING_THRESHOLD
							) {
								debugLog(
						...
						debugLog(
							`[Terminal ${id}] AUTO-CORRECTING (#${autoCorrectionCountRef.current}): resizing PTY to ${xtermCols}x${xtermRows}`,
						);
		...
		const ptyDimensions = useMemo(() => {
			if (readyDimensions) {
				debugLog(
					`[Terminal ${id}] ptyDimensions memo: using readyDimensions cols=${readyDimensions.cols}, rows=${readyDimensions.rows}`,
				);
			...
			debugLog(
				`[Terminal ${id}] ptyDimensions memo: readyDimensions is null, returning null (skipCreation will be true)`,
			);
			return null;
		}, [readyDimensions, id]);
		...
		const { prepareForRecreate, resetForRecreate } = usePtyProcess({
			...
			onCreated: () => {
				...
				if (
					currentCols !== undefined &&
					currentRows !== undefined &&
					currentCols >= MIN_COLS &&
					currentRows >= MIN_ROWS
				) {
					debugLog(
						`[Terminal ${id}] PTY created - forcing PTY resize to match xterm: cols=${currentCols}, rows=${currentRows}`,
					);
					// Use helper to resize PTY with proper tracking and race condition prevention
					...
					debugLog(
						`[Terminal ${id}] PTY created - no valid dimensions available for tracking (cols=${currentCols}, rows=${currentRows})`,
					);
				}
				...
				if (pendingWorktreeConfigRef.current) {
					...
					try {
						...
						console.error(
							"Failed to sync worktree config after PTY creation:",
							error,
						);
					}
		...
		useEffect(() => {
			...
			const expansionStateChanged =
				!isFirstMount && prevIsExpandedRef.current !== isExpanded;
			debugLog(
				`[Terminal ${id}] Expansion effect: isExpanded=${isExpanded}, isFirstMount=${isFirstMount}, expansionStateChanged=${expansionStateChanged}, prevIsExpanded=${prevIsExpandedRef.current}`,
			);
			...
			const performFit = () => {
				...
				rafId = raf(() => {
					...
					const success = fit();
					debugLog(
						`[Terminal ${id}] performFit: fit returned success=${success}, expansionStateChanged=${expansionStateChanged}, isCreatedRef=${isCreatedRef.current}`,
					);
					...
					if (success) {
						...
						if (
							...
							debugLog(
								`[Terminal ${id}] performFit: Forcing PTY resize to cols=${freshCols}, rows=${freshRows}`,
							);
							// Use helper to resize PTY with proper tracking and race condition prevention
...
Please confirm you're ready by saying: I'm ready to work on ${selectedTask.title} - Context is loaded.`;
		...
		const applyWorktreeConfig = useCallback(
			async (config: TerminalWorktreeConfig) => {
		...
		const handleOpenInIDE = useCallback(async () => {
			...
			try {
				...
				toast({
					...
					description:
						err instanceof Error ? err.message : "Could not launch IDE",
					variant: "destructive",
				});
			}
		}, [
			terminal?.worktreeConfig?.worktreePath,
		...
		const showClaudeBusyIndicator =
			terminal?.isClaudeMode && isClaudeBusy !== undefined;
