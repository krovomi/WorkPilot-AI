...
export function GitHubOAuthFlow({ onSuccess, onCancel }: GitHubOAuthFlowProps) {
	...
	const handleAuthTimeout = useCallback(() => {
		...
		setIsTimeout(true);
		setError(
			"Authentication timed out. The authentication window was open for too long. Please try again.",
		);
		setStatus("error");
		authTimeoutRef.current = null;
	}, []);
	...
	const fetchAndNotifyToken = async () => {
		...
		try {
			...
			if (tokenResult.success && tokenResult.data?.token) {
				debugLog(
					"Token retrieved successfully, calling onSuccess with username:",
					username,
				);
				setStatus("success");
	...
	return (
		<div className="space-y-4">
							...
							<div className="flex items-start gap-4">
								...
								<div className="flex-1 space-y-3">
									...
									<p className="text-sm text-muted-foreground">
										The GitHub CLI (gh) is required for OAuth authentication.
										This provides a secure way to authenticate without manually
										creating tokens.
									</p>
						...
						<CardContent className="p-5">
							<div className="flex items-start gap-4">
								...
								<div className="flex-1 space-y-3">
									...
									<p className="text-sm text-muted-foreground">
										Click the button below to authenticate with GitHub. This
										will open your browser where you can authorize the
										application.
									</p>
									{cliVersion && (
			...
			{status === "error" && error && (
				<div className="space-y-4">
					<Card
						className={`border ${isTimeout ? "border-warning/30 bg-warning/10" : "border-destructive/30 bg-destructive/10"}`}
					>
						<CardContent className="p-5">
							<div className="flex items-start gap-3">
								{isTimeout ? (
								...
								<div className="flex-1">
									<h3
										className={`text-lg font-medium ${isTimeout ? "text-warning" : "text-destructive"}`}
									>
										{isTimeout
									...
									<p
										className={`text-sm mt-1 ${isTimeout ? "text-warning/80" : "text-destructive/80"}`}
									>
										{error}
									</p>
								</div>
								...
								<div className="space-y-4">
									<div className="flex items-start gap-3">
										...
										<div className="flex-1">
											...
											<p className="text-sm text-muted-foreground mt-1">
												The browser couldn't be opened automatically. Please
												visit the URL below to complete authentication:
											</p>
										</div>
									</div>
											...
											<Button
												...
												onClick={async () => {
													try {
														...
														urlCopyTimeoutRef.current = setTimeout(
															() => setUrlCopied(false),
															2000,
														);
													} catch (err) {
