...
export function OllamaModelSelector({
	...
	const installCheckTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	 ...
	 */
	const checkInstalledModels = useCallback(
		async (abortSignal?: AbortSignal) => {
			...
			try {
				...
				const installResult =
					await globalThis.electronAPI.checkOllamaInstalled();
				...
				const statusResult =
					await globalThis.electronAPI.checkOllamaStatus(baseUrl);
				if (abortSignal?.aborted) return;
				...
				const result =
					await globalThis.electronAPI.listOllamaEmbeddingModels(baseUrl);
				if (abortSignal?.aborted) return;
				...
				if (result?.success && result?.data?.embedding_models) {
					...
					setModels(
						RECOMMENDED_MODELS.map((model) => {
							...
							const isInstalled =
								installedFullNames.has(model.name) ||
	...
	return (
		<div className={cn("space-y-3", className)}>
			...
			<p className="text-xs text-muted-foreground">
				Select an installed model for semantic search. Memory works with keyword
				search even without embeddings.
			</p>
		</div>
	);
}
