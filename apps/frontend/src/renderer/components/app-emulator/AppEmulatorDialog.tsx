...
function classifyLogLevel(keyword: string): string {
	...
	if (
		/^(error|err|fatal|critical|crit|panic|exception|severe|fail(ed)?)$/.test(k)
	)
		return "text-red-400";
	if (/^(warn|warning|wrn)$/.test(k)) return "text-amber-400";
	if (/^(info|information|inf|notice)$/.test(k)) return "text-sky-400";
	if (/^(debug|dbg|dbug|trace|trce|trc|verbose|verb|vrb)$/.test(k))
...
function classifyLogLine(line: string): string {
	...
	if (
		/[✓✔✅]/.test(line) ||
		/\b(compiled successfully|ready in|ready on|application started|now listening|server (started|running))\b/i.test(
			line,
		)
	) {
...
export function AppEmulatorDialog() {
	...
	const { isOpen, closeDialog, phase, config, url, output, error, status } =
		useAppEmulatorStore();
	...
	const showPreview =
		resolvedTab === PREVIEW_TAB && phase === "running" && isWeb && !!url;

	// Filter output lines to the active service tab (no filtering for PREVIEW_TAB or null)
	const filteredOutput = useMemo(() => {
		if (!resolvedTab || resolvedTab === PREVIEW_TAB || !isMultiService)
			return output;
	...
	useEffect(() => {
		if (isOpen && phase === "idle") {
			if (selectedProject?.path) {
				...
				console.warn(
					"[AppEmulator] No project path available. selectedProjectId:",
					selectedProjectId,
					"projects:",
					projects.length,
				);
	...
	const handleCopyLogs = useCallback(() => {
		...
		if (navigator.clipboard?.writeText) {
			navigator.clipboard
				.writeText(searchFilteredOutput)
				.then(markCopied)
				.catch(() => {
			...
			<DialogContent className="max-w-5xl h-[80vh] flex flex-col">
							...
							<>
								{services?.map((svc) => (
									...
									>
										...
										{svc.label}
										{svc.port > 0 && (
											<span className="opacity-70">:{svc.port}</span>
										)}
										{svc.isPrimary && (
											<span className="text-xs opacity-70 ml-0.5">API</span>
										)}
									</Badge>
								))}
							...
							<>
								...
								{config.port > 0 && (
									<Badge variant="secondary">
										{t("appEmulator:projectInfo.port")}: {config.port}
									</Badge>
								)}
							</>
						)}
				...
				<div className="flex-1 min-h-0 overflow-hidden rounded-lg border border-border flex flex-col">
						...
						<div className="flex-1 min-h-0 flex flex-col overflow-hidden">
							...
							<div className="flex items-center gap-3 px-4 py-3 border-b border-border shrink-0">
								...
								<div className="flex items-center gap-1.5">
									...
									<span
										className={`text-xs font-medium ${phase === "starting" ? "text-foreground" : "text-muted-foreground"}`}
									>
							...
							>
								{searchFilteredOutput ? (
									...
									<span className="italic opacity-50">
										{t("appEmulator:steps.waitingOutput")}
									</span>
								)}
							</pre>
						</div>
					...
					{phase === "error" && (
						<div className="flex-1 min-h-0 flex flex-col overflow-hidden">
							<div className="flex flex-col items-center gap-2 py-6 px-4 shrink-0">
								<p className="text-sm font-medium text-destructive text-center max-w-md">
									{error}
								</p>
								{selectedProject?.path && (
									<p className="text-xs text-muted-foreground text-center">
										{t("appEmulator:errors.scannedPath")}:{" "}
										{selectedProject.path}
									</p>
								)}
							</div>
					...
					{phase === "idle" && !selectedProject && (
						<div className="flex-1 flex flex-col items-center justify-center gap-4">
							<p className="text-sm text-muted-foreground">
								{t("appEmulator:errors.noProject")}
							</p>
						</div>
					)}
				</div>
