...
export function Worktrees({ projectId }: WorktreesProps) {
	...
	const [terminalWorktreeToDelete, setTerminalWorktreeToDelete] =
		useState<TerminalWorktreeConfig | null>(null);
	const [isDeletingTerminal, setIsDeletingTerminal] = useState(false);

	// Merge dialog state
	const [showMergeDialog, setShowMergeDialog] = useState(false);
	const [selectedWorktree, setSelectedWorktree] =
		useState<WorktreeListItem | null>(null);
	const [isMerging, setIsMerging] = useState(false);
	const [mergeResult, setMergeResult] = useState<WorktreeMergeResult | null>(
		null,
	);

	// Delete confirmation state
	const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
	const [worktreeToDelete, setWorktreeToDelete] =
		useState<WorktreeListItem | null>(null);
	...
	const isAllSelected = useMemo(
		() =>
			totalWorktrees > 0 &&
			worktrees.every((w) =>
				selectedWorktreeIds.has(`${TASK_PREFIX}${w.specName}`),
			) &&
			terminalWorktrees.every((wt) =>
				selectedWorktreeIds.has(`${TERMINAL_PREFIX}${wt.name}`),
			),
	...
	const loadWorktrees = useCallback(async () => {
		...
		try {
			...
			if (terminalResult.success && terminalResult.data) {
				...
				console.warn(
					"[Worktrees] Terminal worktrees fetch failed or empty:",
					terminalResult,
				);
			}
		} catch (err) {
	...
	const findTaskForWorktree = useCallback(
		(specName: string) => {
	...
	const handleDelete = async () => {
		...
		try {
			...
			if (task) {
				...
				result = await window.electronAPI.discardOrphanedWorktree(
					projectId,
					worktreeToDelete.specName,
				);
			} else {
				setError(
					t("common:errors.taskNotFoundForWorktree", {
						specName: worktreeToDelete.specName,
					}),
				);
				setIsDeleting(false);
				return;
	...
	const handleCreatePR = async (
		...
		try {
			const result = await window.electronAPI.createWorktreePR(
				prTask.id,
				options,
			);
			if (result.success && result.data) {
	...
	const executeBulkDelete = useCallback(async () => {
		...
		for (const specName of taskSpecNames) {
			...
			try {
				...
				if (!result.success) {
					errors.push(
						result.error ||
							t("common:errors.failedToDeleteTaskWorktree", { specName }),
					);
				}
			} catch (err) {
				errors.push(
		...
		for (const name of terminalNames) {
			...
			try {
				...
				if (!result.success) {
					errors.push(
						result.error ||
							t("common:errors.failedToDeleteTerminalWorktree", { name }),
					);
				}
			} catch (err) {
				errors.push(
		...
		if (errors.length > 0) {
			setError(
				`${t("common:errors.bulkDeletePartialFailure")}\n${errors.join("\n")}`,
			);
		} else {
			toast({
				title: t("common:labels.success"),
	...
	const handleDeleteTerminalWorktree = async () => {
		...
		try {
			...
			setError(
				err instanceof Error
					? err.message
					: "Failed to delete terminal worktree",
			);
		} finally {
	...
	return (
		<div className="flex h-full flex-col p-6">
				...
				<div className="flex items-center justify-between py-2 mb-4 border-b border-border shrink-0">
					<div className="flex items-center gap-3">
						...
						>
							...
							{isAllSelected
								? t("common:selection.clearSelection")
				...
				terminalWorktrees.length === 0 && (
					<div className="flex h-full flex-col items-center justify-center text-center">
						...
						<p className="text-sm text-muted-foreground mt-2 max-w-md">
							...
							{t("dialogs:worktrees.emptyDescriptionTerminal")}
						</p>
					</div>
				)}
											...
											<CardContent className="pt-0">
												...
												<div className="flex flex-wrap gap-4 text-sm mb-4">
													<div className="flex items-center gap-1.5 text-muted-foreground">
														...
														<span>
															{worktree.filesChanged ?? 0} files changed
														</span>
													</div>
													<div className="flex items-center gap-1.5 text-muted-foreground">
														<ChevronRight className="h-3.5 w-3.5" />
														<span>
															{worktree.commitCount ?? 0} commits ahead
														</span>
													</div>
												...
												<div className="flex items-center gap-2 text-xs text-muted-foreground mb-4 bg-muted/50 rounded-md p-2">
													<span className="font-mono">
														{worktree.baseBranch || t("common:labels.orphaned")}
													</span>
							...
							<div className="rounded-lg bg-muted p-4 text-sm space-y-3">
								...
								<div className="border-t border-border pt-3 mt-3">
									<div className="flex items-center justify-between text-xs">
										...
										<span>
											{selectedWorktree.commitCount ?? 0} commits,{" "}
											{selectedWorktree.filesChanged ?? 0} files
										</span>
									</div>
								</div>
							</div>
						...
						<div className="py-4">
							...
							>
								<div className="flex items-start gap-2">
									...
									<div>
										<p
											className={`font-medium ${mergeResult.success ? "text-success" : "text-destructive"}`}
										>
											{mergeResult.success
												? "Merge Successful"
												: "Merge Failed"}
			...
			<AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This will permanently delete the worktree and all uncommitted
							changes.
							{worktreeToDelete && (
								<span className="block mt-2 font-mono text-sm">
									{worktreeToDelete.isOrphaned
										? t("common:labels.orphaned")
			...
			<AlertDialog
				open={!!terminalWorktreeToDelete}
				onOpenChange={(open) => !open && setTerminalWorktreeToDelete(null)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>Delete Terminal Worktree?</AlertDialogTitle>
						<AlertDialogDescription>
							This will permanently delete the worktree and its branch. Any
					...
					<AlertDialogFooter>
						<AlertDialogCancel disabled={isDeletingTerminal}>
							Cancel
						</AlertDialogCancel>
						<AlertDialogAction
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle className="flex items-center gap-2">
							...
							{t("dialogs:worktrees.bulkDeleteTitle", {
								count: selectedWorktreeIds.size,
							})}
						</AlertDialogTitle>
						<AlertDialogDescription>
