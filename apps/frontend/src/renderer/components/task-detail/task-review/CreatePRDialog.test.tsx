...
describe("CreatePRDialog", () => {
	...
	it("should render dialog when open", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByRole("heading", { name: /create pull request/i }),
			).toBeInTheDocument();
		});
	});
	...
	it("should display source branch info", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("auto-claude/implement-user-authentication"),
			).toBeInTheDocument();
		});
	});
	...
	it("should call onCreatePR with form values when Create PR is clicked", async () => {
		...
		await waitFor(() => {
			expect(screen.getByLabelText(/pr title/i)).toHaveValue(
	...
	it("should show success state after PR is created", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("https://github.com/test/repo/pull/123"),
			).toBeInTheDocument();
		});
	});
	...
	it("should show error state when PR creation fails", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Failed to push branch to remote"),
			).toBeInTheDocument();
		});
	});
	...
	it("should show already exists message when PR already exists", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByText("https://github.com/test/repo/pull/456"),
			).toBeInTheDocument();
		});
	});
