...
export function LoadingMessage({
	message = "Loading workspace info...",
}: LoadingMessageProps) {
	return (
...
export function StagedInProjectMessage({
	...
	const handleDeleteWorktreeAndMarkDone = async () => {
		...
		try {
			...
			if (!statusResult.success) {
				...
				setError(
					"Worktree deleted but failed to update task status: " +
	...
	const handleReviewAgain = async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to reset staged state",
			);
		} finally {
	...
	return (
		<div className="rounded-xl border border-success/30 bg-success/10 p-4">
			...
			<p className="text-sm text-muted-foreground mb-3">
				...
				{task.stagedAt
					? ` on ${new Date(task.stagedAt).toLocaleDateString()}`
					: ""}
				.
			</p>
			...
			<div className="flex flex-col gap-2">
				...
				{hasWorktree && (
					<p className="text-xs text-muted-foreground">
						...
						"Mark Done Only" keeps it for reference.
					</p>
				)}
			</div>
		</div>
