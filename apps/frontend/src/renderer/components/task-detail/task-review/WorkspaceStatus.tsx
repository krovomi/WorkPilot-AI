...
export function WorkspaceStatus({
	...
	useEffect(() => {
		if (!isMerging && showOverlay && mergeStartTimeRef.current !== null) {
			...
			const hasTerminalEvent =
				mergeProgress?.stage === "complete" || mergeProgress?.stage === "error";
	...
	useEffect(() => {
		...
		const cleanup = globalThis.electronAPI.onMergeProgress(
			(eventTaskId: string, progress: MergeProgress) => {
	...
	const alreadyMergedFiles =
		mergePreview?.gitConflicts?.alreadyMergedFiles || [];
	...
	const pathMappedAIMergeCount =
		mergePreview?.summary?.pathMappedAIMergeCount || 0;
	...
	const getMergeWarningMessage = () => {
		if (hasPathMappedMerges) {
			return (
				<span className="text-warning">
					...
					{t(
						totalRenames === 1
							? "taskReview:merge.filesNeedAIMergeDueToRenames"
							: "taskReview:merge.filesNeedAIMergeDueToRenamesPlural",
						{ renameCount: totalRenames, count: pathMappedAIMergeCount },
					)}
				</span>
			);
		}
		...
		if (totalRenames > 0) {
			return (
				<span className="text-warning">
					{" "}
					{t("taskReview:merge.fileRenamesDetected", { count: totalRenames })}
				</span>
			);
		}
		...
		return (
			<span className="text-warning">
				{" "}
				{t("taskReview:merge.filesRenamedOrMoved")}
			</span>
		);
	};
				...
				<>
					...
					<div>
						<span className="text-sm font-medium text-warning">
							{t("taskReview:merge.status.branchDiverged")}
						</span>
						<span className="text-xs text-muted-foreground ml-2">
							{t("taskReview:merge.status.aiWillResolve")}
						</span>
					</div>
				</>
			);
			...
			return (
				<>
					...
					<div>
						<span className="text-sm font-medium text-warning">
							{hasPathMappedMerges
								? t("taskReview:merge.status.filesRenamed")
								: t("taskReview:merge.status.branchBehind")}
						</span>
						<span className="text-xs text-muted-foreground ml-2">
							{t("taskReview:merge.status.aiWillResolve")} (
							{hasPathMappedMerges
			...
			return (
				<>
					...
					<span className="text-sm font-medium text-success">
						{t("taskReview:merge.status.readyToMerge")}
					</span>
					<span className="text-xs text-muted-foreground ml-1">
						{mergePreview?.summary?.totalFiles || 0}{" "}
		...
		return (
			<>
				...
				<span className="text-sm font-medium text-warning">
					...
					{(mergePreview?.conflicts?.length || 0) === 1
						? t("taskReview:merge.status.conflict")
						: t("taskReview:merge.status.conflicts")}
				</span>
			</>
		);
	};
	...
	return (
		<div className="rounded-xl border border-border bg-card overflow-hidden">
			...
			<div className="px-4 py-3 space-y-3">
					...
					<div className="flex items-start gap-2 p-2.5 rounded-lg bg-warning/10 border border-warning/20">
						...
						<div className="flex-1 min-w-0">
							<p className="text-sm font-medium text-warning">
								...
								{uncommittedCount === 1 ? "change" : "changes"} in main project
							</p>
							<p className="text-xs text-muted-foreground mt-0.5">
								Commit or stash them in your terminal before staging to avoid
								conflicts.
							</p>
						</div>
					</div>
				...
				{mergePreview && isAlreadyMerged && (
					<div className="flex items-start gap-2 p-2.5 rounded-lg bg-success/10 border border-success/20">
						...
						<div className="flex-1 min-w-0">
							...
							{alreadyMergedFiles.length > 0 &&
								alreadyMergedFiles.length <= 5 && (
									<div className="mt-2 text-xs text-muted-foreground">
				...
				{hasGitConflicts &&
					mergePreview?.gitConflicts &&
					!isAlreadyMerged &&
					!isSuperseded && (
						<div className="text-xs text-muted-foreground pl-6">
							{t("taskReview:merge.branchHasNewCommits", {
								branch: mergePreview.gitConflicts.baseBranch,
								count: mergePreview.gitConflicts.commitsBehind,
							})}
							{mergePreview.gitConflicts.conflictingFiles.length > 0 && (
								<span className="text-warning">
									{" "}
									{t("taskReview:merge.filesNeedMerging", {
										count: mergePreview.gitConflicts.conflictingFiles.length,
									})}
								</span>
							)}
						</div>
				...
				{!hasGitConflicts &&
					...
					!isSuperseded && (
						<div className="text-xs text-muted-foreground pl-6">
							{t("taskReview:merge.branchHasNewCommitsSinceBuild", {
								branch: mergePreview.gitConflicts.baseBranch,
								count: commitsBehind,
							})}
			...
			<div className="px-4 py-3 bg-muted/20 border-t border-border space-y-3">
				...
				{mergePreview && !isAlreadyMerged && !isSuperseded && (
					...
					<label className="inline-flex items-center gap-2.5 text-sm cursor-pointer select-none px-3 py-2 rounded-lg border border-border bg-background/50 hover:bg-background/80 transition-colors">
						...
						>
							{t("taskReview:merge.status.stageOnly")}
						</span>
					</label>
				)}
				...
				<div className="flex gap-2">
					...
					{!mergePreview && !isLoadingPreview && (
						...
						>
							...
							{t("taskReview:merge.checkForConflicts")}
						</Button>
					)}
					...
					{isLoadingPreview && (
						<Button variant="default" disabled className="flex-1">
							<Loader2 className="mr-2 h-4 w-4 animate-spin" />
							{t("taskReview:merge.checkingForConflicts")}
						</Button>
					)}
					...
					{mergePreview &&
						...
						!isSuperseded && (
							<Tooltip>
								...
								<TooltipContent>
									<p className="max-w-xs">{t("tasks:review.mergeTooltip")}</p>
								</TooltipContent>
							</Tooltip>
						)}
