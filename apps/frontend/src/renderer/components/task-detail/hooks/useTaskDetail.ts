...
function validateTaskSubtasks(task: Task): boolean {
	...
	if (task.subtasks.length === 0) {
		console.warn(
			"[validateTaskSubtasks] Task has empty subtasks array:",
			task.id,
		);
		return false;
	}
	...
	for (let i = 0; i < task.subtasks.length; i++) {
		...
		if (!subtask || typeof subtask !== "object") {
			console.warn(
				`[validateTaskSubtasks] Invalid subtask at index ${i}:`,
				subtask,
			);
			return false;
		}
		...
		if (
			...
			console.warn(
				`[validateTaskSubtasks] Subtask at index ${i} missing description:`,
				subtask,
			);
			return false;
		}
		...
		if (!subtask.id || typeof subtask.id !== "string") {
			console.warn(
				`[validateTaskSubtasks] Subtask at index ${i} missing id:`,
				subtask,
			);
			return false;
		}
...
export function useTaskDetail({ task }: UseTaskDetailOptions) {
	...
	const [activeTab, setActiveTab] = useState(
		() => consumePendingTaskDetailTab() ?? "overview",
	);
	...
	useEffect(() => {
		...
		const loadLogs = async () => {
			...
			try {
				const result = await window.electronAPI.getTaskLogs(
					selectedProject.id,
					task.specId,
				);
	...
	const loadMergePreview = useCallback(async () => {
		...
		try {
			...
			console.error(
				"[useTaskDetail] Unexpected error in loadMergePreview:",
				err,
			);
			setWorkspaceError(
				"An unexpected error occurred while loading workspace information",
			);
		} finally {
	 ...
	 */
	const reloadPlanForIncompleteTask =
		useCallback(async (): Promise<boolean> => {
			...
			console.warn(
				"[reloadPlanForIncompleteTask] Task has invalid subtasks, reloading plan:",
				{
					taskId: task.id,
					specId: task.specId,
			...
			try {
				...
				if (!result.success || !result.data) {
					console.error(
						"[reloadPlanForIncompleteTask] Failed to reload tasks:",
						result.error,
					);
					return false;
				}
				...
				const updatedTask = result.data.find(
					(t) => t.id === task.id || t.specId === task.specId,
				);
				if (!updatedTask) {
					console.error(
						"[reloadPlanForIncompleteTask] Task not found in reloaded tasks",
					);
					return false;
				}
				...
				if (!validateTaskSubtasks(updatedTask)) {
					console.error(
						"[reloadPlanForIncompleteTask] Reloaded task still has invalid subtasks",
					);
					return false;
				}
				...
				console.error(
					"[reloadPlanForIncompleteTask] Error reloading plan:",
					err,
				);
				return false;
			} finally {
