...
export function DebugSettings() {
	...
	const handleSentryToggle = async (checked: boolean) => {
		...
		try {
			...
			if (result.success) {
				...
				setError(
					t(
						"debug.errorReporting.saveFailed",
						"Failed to save error reporting setting",
					),
				);
			}
		} catch (_err) {
	...
	const handleCopyDebugInfo = async () => {
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to copy debug info",
			);
		}
	};
	...
	return (
		<SettingsSection
			...
			description={t(
				"debug.description",
				"Access logs and debug information for troubleshooting",
			)}
		>
			<div className="space-y-6">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-xs text-muted-foreground mt-0.5">
									{t(
										"debug.errorReporting.description",
										"Send crash reports to help improve WorkPilot AI. No personal data or code is collected.",
									)}
								</p>
							</div>
						</div>
						...
						{debugInfo.recentErrors.length > 0 && (
							<div className="rounded-lg border border-border p-4">
								<h4 className="font-medium text-sm mb-3 flex items-center gap-2">
									...
									{t("debug.recentErrors", "Recent Errors")} (
									{debugInfo.recentErrors.length})
								</h4>
				...
				<div className="text-xs text-muted-foreground bg-muted/30 p-3 rounded-md">
					...
					<p>
						{t(
							"debug.helpText",
							'When reporting bugs, click "Copy Debug Info" to get system information and recent errors that help us diagnose the issue.',
						)}
					</p>
				</div>
			</div>
