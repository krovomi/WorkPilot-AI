...
export function AuthTerminal({
	...
	const [status, setStatus] = useState<
		"connecting" | "ready" | "onboarding" | "success" | "error"
	>("connecting");
	...
	useEffect(() => {
		...
		const createTerminal = async () => {
			...
			try {
				...
				console.warn("[AuthTerminal] Creating terminal:", terminalId, {
					cols,
					rows,
					configDir,
				});
				...
				if (!result.success) {
					console.error(
						"[AuthTerminal] Failed to create terminal:",
						result.error,
					);
					setStatus("error");
				...
				if (loginSentRef.current) {
					debugLog("SKIPPED scheduling command pre-fill (already sent)", {
						terminalId,
					});
				} else {
					...
					loginTimeoutRef.current = setTimeout(() => {
						...
						if (loginSentRef.current) {
							debugLog("SKIPPED command pre-fill (already sent)", {
								terminalId,
							});
						} else {
	...
	useEffect(() => {
			...
			if (id === terminalId && xterm) {
				...
				if (isCodexAuth && statusRef.current === "ready") {
					...
					if (hasSuccessIndicator) {
						debugLog(
							"Detected Codex CLI authentication success from terminal output",
							{
								terminalId,
								output: data.trim(),
								extractedEmail,
			...
			if (info.terminalId === terminalId) {
				if (info.success) {
					...
					if (info.needsOnboarding) {
						debugLog("Setting status to onboarding (auth already reported)", {
							terminalId,
						});
						setStatus("onboarding");
					} else {
						debugLog("Setting status to success (no onboarding needed)", {
							terminalId,
						});
						setStatus("success");
					}
			...
			if (id === terminalId) {
				...
				debugLog("Terminal exit event", {
					...
					willTransitionToSuccess:
						statusRef.current === "onboarding" && exitCode === 0,
				});
				...
				// assume authentication was successful (Codex CLI often exits immediately after auth)
					...
					debugLog(
						"Codex CLI terminal exited with code 0, assuming authentication success",
						{ terminalId },
					);
					authCompletedRef.current = true;
		...
		const unsubOnboardingComplete =
			globalThis.electronAPI.onTerminalOnboardingComplete((info) => {
				if (info.terminalId === terminalId) {
					...
					if (statusRef.current === "onboarding") {
						debugLog("Auto-closing terminal after onboarding complete", {
							terminalId,
						});
						setStatus("success");
						// Only call onAuthSuccess if not already reported (OAuth token handler)
			...
			<div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
				<div className="flex items-center gap-2">
					...
					<span className="text-sm font-medium">
						...
						{status === "ready" &&
							t("authTerminal.authenticate", { profileName })}
						{status === "onboarding" &&
							t("authTerminal.completeSetup", { profileName })}
						{status === "success" &&
							(authEmail
								? t("authTerminal.authenticatedAs", { email: authEmail })
								: t("authTerminal.authenticated"))}
