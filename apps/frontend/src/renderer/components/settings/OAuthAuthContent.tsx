	...
	if (providerId === "windsurf") {
		return (
			<div className="space-y-4">
				...
				{windsurfAccountInfo &&
					(windsurfAccountInfo.userName || windsurfAccountInfo.planName) && (
							...
							<div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
								{windsurfAccountInfo.userName && (
									<>
										<span className="text-muted-foreground">
											{t(
												"sections.accounts.providerConfig.windsurfAuth.accountName",
											)}
										</span>
								...
								{windsurfAccountInfo.planName && (
									<>
										<span className="text-muted-foreground">
											{t(
												"sections.accounts.providerConfig.windsurfAuth.accountPlan",
											)}
										</span>
										<span className="font-medium">
											{windsurfAccountInfo.planName}
								...
								{windsurfAccountInfo.usageInfo &&
									windsurfAccountInfo.usageInfo.totalMessages > 0 && (
										<>
											<span className="text-muted-foreground">
												{t(
													"sections.accounts.providerConfig.windsurfAuth.accountCredits",
												)}
											</span>
											<span className="font-medium">
												{Math.round(
													windsurfAccountInfo.usageInfo.usedMessages / 100,
												)}
												/
												{Math.round(
													windsurfAccountInfo.usageInfo.totalMessages / 100,
												)}
											</span>
										</>
									)}
								{windsurfAccountInfo.usageInfo &&
									windsurfAccountInfo.usageInfo.totalFlowActions > 0 && (
										<>
											<span className="text-muted-foreground">
												{t(
													"sections.accounts.providerConfig.windsurfAuth.accountFlowActions",
												)}
											</span>
											<span className="font-medium">
												{Math.round(
													windsurfAccountInfo.usageInfo.usedFlowActions / 100,
												).toLocaleString()}
												/
												{Math.round(
													windsurfAccountInfo.usageInfo.totalFlowActions / 100,
												).toLocaleString()}
											</span>
										</>
									)}
				...
				<div className="flex items-center gap-2 text-xs text-muted-foreground">
					<span>
						{t("sections.accounts.providerConfig.windsurfAuth.orManual")}
					</span>
					<button
						type="button"
						className="text-primary underline hover:no-underline"
						onClick={() =>
							window.open("https://windsurf.com/account", "_blank")
						}
					>
						{t(
							"sections.accounts.providerConfig.windsurfAuth.openWindsurfLogin",
						)}
					</button>
				</div>
	...
	return (
		<div className="space-y-4">
			...
			>
				{isAuthenticating ? (
					...
					<>
						<LogIn className="w-4 h-4 mr-2" />
						{t(
							"sections.accounts.providerConfig.windsurfAuth.connectWithClaude",
						)}
					</>
				)}
			</Button>
			...
			<div className="text-xs text-muted-foreground">
				<p>
					{t(
						"sections.accounts.providerConfig.windsurfAuth.terminalInstructions",
					)}
				</p>
			</div>
		</div>
