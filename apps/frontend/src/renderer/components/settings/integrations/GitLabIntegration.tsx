...
export function GitLabIntegration({
	...
	debugLog(
		...
		envConfig
			? {
					gitlabEnabled: envConfig.gitlabEnabled,
					hasToken: !!envConfig.gitlabToken,
	...
	useEffect(() => {
		debugLog(
			`useEffect[branches] - gitlabEnabled: ${envConfig?.gitlabEnabled}, projectPath: ${projectPath}`,
		);
	...
	const fetchBranches = async () => {
		...
		try {
			...
			debugLog("fetchBranches: getGitBranches result:", {
			...
			if (result.success && result.data) {
				...
				if (!settings?.mainBranch && !envConfig?.defaultBranch) {
					...
					const detectResult =
						await window.electronAPI.detectMainBranch(projectPath);
	...
	const fetchUserProjects = async () => {
		...
		try {
			const hostname = envConfig?.gitlabInstanceUrl
				?.replace(/^https?:\/\//, "")
				.replace(/\/$/, "");
				...
				<>
											...
											{glabInstallSuccess ? (
												<div className="rounded-md border border-success/30 bg-success/10 p-3">
													<div className="flex items-center justify-between">
														...
														>
															<RefreshCw
																className={`h-3 w-3 ${isCheckingGlab ? "animate-spin" : ""}`}
															/>
								...
								<div className="rounded-lg border border-success/30 bg-success/10 p-3 mb-4">
									<div className="flex items-center gap-2">
										...
										<p className="text-xs text-success">
											...
											<span className="font-mono">{glabVersion}</span>
										</p>
									</div>
								</div>
							)}
					...
					{projectPath && (
						<BranchSelector
							branches={branches}
							selectedBranch={
								settings?.mainBranch || envConfig.defaultBranch || ""
							}
...
function ProjectDropdown({
	...
	const selectedProjectData = projects.find(
		(p) => p.pathWithNamespace === selectedProject,
	);
	...
	return (
		<div className="space-y-2">
								...
								filteredProjects.map((project) => (
									<button
										...
										className={`w-full px-3 py-2 text-left hover:bg-accent flex items-start gap-2 ${
											project.pathWithNamespace === selectedProject
												? "bg-accent"
												: ""
										}`}
									>
			...
			{selectedProject && (
				<p className="text-xs text-muted-foreground">
					{t("settings.selected")}:{" "}
					<code className="px-1 bg-muted rounded">{selectedProject}</code>
				</p>
			)}
		</div>
	);
...
function ProjectInput({ value, onChange }: ProjectInputProps) {
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-xs text-muted-foreground">
				{t("settings.projectFormat")}{" "}
				<code className="px-1 bg-muted rounded">group/project</code>{" "}
	...
	return (
		<div className="rounded-lg border border-info/30 bg-info/5 p-3">
			<div className="flex items-start gap-3">
				...
				<svg
					className="h-5 w-5 text-info mt-0.5"
					viewBox="0 0 24 24"
					fill="currentColor"
				>
					...
					<div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
						...
						<div className="max-h-40 overflow-y-auto border-t border-border">
							{filteredBranches.length === 0 ? (
								<div className="px-3 py-4 text-sm text-muted-foreground text-center">
									{filter
										? t("settings.noMatchingBranches")
										: t("settings.noBranchesFound")}
								</div>
							) : (
