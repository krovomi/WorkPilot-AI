...
export function GitHubCopilotAuthTerminal({
	...
	const [status, setStatus] = useState<
		"connecting" | "ready" | "authenticating" | "success" | "error"
	>("connecting");
	...
	useEffect(() => {
		...
		const createTerminal = async () => {
			...
			try {
				...
				console.warn(
					"[GitHubCopilotAuthTerminal] Creating terminal:",
					terminalId,
					{ cols, rows },
				);
				...
				if (!result.success) {
					console.error(
						"[GitHubCopilotAuthTerminal] Failed to create terminal:",
						result.error,
					);
					setStatus("error");
					const errorMsg =
						result.error || t("githubCopilot.authTerminal.failedToCreate");
				...
				xterm.writeln(
					`\x1b[1;36m║\x1b[0m   \x1b[1m${titleText}\x1b[0m${" ".repeat(Math.max(0, 60 - titleText.length - 3))}\x1b[1;36m║\x1b[0m`,
				);
				xterm.writeln(
					"\x1b[1;36m╠════════════════════════════════════════════════════════════╣\x1b[0m",
				);
				...
				xterm.writeln(
					`\x1b[1;36m║\x1b[0m   \x1b[33m1.\x1b[0m ${step1Text}${" ".repeat(Math.max(0, 60 - step1Text.length - 6))}\x1b[1;36m║\x1b[0m`,
				);
				xterm.writeln(
					`\x1b[1;36m║\x1b[0m   \x1b[33m2.\x1b[0m ${step2Text}${" ".repeat(Math.max(0, 60 - step2Text.length - 6))}\x1b[1;36m║\x1b[0m`,
				);
				xterm.writeln(
					`\x1b[1;36m║\x1b[0m   \x1b[33m3.\x1b[0m ${step3Text}${" ".repeat(Math.max(0, 60 - step3Text.length - 6))}\x1b[1;36m║\x1b[0m`,
				);
				xterm.writeln(
					"\x1b[1;36m║\x1b[0m                                                            \x1b[1;36m║\x1b[0m",
				);
				...
				if (!loginSentRef.current) {
					debugLog("Scheduling gh auth login pre-fill", {
						terminalId,
						delay: 500,
					});
					loginTimeoutRef.current = setTimeout(() => {
						...
						if (!loginSentRef.current) {
							...
							debugLog("SKIPPED gh auth login pre-fill (already sent)", {
								terminalId,
							});
						}
					}, 500);
				} else {
					debugLog("SKIPPED scheduling gh auth login pre-fill (already sent)", {
						terminalId,
					});
				}

				console.warn(
					"[GitHubCopilotAuthTerminal] Terminal created successfully",
				);
			} catch (error) {
				console.error(
					"[GitHubCopilotAuthTerminal] Error creating terminal:",
					error,
				);
				setStatus("error");
				const errorMsg =
					error instanceof Error
						? error.message
						: t("githubCopilot.authTerminal.unknownError");
				setErrorMessage(errorMsg);
				onAuthError?.(errorMsg);
			}
		};
	...
	useEffect(() => {
		debugLog("Setting up event listeners effect", {
			terminalId,
		...
		const unsubOutput = window.electronAPI.onTerminalOutput((id, data) => {
			if (id === terminalId && xterm) {
				...
				if (
					...
					data.includes("Authentication successful")
				) {
					if (!authCompletedRef.current) {
						...
						const usernameMatch =
							data.match(/as\s+(\S+)/) || data.match(/(\S+)@github\.com/);
				...
				if (
					...
					data.includes("Error:") ||
					data.includes("Failed to authenticate")
				) {
					if (!authCompletedRef.current) {
						authCompletedRef.current = true;
						debugLog("Authentication failure detected", { terminalId });
						setStatus("error");
						const errorMsg = data.includes("Error:")
							? data.split("Error:")[1]?.trim()
							: t("githubCopilot.authTerminal.authFailed");
						setErrorMessage(errorMsg);
						onAuthError?.(errorMsg);
					}
				}
				...
				if (
					data.includes("gh auth login") ||
					data.includes("What is your preferred protocol")
				) {
		...
		const inputDisposable = xterm.onData((data) => {
			...
			if (data === "\r" || data === "\n") {
				...
				debugLog("User input (paste or special)", {
					terminalId,
		...
		const unsubExit = window.electronAPI.onTerminalExit((id, exitCode) => {
			if (id === terminalId) {
				console.warn(
					"[GitHubCopilotAuthTerminal] Terminal exited:",
					exitCode,
					"status:",
					statusRef.current,
				);
				debugLog("Terminal exit event", {
					...
					loginSent: loginSentRef.current,
					willTransitionToSuccess:
						statusRef.current === "authenticating" && exitCode === 0,
				});
				...
				if (
					...
					authCompletedRef.current = true;
					debugLog("Transitioning from authenticating to success", {
						terminalId,
					});
					setStatus("success");
			...
			<div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
				<div className="flex items-center gap-2">
					...
					<span className="text-sm font-medium">
						{status === "connecting" &&
							t("githubCopilot.authTerminal.connecting")}
						{status === "ready" &&
							t("githubCopilot.authTerminal.authenticate", { profileName })}
						{status === "authenticating" &&
							t("githubCopilot.authTerminal.completeSetup", { profileName })}
						{status === "success" &&
							(authUsername
								? t("githubCopilot.authTerminal.authenticatedAs", {
										username: authUsername,
									})
								: t("githubCopilot.authTerminal.authenticated"))}
