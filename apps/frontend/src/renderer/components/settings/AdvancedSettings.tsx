...
export function AdvancedSettings({
	...
	const [appUpdateInfo, setAppUpdateInfo] =
		useState<AppUpdateAvailableEvent | null>(null);
	const [isCheckingAppUpdate, setIsCheckingAppUpdate] = useState(false);
	const [isDownloadingAppUpdate, setIsDownloadingAppUpdate] = useState(false);
	const [appDownloadProgress, setAppDownloadProgress] =
	...
	const [stableDowngradeInfo, setStableDowngradeInfo] =
	...
	useEffect(() => {
		...
		const cleanupProgress = window.electronAPI.onAppUpdateProgress(
			(progress) => {
		...
		const cleanupStableDowngrade =
			window.electronAPI.onAppUpdateStableDowngrade((info) => {
				...
				<div className="space-y-6">
					...
					<div className="rounded-lg border border-border bg-muted/50 p-5 space-y-4">
						...
						<div className="pt-2">
							...
							>
								<RefreshCw
									className={`mr-2 h-4 w-4 ${isCheckingAppUpdate ? "animate-spin" : ""}`}
								/>
						...
						<div className="rounded-lg border-2 border-info/50 bg-info/5 p-5 space-y-4">
								...
								<div>
									...
									{appUpdateInfo?.releaseDate && (
										<p className="text-xs text-muted-foreground mt-1">
											{t("updates.released")}{" "}
											{new Date(appUpdateInfo.releaseDate).toLocaleDateString()}
										</p>
									)}
								</div>
							...
							{isDownloadingAppUpdate && appDownloadProgress && (
								<div className="space-y-2">
									...
									<Progress
										value={appDownloadProgress.percent}
										className="h-2"
									/>
									<p className="text-xs text-muted-foreground text-right">
										{(appDownloadProgress.transferred / 1024 / 1024).toFixed(2)}{" "}
										MB / {(appDownloadProgress.total / 1024 / 1024).toFixed(2)}{" "}
										MB
									</p>
								</div>
							)}
								...
								<div className="flex items-start gap-3 text-sm text-warning bg-warning/10 border border-warning/30 rounded-lg p-3">
									...
									<div className="space-y-1">
										<p className="font-medium text-warning">
											{t("updates.readOnlyVolumeTitle")}
										</p>
										<p className="text-muted-foreground">
											{t("updates.readOnlyVolumeDescription")}
										</p>
									</div>
								</div>
							)}
							...
							{stableDowngradeInfo.releaseNotes && (
								<div className="bg-background rounded-lg p-4 max-h-48 overflow-y-auto border border-border/50">
									<ReleaseNotesRenderer
										content={stableDowngradeInfo.releaseNotes}
									/>
								</div>
							)}
	...
	const notificationItems: Array<{
		...
		{
			key: "onTaskComplete",
			labelKey: "notifications.onTaskComplete",
			descriptionKey: "notifications.onTaskCompleteDescription",
		},
		{
			key: "onTaskFailed",
			labelKey: "notifications.onTaskFailed",
			descriptionKey: "notifications.onTaskFailedDescription",
		},
		{
			key: "onReviewNeeded",
			labelKey: "notifications.onReviewNeeded",
			descriptionKey: "notifications.onReviewNeededDescription",
		},
		{
			key: "sound",
			labelKey: "notifications.sound",
			descriptionKey: "notifications.soundDescription",
		},
	];
