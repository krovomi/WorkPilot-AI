import { FolderOpen, Monitor, RotateCcw, Save, Trash2 } from "lucide-react";
...
function isValidCustomPreset(value: unknown): value is CustomPreset {
	...
	return (
		...
		isValidFontWeight(
			typeof settings.fontWeight === "number" ? settings.fontWeight : 0,
		) &&
		isValidLineHeight(
			typeof settings.lineHeight === "number" ? settings.lineHeight : 0,
		) &&
		isValidLetterSpacing(
			typeof settings.letterSpacing === "number" ? settings.letterSpacing : 0,
		) &&
		isValidScrollback(
...
export function PresetsPanel({
	...
	const applySettings = useTerminalFontSettingsStore(
	...
	const handleSaveCustomPreset = () => {
		...
		const isDuplicate = customPresets.some(
			(preset) => preset.name === trimmedName,
		);
		if (isDuplicate) {
			toast({
				variant: "destructive",
				title: t("terminalFonts.presets.duplicateName", {
		...
		toast({
			title: t("terminalFonts.presets.saved", {
				defaultValue: 'Preset "{{name}}" saved successfully',
				name: trimmedName,
			}),
		});
	};
	...
	const handleDeleteCustomPreset = (presetId: string) => {
		...
		if (preset) {
			toast({
				title: t("terminalFonts.presets.deleted", {
					defaultValue: 'Preset "{{name}}" deleted',
					name: preset.name,
				}),
			});
		}
			...
			<div className="space-y-2.5 border-t border-border/60 pt-5">
				...
				<div className="flex items-center gap-2">
					<input
						...
						placeholder={t(
							"settings:terminalFonts.presets.presetNamePlaceholder",
							{
					...
					>
						...
						<span>{t("common:buttons.save", { defaultValue: "Save" })}</span>
					</button>
				</div>
										...
										<div className="truncate text-[11px] text-muted-foreground">
											{t("settings:terminalFonts.presets.summary", {
												font:
													...
													t("settings:terminalFonts.presets.unknownFont", {
