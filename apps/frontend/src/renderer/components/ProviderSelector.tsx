...
const providerIcons: Record<string, React.ReactNode> = {
	...
	aws: (
		...
		>
			...
			/>
			<path
				d="M21.422 16.956c-2.597 1.923-6.37 2.942-9.613 2.942-4.549 0-8.647-1.683-11.742-4.48-.243-.22-.025-.52.267-.349 3.342 1.947 7.47 3.12 11.733 3.12 2.877 0 6.037-.597 8.95-1.836.439-.187.808.287.405.603z"
				fill="#FF9900"
			/>
			<path
				d="M22.474 15.753c-.334-.428-2.204-.202-3.046-.102-.254.03-.294-.192-.064-.352 1.49-1.05 3.934-.746 4.218-.395.283.354-.075 2.81-1.472 3.98-.214.18-.418.084-.323-.153.315-.783 1.02-2.544.687-2.978z"
				fill="#FF9900"
			/>
		</svg>
	),
...
export const ProviderSelector: React.FC<ProviderSelectorProps> = ({
	...
	useEffect(() => {
		if (selected && !selectedProvider) {
			...
			try {
				localStorage.setItem("selectedProvider", selected);
			} catch {
				/* ignore */
			}
		}
	...
	useEffect(() => {
		if (!selected && providers.length > 0) {
			const defaultProvider =
				providers.find((p: CanonicalProvider) => p.name === "anthropic") ??
				providers[0];
	...
	useEffect(() => {
		...
		if (storedProvider && storedProvider !== selected) {
			...
			const providerProfile = profiles.find(
				(profile) => detectProvider(profile.baseUrl) === storedProvider,
			);
			if (providerProfile) {
	...
	const handleSelect = async (value: string) => {
		if (!status[value]) {
			...
			const provider = providers.find(
				(p: CanonicalProvider) => p.name === value,
			);
	...
	const handleConfirmAuthRedirect = () => {
		...
		toast({
			title: t(
				"dialogs:providerSelector.authRedirect.title",
				"Redirection vers les comptes",
			),
			description: t(
				"dialogs:providerSelector.authRedirect.description",
				`Configuration de ${pendingProvider} requise`,
			),
			duration: 3000,
		});
	};
		...
		return (
			...
			>
				...
				<text
					...
					dy=".3em"
				>
					{capitalize(provider.label.charAt(0))}
				</text>
			</svg>
		);
	};
	...
	const renderProviderStatus = (providerName: string) => {
		if (status[providerName]) {
			return (
				<Badge variant="success">
					{t("dialogs:providerSelector.status.ok")}
				</Badge>
			);
		}
		...
		return (
			<Badge variant="warning">
				{t("dialogs:providerSelector.status.notAuthenticated")}
			</Badge>
		);
	};
			...
			<AlertDialog open={showAuthDialog} onOpenChange={setShowAuthDialog}>
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							onClick={handleConfirmAuthRedirect}
