...
export function WorktreeSelector({
	...
	const [deleteWorktree, setDeleteWorktree] =
	...
	const fetchWorktrees = async () => {
		...
		try {
			...
			const [terminalResult, taskResult, otherResult] = await Promise.all([
				...
				project?.id
					? window.electronAPI.listWorktrees(project.id, {
							includeStats: false,
						})
			...
			if (terminalResult.success && terminalResult.data) {
				const available = currentWorktree
					? terminalResult.data.filter(
							(wt) => wt.worktreePath !== currentWorktree.worktreePath,
						)
					: terminalResult.data;
			...
			if (taskResult?.success && taskResult.data?.worktrees) {
				const availableTaskWorktrees = currentWorktree
					? taskResult.data.worktrees.filter(
							(wt) => wt.path !== currentWorktree.worktreePath,
						)
	...
	const selectTaskWorktree = useCallback(
		(taskWt: WorktreeListItem) => {
	...
	const selectOtherWorktree = useCallback(
		(otherWt: OtherWorktreeInfo) => {
			const config: TerminalWorktreeConfig = {
	...
	const activeDescendant =
		allItems.length > 0 && focusedIndex < allItems.length
	...
	return (
		<>
				...
				>
					...
					<ScrollArea className="max-h-[300px]">
						<div
							id={listboxId}
							role="listbox"
							aria-label={t("terminal:worktree.searchPlaceholder")}
							className="p-1"
						>
							{isLoading ? (
									...
									{taskItems.length > 0 && (
										<>
											...
											{taskItems.map((item, i) =>
												renderWorktreeItem(item, terminalItems.length + i),
											)}
										</>
									)}
									...
									{otherItems.length > 0 && (
										<>
											...
											{otherItems.map((item, i) =>
												renderWorktreeItem(
													item,
													terminalItems.length + taskItems.length + i,
												),
											)}
										</>
									)}
			...
			<AlertDialog
				open={!!deleteWorktree}
				onOpenChange={(open) => !open && setDeleteWorktree(null)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							{t("terminal:worktree.deleteTitle", "Delete Worktree?")}
						</AlertDialogTitle>
						<AlertDialogDescription>
							{t(
								"terminal:worktree.deleteDescription",
