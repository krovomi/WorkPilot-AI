...
export function MRDetail({
	...
	const [newCommitsCheck, setNewCommitsCheck] =
		useState<GitLabNewCommitsCheck | null>(null);
	...
	const mrStatus: {
		...
		const totalPosted =
			postedFindingIds.size + (reviewResult.postedFindingIds?.length ?? 0);
		...
		const unpostedFindings = reviewResult.findings.filter(
			(f) =>
				!postedFindingIds.has(f.id) &&
				!reviewResult.postedFindingIds?.includes(f.id),
		);
		...
		<ErrorBoundary>
			<ScrollArea className="flex-1">
				<div className="p-4 space-y-4">
					...
					<div className="space-y-2">
						<div className="flex items-start justify-between gap-4">
							...
							<Button
								...
								size="icon"
								asChild
								aria-label={t("accessibility.openOnGitLabAriaLabel")}
							>
						...
						<div className="flex items-center gap-2">
							...
							{newCommitsCheck?.hasNewCommits && !isReviewing ? (
								...
								>
									...
									Follow-up Review ({newCommitsCheck.newCommitCount} new commit
									{newCommitsCheck.newCommitCount !== 1 ? "s" : ""})
								</Button>
							) : (
								<Button
									onClick={onRunReview}
							...
							{postSuccess && (
								<div className="flex items-center gap-2 text-success text-sm">
									...
									Posted {postSuccess.count} finding
									{postSuccess.count !== 1 ? "s" : ""} to GitLab
								</div>
							)}
						</div>
					...
					<Card
						className={`border-2 ${mrStatus.color} ${mrStatus.status === "ready_for_followup" ? "animate-pulse-subtle" : ""}`}
					>
						<CardContent className="py-3">
							<div className="flex items-center gap-3">
								...
								{mrStatus.status === "waiting_for_changes" &&
									newCommitsCheck?.hasNewCommits && (
										<Badge
											variant="outline"
											className="bg-primary/20 text-primary border-primary/50 shrink-0"
										>
											<RefreshCw className="h-3 w-3 mr-1" />
											{newCommitsCheck.newCommitCount} new commit
											{newCommitsCheck.newCommitCount !== 1 ? "s" : ""}
										</Badge>
									)}
							</div>
									...
									<div className="flex flex-wrap gap-2 pb-2 border-b border-border">
										...
										{(reviewResult.newFindingsSinceLastReview?.length ?? 0) >
											...
											>
												...
												{reviewResult.newFindingsSinceLastReview?.length} new
												issue
												{reviewResult.newFindingsSinceLastReview?.length !== 1
													? "s"
													: ""}
											</Badge>
										)}
									...
									<p className="text-xs text-muted-foreground">
										...
										{reviewResult.reviewedCommitSha && (
											<>
												{" "}
												at commit{" "}
												{reviewResult.reviewedCommitSha.substring(0, 7)}
											</>
										)}
									</p>
								)}
