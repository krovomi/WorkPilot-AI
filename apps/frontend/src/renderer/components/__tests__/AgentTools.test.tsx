...
const mockGetProjectEnv = vi
	.fn()
	.mockResolvedValue({ success: true, data: null });
const mockUpdateProjectEnv = vi.fn().mockResolvedValue({ success: true });
const mockCheckMcpHealth = vi
	.fn()
	.mockResolvedValue({ success: true, data: null });
const mockTestMcpConnection = vi
	.fn()
	.mockResolvedValue({ success: true, data: null });
...
describe("AgentTools - Agent Profile Resolution", () => {
	...
	describe("Profile Resolution Logic", () => {
		...
		it("should default to auto profile when selectedProfileId is undefined", () => {
			...
			const profile =
				DEFAULT_AGENT_PROFILES.find((p) => p.id === effectiveProfileId) ||
				DEFAULT_AGENT_PROFILES[0];
	...
	describe("Agent Settings Resolution (Utility)", () => {
		...
		it("should resolve feature-based agent settings correctly", () => {
			...
			const phaseThinking = {
				spec: "medium" as const,
				planning: "medium" as const,
				coding: "medium" as const,
				qa: "medium" as const,
			};
			const featureModels = DEFAULT_FEATURE_MODELS;
			const featureThinking = DEFAULT_FEATURE_THINKING;
