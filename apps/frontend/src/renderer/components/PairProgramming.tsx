...
function SetupForm({ onStart }: SetupFormProps) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		onStart({
			projectId: "",
			goal: goal.trim(),
			devScope: devScope.trim(),
			aiScope: aiScope.trim(),
		});
	};
...
function ActionIcon({ type }: { type: string }) {
	switch (type) {
		...
		case "command_run":
			return <TerminalIcon className="w-3.5 h-3.5 text-yellow-500" />;
		default:
...
function ActionEntry({ action }: { action: AiAction }) {
	const { t } = useTranslation("pairProgramming");
	const label = t(`actions.${action.actionType}`, {
			...
			{conflicts.length > 0 && (
				<div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border-b border-amber-500/30 text-amber-600 text-xs flex-shrink-0">
					...
					<span className="font-mono">
						{conflicts.map((c) => c.filePath).join(", ")}
					</span>
				</div>
			)}
