...
export function RoadmapGenerationProgress({
	...
	const { phase, progress, message, error, startedAt, lastActivityAt } =
		generationStatus;
		...
		<div className={cn("space-y-4 p-6 rounded-xl bg-card border", className)}>
			...
			{isActivePhase && onStop && (
				<div className="flex justify-end mb-2">
					<Tooltip>
						...
						<TooltipContent>
							{t("roadmapProgress.stopGeneration")}
						</TooltipContent>
					</Tooltip>
				</div>
			)}
			...
			<div className="flex flex-col items-center text-center space-y-3">
				...
				<div className="relative">
					...
					>
						<Icon
							className={cn("h-8 w-8", config.color.replace("bg-", "text-"))}
						/>
					</motion.div>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<span className="text-xs text-muted-foreground/70 cursor-help">
											Â· {t("roadmapProgress.lastActivityPrefix")}{" "}
											{lastActivityDisplay}
										</span>
									</TooltipTrigger>
									<TooltipContent>
