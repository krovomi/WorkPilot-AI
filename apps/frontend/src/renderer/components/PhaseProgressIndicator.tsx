...
export const PhaseProgressIndicator = memo(function PhaseProgressIndicator({
	...
	const isIndeterminatePhase =
		phase === "planning" || phase === "qa_review" || phase === "qa_fixing";
				...
				<AnimatePresence mode="wait">
					{isStuck ? (
						...
						<motion.div
							...
							initial={{ opacity: 0 }}
							animate={
								isVisible ? { opacity: [0.3, 0.6, 0.3] } : { opacity: 0.45 }
							}
							transition={
								isVisible
						...
						<motion.div
							...
							className={cn(
								"absolute h-full w-1/3 rounded-full left-1/3",
								colors.color,
							)}
						/>
					) : null}
			...
			{totalSubtasks > 0 && (
				<div className="flex flex-wrap gap-1.5 mt-2">
					...
					{totalSubtasks > 10 && (
						<span
							key="overflow-count"
							className="text-[10px] text-muted-foreground font-medium ml-0.5"
						>
							+{totalSubtasks - 10}
						</span>
					)}
	...
	return (
		<div className="flex items-center gap-1 mt-2">
			{phases.map((phase, index) => {
				...
				const isPlanClickable =
					phase.key === "planning" && state === "complete" && !!onPlanClick;
					...
					<div key={phase.key} className="flex items-center">
						<motion.div
							className={cn(
								...
								isPlanClickable &&
									"cursor-pointer hover:bg-success/20 transition-colors",
							)}
							animate={
								shouldAnimate ? { opacity: [1, 0.6, 1] } : { opacity: 1 }
							}
							transition={
								shouldAnimate
							...
							title={
								isPlanClickable
									? t("execution.shortPhases.planTooltip")
									: undefined
							}
