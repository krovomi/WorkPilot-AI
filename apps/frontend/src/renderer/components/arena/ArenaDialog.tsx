		...
		<>
			...
			<div
				...
				onKeyDown={(e) =>
					canVote &&
					(e.key === "Enter" || e.key === " ") &&
					onVote(participant.label)
				}
				aria-label={
					canVote
						? `${t("battle.model")} ${participant.label}. ${isWinner && isRevealed ? t("battle.winner") : ""}`
						: undefined
				}
			>
				{/* Header */}
					...
					<div className="flex items-center gap-2">
						<Badge
							...
							className={cn(
								...
								LABEL_COLORS[participant.label],
							)}
						>
				...
				<div className="flex items-center justify-between px-3 py-2 border-t border-border text-xs text-muted-foreground">
					...
					<span>
						{participant.durationMs > 0
							? `${(participant.durationMs / 1000).toFixed(1)}s`
							: "—"}
					</span>
				</div>
...
function BattleTab() {
	...
	useEffect(() => {
		...
		const c4 = api.onArenaBattleError(
			({ error: err }: { battleId: string; error: string }) => setError(err),
		);
		...
		<div className="flex flex-col gap-4 h-full">
			...
			{activeBattle && (
				<div className="flex flex-col gap-3 flex-1">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-2">
							<Badge variant="outline" className="gap-1">
								{
									TASK_TYPES.find((t) => t.value === activeBattle.taskType)
										?.icon
								}
							...
							{isRunning && (
								<Badge
									variant="outline"
									className="gap-1 text-primary border-primary/50 bg-primary/10"
								>
							...
							{canVote && (
								<Badge
									variant="outline"
									className="gap-1 text-yellow-500 border-yellow-500/50 bg-yellow-500/10"
								>
							...
							{activeBattle.status === "completed" && (
								<Badge
									variant="outline"
									className="gap-1 text-emerald-500 border-emerald-500/50 bg-emerald-500/10"
								>
									<CheckCircle2 className="h-3 w-3" />
					...
					{/* Revealed winner */}
					{activeBattle.status === "completed" &&
						activeBattle.revealed &&
						activeBattle.winnerLabel && (
			...
			>
				<div className="flex items-center gap-2 min-w-0">
					<span className="text-base">
						{TASK_TYPES.find((tt) => tt.value === battle.taskType)?.icon}
					</span>
					<span className="text-sm truncate">
						{battle.prompt.length > 60
							? `${battle.prompt.slice(0, 60)}…`
							: battle.prompt}
					</span>
				</div>
				<div className="flex items-center gap-2 shrink-0">
					{battle.status === "completed" && winner && (
						<Badge
							variant="outline"
							className="gap-1 text-yellow-500 border-yellow-500/40"
						>
							<Trophy className="h-3 w-3" />
							{winner.modelName}
						</Badge>
					...
					{battle.participants.map((p) => (
						...
						>
							<div className="flex items-center justify-between mb-1">
								<Badge
									variant="outline"
									className={cn("text-xs", LABEL_COLORS[p.label])}
								>
		...
		<div className="flex flex-col gap-4">
				...
				<div className="flex items-center gap-3">
					<div
						className={cn(
							...
							autoRoutingEnabled ? "bg-primary/10" : "bg-muted",
						)}
					>
						<Route
							className={cn(
								"h-5 w-5",
								autoRoutingEnabled ? "text-primary" : "text-muted-foreground",
							)}
						/>
					</div>
					<div>
			...
			{hasRecommendations ? (
							...
							>
								...
								<div className="flex items-center gap-3">
									...
									>
										...
										{t(`routing.confidence.${rec.confidence}`)}
									</Badge>
								</div>
							</div>
						);
				...
				<div className="flex flex-col items-center justify-center h-40 text-muted-foreground gap-2">
					...
					<p className="text-xs opacity-60">
						{t("routing.noRecommendationsHint")}
					</p>
				</div>
			)}
			...
			{analytics && analytics.totalVotes < 10 && (
				<div className="flex items-start gap-2 text-xs text-muted-foreground p-3 rounded-xl bg-muted/50">
					...
					<span>
						{t("routing.needMoreData", { count: 10 - analytics.totalVotes })}
					</span>
				</div>
			)}
		</div>
