...
import type {
	...
	TeamSyncPeer,
	TeamSyncStatus,
} from "../../../preload/api/modules/team-sync-api";
...
function EpisodeTypeBadge({ type }: Readonly<{ type: string }>) {
	...
	return (
		<span
			className={`text-xs font-mono px-1.5 py-0.5 rounded bg-white/10 ${color}`}
		>
...
export function TeamSyncPanel({ projectDir }: Readonly<Props>) {
	...
	useEffect(() => {
		...
		const unsub = api.onTeamSyncServerStatus(
			(s: { running: boolean; port: number }) => {
				setStatus((prev) =>
					prev
						? { ...prev, server_running: s.running, server_port: s.port }
						: prev,
				);
			},
		);
		...
		try {
			...
			if (res.success) {
				...
				if (count > 0) {
					showFeedback(
						"success",
						t("feedback.pullSuccess", { count, peers: peerCount }),
					);
				} else {
	...
	const handleToggleServer = async () => {
		if (status?.server_running) {
			...
			if (res.success) {
				showFeedback(
					"success",
					t("feedback.serverStarted", { port: res.port }),
				);
			} else {
	...
	return (
		<div className="flex flex-col gap-6 p-6 max-w-3xl mx-auto">
			...
			{feedback && (
				<div
					className={`px-4 py-2 rounded-lg text-sm font-medium ${feedback.type === "success" ? "bg-green-500/20 text-green-300 border border-green-500/30" : "bg-red-500/20 text-red-300 border border-red-500/30"}`}
				>
					{feedback.msg}
				</div>
			)}
			{/* Status bar */}
			<div
				className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${status?.enabled ? "border-green-500/30 bg-green-500/10 text-green-300" : "border-yellow-500/30 bg-yellow-500/10 text-yellow-300"}`}
			>
				<span
					className={`w-2 h-2 rounded-full ${status?.enabled ? "bg-green-400" : "bg-yellow-400"}`}
				/>
			...
			{status?.enabled && (
				<div className="flex gap-3 flex-wrap">
					<StatCard
						label={t("stats.localEpisodes")}
						value={status.local_episode_count}
					/>
					<StatCard
						label={t("stats.importedEpisodes")}
						value={status.imported_episode_count}
					/>
				...
				<div className="rounded-xl border border-white/10 bg-white/5 p-5 flex flex-col gap-4">
					...
					<div className="flex flex-col gap-1">
						...
						<p className="text-xs text-white/40 mt-1">
							{t(
								`config.mode${configMode === "directory" ? "Directory" : "Http"}Hint`,
							)}
						</p>
					</div>
							...
							<div className="flex gap-3">
								<div className="flex flex-col gap-1 flex-1">
									...
									<input
										...
										onChange={(e) =>
											setConfigServerPort(Number(e.target.value))
										}
			...
			<div className="flex gap-3">
				...
				>
					{pushing ? <span className="animate-spin">⟳</span> : "↑"}
					{t("actions.push")}
				</button>
				<button
					type="button"
				...
				>
					{pulling ? <span className="animate-spin">⟳</span> : "↓"}
					{t("actions.pull")}
				</button>
				{configMode === "http" && (
					<button
									...
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<div className="text-xs text-white/40">
												{peer.exported_at
													? t("peers.lastExport", {
															date: new Date(peer.exported_at).toLocaleString(),
														})
								...
								{selectedPeer === peer.member_id && (
									<div className="border-t border-white/10 px-4 py-3 flex flex-col gap-2 bg-black/20 max-h-64 overflow-y-auto">
										<h4 className="text-xs font-medium text-white/50">
											{t("episodes.title", { member: peer.member_id })}
										</h4>
										{peerEpisodes.length === 0 ? (
											...
											peerEpisodes.slice(0, 50).map((ep) => (
												<div
													key={`${ep.type}-${ep.spec}-${ep.timestamp}`}
													className="flex flex-col gap-1 bg-white/5 rounded-md px-3 py-2"
												>
				...
				<ol className="flex flex-col gap-2">
					{(["step1", "step2", "step3", "step4"] as const).map((step, i) => (
						<li key={step} className="flex gap-3 text-sm text-white/60">
							<span className="w-5 h-5 rounded-full bg-blue-600/30 text-blue-300 text-xs flex items-center justify-center shrink-0 mt-0.5">
								{i + 1}
							</span>
							{t(`help.${step}`)}
						</li>
