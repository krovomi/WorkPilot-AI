...
export function TerminalGrid({
	...
	const [pendingCleanup, setPendingCleanup] = useState<Map<string, number>>(
		new Map(),
	);
		...
		for (const terminal of exitedTerminals) {
			...
			if (!cleanupTimersRef.current.has(terminal.id)) {
				...
				setPendingCleanup((prev) =>
					new Map(prev).set(terminal.id, cleanupTime),
				);
	...
	const [spotlitTerminalId, setSpotlitTerminalId] = useState<string | null>(
		null,
	);
	const jumpToTerminalId = useTerminalStore((state) => state.jumpToTerminalId);
	const clearTerminalJump = useTerminalStore(
		(state) => state.clearTerminalJump,
	);
	...
	useEffect(() => {
		...
		const fetchSessionDates = async () => {
			...
			try {
				const result =
					await globalThis.electronAPI.getTerminalSessionDates(projectPath);
	...
	const addRestoredTerminal = useTerminalStore(
		(state) => state.addRestoredTerminal,
	);
		...
		async (date: string) => {
			...
			try {
				...
				const sessionsResult =
					await globalThis.electronAPI.getTerminalSessionsForDate(
						date,
						projectPath,
					);
				const sessionsToRestore = sessionsResult.success
					? sessionsResult.data || []
					: [];

				console.warn(
					`[TerminalGrid] Found ${sessionsToRestore.length} sessions to restore from ${date}`,
				);
				...
				const result =
					await globalThis.electronAPI.restoreTerminalSessionsFromDate(
						date,
						projectPath,
						80,
						24,
				...
				if (result.success && result.data) {
					console.warn(
						`[TerminalGrid] Main process restored ${result.data.restored} sessions from ${date}`,
					);
					...
					for (const sessionResult of result.data.sessions) {
						if (sessionResult.success) {
							const fullSession = sortedSessions.find(
								(s) => s.id === sessionResult.id,
							);
							if (fullSession) {
								console.warn(
									`[TerminalGrid] Adding restored terminal to store: ${fullSession.id}`,
								);
								addRestoredTerminal(fullSession);
								// Stagger terminal initialization to prevent race conditions
								await new Promise((resolve) =>
									setTimeout(resolve, TERMINAL_INIT_STAGGER_MS),
								);
							}
						}
					}
	...
	const [draggingTerminalId, setDraggingTerminalId] = React.useState<
		string | null
	>(null);
	...
	const handleInvokeClaudeAll = useCallback(() => {
		terminals.forEach((terminal) => {
			if (terminal.status === "running" && !terminal.isClaudeMode) {
				...
				globalThis.electronAPI.invokeClaudeInTerminal(
					terminal.id,
	...
	const handleDragEnd = useCallback(
		(event: DragEndEvent) => {
				...
				if (activeId !== overId && terminals.some((t) => t.id === overId)) {
					...
					if (projectPath) {
						queueMicrotask(async () => {
							...
							try {
								const result =
									await globalThis.electronAPI.updateTerminalDisplayOrders(
										projectPath,
										orders,
									);
								if (!result.success) {
									console.warn(
										"[TerminalGrid] Failed to persist terminal order:",
										result.error,
									);
								}
							} catch (error) {
			...
			if (terminalId && activeData?.path) {
				...
				const quotedPath = activeData.path.includes(" ")
					? `"${activeData.path}"`
					: activeData.path;
	...
	if (terminals.length === 0) {
		return (
			...
			<div
				...
				role="region"
				aria-label="Terminal grid empty state"
			>
				<div className="flex flex-col items-center gap-3 text-center">
					...
					<div>
						<h2 className="text-lg font-semibold text-foreground">
							{t("terminalGrid.agentTerminals")}
						</h2>
						<p className="mt-1 text-sm text-muted-foreground max-w-md">
							{t("terminalGrid.emptyDescription")}
						</p>
					</div>
				</div>
				...
				<div className="flex h-10 items-center justify-between border-b border-border bg-card/30 px-3">
					<div className="flex items-center gap-2">
						<span className="text-xs font-medium text-muted-foreground">
							{t("terminalGrid.count", { count: terminals.length })}
						</span>
					</div>
					<div className="flex items-center gap-2">
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									...
									>
										...
										{t("terminalGrid.history")}
										<ChevronDown className="h-3 w-3" />
									</Button>
								</DropdownMenuTrigger>
								<DropdownMenuContent align="end" className="w-56">
									<div className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
										{t("terminalGrid.restoreSessions")}
									</div>
									<DropdownMenuSeparator />
									{sessionDates.map((dateInfo) => (
										...
										>
											...
											<span className="text-xs text-muted-foreground">
												{dateInfo.sessionCount} session
												{dateInfo.sessionCount === 1 ? "" : "s"}
											</span>
										</DropdownMenuItem>
									))}
								</DropdownMenuContent>
						...
						{terminals.some(
							(t) => t.status === "running" && !t.isClaudeMode,
						) && (
							<Button
								variant="outline"
								size="sm"
							...
							>
								...
								{t("terminalGrid.invokeClaudeAll")}
							</Button>
						)}
						<Button
							variant="outline"
				...
				<div className="flex flex-1 overflow-hidden">
					...
					>
						{expandedTerminalId ? (
							...
							(() => {
								const expandedTerminal = terminals.find(
									(t) => t.id === expandedTerminalId,
								);
								...
								return (
									<div
										className={cn(
											...
											spotlitTerminalId === expandedTerminal.id &&
												"terminal-spotlight",
										)}
									>
										<SortableTerminalWrapper
											...
											isExpanded={true}
											onToggleExpand={() =>
												handleToggleExpand(expandedTerminal.id)
											}
										/>
									</div>
								);
								...
								<Group orientation="vertical" className="h-full">
									{terminalRows.map((row, rowIndex) => (
										<React.Fragment key={row.map((t) => t.id).join("-")}>
											<Panel
												id={`row-${rowIndex}`}
												defaultSize={100 / terminalRows.length}
												minSize={15}
											>
															...
															>
																<div className="h-full p-1">
																	<SortableTerminalWrapper
																		...
																		isExpanded={false}
																		onToggleExpand={() =>
																			handleToggleExpand(terminal.id)
																		}
																	/>
																</div>
															</Panel>
