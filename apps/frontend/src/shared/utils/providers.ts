...
export async function getStaticProviders(
	...
	for (const p of allProviders) {
		...
		const hasProfile = profiles.some((prof) => {
			...
			// Use detectProvider (URL-hostname-safe) for all providers
		...
		if (p.name === "copilot") {
			...
			const hasGlobalUrl = !!(
				settings?.globalOllamaApiUrl &&
				String(settings.globalOllamaApiUrl).trim()
			);
			...
			const hasGlobalKey = !!(
				settings?.globalWindsurfApiKey &&
				String(settings.globalWindsurfApiKey).trim()
			);
			...
			const hasGlobalKey = !!(
				globalSettingsKey &&
				settings?.[globalSettingsKey] &&
				String(settings[globalSettingsKey]).trim()
			);
