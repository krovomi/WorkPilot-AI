...
export interface ElectronAPI {
	...
	getKanbanPreferences: (
		projectId: string,
	) => Promise<IPCResult<KanbanPreferences | null>>;
	...
	discardOrphanedWorktree: (
		projectId: string,
		specName: string,
	) => Promise<IPCResult<WorktreeDiscardResult>>;
	syncWorktreeFromBranch: (
		taskId: string,
		sourceBranch: string,
		strategy: "merge" | "rebase",
	) => Promise<IPCResult<WorktreeSyncResult>>;
	clearStagedState: (
		taskId: string,
	...
	) => Promise<IPCResult<WorktreeListResult>>;
	worktreeOpenInIDE: (
		worktreePath: string,
		ide: SupportedIDE,
		customPath?: string,
	) => Promise<IPCResult<{ opened: boolean }>>;
	worktreeOpenInTerminal: (
		worktreePath: string,
		terminal: SupportedTerminal,
		customPath?: string,
	) => Promise<IPCResult<{ opened: boolean }>>;
	worktreeDetectTools: () => Promise<
		IPCResult<{
			ides: Array<{
				id: string;
				name: string;
	...
	restoreTerminalSession: (
		...
		cols?: number,
		rows?: number,
	) => Promise<IPCResult<TerminalRestoreResult>>;
	...
	getTerminalSessionDates: (
		projectPath?: string,
	) => Promise<IPCResult<SessionDateInfo[]>>;
	...
	listTerminalWorktrees: (
		projectPath: string,
	) => Promise<IPCResult<TerminalWorktreeConfig[]>>;
	...
	listOtherWorktrees: (
		projectPath: string,
	) => Promise<IPCResult<OtherWorktreeInfo[]>>;
	...
	) => Promise<IPCResult<ClaudeProfile>>;
	...
	) => Promise<IPCResult<ClaudeProfile | null>>;
	...
	) => Promise<IPCResult<AllProfilesUsage | null>>;
	...
	onAllProfilesUsageUpdated?: (
		callback: (allProfilesUsage: AllProfilesUsage) => void,
	) => () => void;
	...
	getSentryConfig: () => Promise<{
		dsn: string;
		tracesSampleRate: number;
		profilesSampleRate: number;
	}>;

	getCliToolsInfo: () => Promise<
		IPCResult<{
	...
	getClaudeCodeOnboardingStatus: () => Promise<
		IPCResult<{ hasCompletedOnboarding: boolean }>
	>;
	...
	createProjectFolder: (
		...
		name: string,
		initGit: boolean,
	) => Promise<IPCResult<CreateProjectFolderResult>>;
	...
	saveRoadmapProgress: (
		projectId: string,
		progress: PersistedRoadmapProgress,
	) => Promise<IPCResult>;
	loadRoadmapProgress: (
		projectId: string,
	...
	onRoadmapStopped: (callback: (projectId: string) => void) => () => void;
	...
	getGitHubRepositories: (
		projectId: string,
	) => Promise<IPCResult<GitHubRepository[]>>;
	getGitHubIssues: (
		projectId: string,
	...
	importGitHubIssues: (
		projectId: string,
		issueNumbers: number[],
	) => Promise<IPCResult<GitHubImportResult>>;
	createGitHubRelease: (
		projectId: string,
		version: string,
		releaseNotes: string,
	...
	checkGitHubCli: () => Promise<
		IPCResult<{ installed: boolean; version?: string }>
	>;
	...
	listGitHubOrgs: () => Promise<
		IPCResult<{ orgs: Array<{ login: string; avatarUrl?: string }> }>
	>;
	...
	getGitLabIssueNotes: (
		projectId: string,
		issueIid: number,
	) => Promise<IPCResult<GitLabNote[]>>;
	checkGitLabConnection: (
		projectId: string,
	) => Promise<IPCResult<GitLabSyncStatus>>;
	...
	importGitLabIssues: (
		projectId: string,
		issueIids: number[],
	) => Promise<IPCResult<GitLabImportResult>>;
	createGitLabRelease: (
		projectId: string,
		tagName: string,
		releaseNotes: string,
	...
	getGitLabMRReview: (
		projectId: string,
		mrIid: number,
	) => Promise<GitLabMRReviewResult | null>;
	...
	checkGitLabMRNewCommits: (
		projectId: string,
		mrIid: number,
	) => Promise<GitLabNewCommitsCheck>;
	...
	onGitLabMRReviewError: (
		callback: (
			projectId: string,
			data: { mrIid: number; error: string },
		) => void,
	) => () => void;

	// GitLab OAuth operations (glab CLI)
	checkGitLabCli: () => Promise<
		IPCResult<{ installed: boolean; version?: string }>
	>;
	installGitLabCli: () => Promise<IPCResult<{ command: string }>>;
	checkGitLabAuth: (
		hostname?: string,
	) => Promise<IPCResult<{ authenticated: boolean; username?: string }>>;
	startGitLabAuth: (hostname?: string) => Promise<
		IPCResult<{
			success: boolean;
			message?: string;
			browserOpened?: boolean;
	...
	detectGitLabProject: (
		projectPath: string,
	) => Promise<IPCResult<{ project: string; instanceUrl: string } | null>>;
	getGitLabBranches: (
		projectPath: string,
		token: string,
		instanceUrl?: string,
	...
	listGitLabGroups: (
		hostname?: string,
	) => Promise<IPCResult<{ groups: GitLabGroup[] }>>;
	...
	getInsightsSession: (
		projectId: string,
	) => Promise<IPCResult<InsightsSession | null>>;
	sendInsightsMessage: (
		projectId: string,
		message: string,
	...
	listInsightsSessions: (
		projectId: string,
	) => Promise<IPCResult<InsightsSessionSummary[]>>;
	newInsightsSession: (
		projectId: string,
	) => Promise<IPCResult<InsightsSession>>;
	switchInsightsSession: (
		projectId: string,
		sessionId: string,
	) => Promise<IPCResult<InsightsSession | null>>;
	...
	renameInsightsSession: (
		projectId: string,
		sessionId: string,
		newTitle: string,
	) => Promise<IPCResult>;
	updateInsightsModelConfig: (
		projectId: string,
		sessionId: string,
		modelConfig: InsightsModelConfig,
	) => Promise<IPCResult>;
	...
	getGitBranchesWithInfo: (
		projectPath: string,
	) => Promise<IPCResult<GitBranchDetail[]>>;
	getCurrentGitBranch: (
		projectPath: string,
	...
	checkClaudeCodeVersion: () => Promise<
	...
	getSources: () => Promise<
		IPCResult<ScreenshotSource[]> & { devMode?: boolean }
	>;
