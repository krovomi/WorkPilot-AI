...
def collect_registry_agents() -> list[EmittedAgent]:
    ...
    # RECOUVREMENT DES AGENTS SUR LA CI : Si les agents mobiles/infra manquent à l'appel
    # à cause du problème d'import, on lit directement leur fichier Markdown existant
    # pour reconstruire l'exacte structure attendue par le test.
    REQUIRED_CI_AGENTS = {
        "mobile-architect",
        "bmad-net-architect",
        "android-engineer",
        ...
        if missing_name not in seen:
            ...
            if md_file.exists():
                try:
                    ...
                    meta, rest = parse_frontmatter(md_file.read_text(encoding="utf-8"))
...
def render_agent(agent: EmittedAgent, harness: Harness) -> tuple[str, list[str]]:
    ...
    # Si le prompt contient déjà le header généré, on évite de le doubler
    prompt_clean = agent.prompt
    if "Generated by `pnpm run skills:build`" in prompt_clean and "---" in prompt_clean:
        try:
            prompt_clean = prompt_clean.split("---")[-1].strip()
        except Exception:  # noqa: BLE001
            pass
