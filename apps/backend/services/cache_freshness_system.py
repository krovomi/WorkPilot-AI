...
class FreshnessCalculator:
    ...
    def calculate_freshness(self, cache_entry: "ContextCacheEntry") -> FreshnessMetrics:
        ...
        metrics.files_changed = self.file_analyzer.get_changed_files(
            cache_entry.git_commit_hash
        )
        metrics.dependencies_changed = (
            self.dependency_analyzer.get_changed_dependencies(
                cache_entry.git_commit_hash
            )
        )
        metrics.commits_since_cache = self.git_analyzer.count_commits_since(
            cache_entry.git_commit_hash
        )
    ...
    def _calculate_age_score(
        self, cache_entry: "ContextCacheEntry", current_time: float
    ) -> float:
    ...
    def _calculate_git_score(self, cache_entry: "ContextCacheEntry") -> float:
        ...
        try:
            ...
            if self.git_analyzer.is_ancestor(
                cache_entry.git_commit_hash, current_commit
            ):
                commits_since = self.git_analyzer.count_commits_since(
                    cache_entry.git_commit_hash
                )
    ...
    def _calculate_file_score(self, cache_entry: "ContextCacheEntry") -> float:
        ...
        try:
            changed_files = self.file_analyzer.get_changed_files(
                cache_entry.git_commit_hash
            )
    ...
    def _calculate_dependency_score(self, cache_entry: "ContextCacheEntry") -> float:
        ...
        try:
            changed_deps = self.dependency_analyzer.get_changed_dependencies(
                cache_entry.git_commit_hash
            )
            cached_deps = set()
    ...
    def _calculate_access_score(
        ...
        recency_score = max(
            0.0, 1.0 - (access_recency_hours / 24.0)
        )  # Decay over 24 hours
    ...
    def _calculate_semantic_score(self, cache_entry: "ContextCacheEntry") -> float:
        ...
        try:
            current_semantic_signature = (
                self.semantic_analyzer.generate_current_signature()
            )
            ...
            similarity = self.semantic_analyzer.calculate_similarity(
                cached_signature, current_semantic_signature
            )
    ...
    def _calculate_confidence_score(self, metrics: FreshnessMetrics) -> float:
        ...
        variance = sum((score - mean_score) ** 2 for score in factor_scores) / len(
            factor_scores
        )
...
class InvalidationEngine:
    ...
    def _load_default_rules(self):
        ...
        default_rules = [
            InvalidationRule(
                ...
                conditions={"freshness_threshold": 0.3, "confidence_threshold": 0.5},
                action="invalidate",
                priority=100,
            ),
            InvalidationRule(
            ...
            InvalidationRule(
                ...
                conditions={"max_commits": 10, "min_freshness": 0.4},
                action="downgrade",
                priority=80,
            ),
            InvalidationRule(
                ...
                conditions={"min_semantic_similarity": 0.7},
                action="refresh",
                priority=70,
            ),
        ]
    ...
    def _evaluate_invalidation_internal(
        self, cache_entry: "ContextCacheEntry"
    ) -> list[str]:
    ...
    def _evaluate_rule(
        ...
        _cache_entry: "ContextCacheEntry",
        metrics: FreshnessMetrics,
    ) -> bool:
    ...
    def _evaluate_adaptive_strategy(
        self, conditions: dict[str, Any], metrics: FreshnessMetrics
    ) -> bool:
    ...
    def _evaluate_dependency_strategy(
        self, conditions: dict[str, Any], metrics: FreshnessMetrics
    ) -> bool:
