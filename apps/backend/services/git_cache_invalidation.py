...
class GitRepositoryMonitor:
    ...
    def get_commit_info(self, commit_hash: str) -> dict[str, Any] | None:
    ...
    def get_changed_files_since(self, from_commit: str) -> GitChangeEvent | None:
    ...
    def _create_change_event(
        self, commit_hash: str, commit_info: dict[str, Any]
    ) -> GitChangeEvent:
    ...
    def _get_git_diff_output(self, from_commit: str, to_commit: str) -> str | None:
...
class CacheInvalidationStrategy:
    ...
    def should_invalidate(
        self, change_event: GitChangeEvent, cache_entry: "ContextCacheEntry"
    ) -> bool:
...
class GitBasedCacheInvalidator:
    ...
    def _check_for_changes(self):
        ...
        logger.info(
            f"Detected git changes: {change_event.commit_hash[:8]} - {change_event.commit_message[:50]}"
        )
    ...
    def _invalidate_entries(self, cache_keys: list[str], change_event: GitChangeEvent):
        ...
        for strategy in self.strategies:
            if any(
                strategy.should_invalidate(change_event, entry)
    ...
    def export_invalidation_log(self, filepath: str):
        ...
        export_data = {
            ...
            "invalidation_history": [
                event.to_dict() for event in self.invalidation_history
            ],
        }
...
from .intelligent_context_cache import ContextCacheEntry, IntelligentContextCache
