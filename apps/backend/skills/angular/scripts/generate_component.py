...
class AngularComponentGenerator:
    ...
    def _validate_options(
        self, options: ComponentGenerationOptions, result: GenerationResult
    ) -> bool:
        """Validate generation options."""
        if not options.name or not re.match(r"^[a-zA-Z][a-zA-Z0-9]*$", options.name):
            result.errors.append(
                "Invalid component name. Use camelCase starting with a letter."
            )
            return False
    ...
    def _get_file_paths(self, options: ComponentGenerationOptions) -> dict[str, Path]:
        ...
        paths = {
            ...
            "component_style": target_dir
            / f"{base_name}.component.{options.style_ext}",
    ...
    def _get_component_template(self, options: ComponentGenerationOptions) -> str:
        ...
        selector = (
            options.selector or f"{options.prefix}-{self._kebab_case(options.name)}"
        )
        ...
        if imports:
            import_statement = (
                f"import {{ {', '.join(imports)} }} from '@angular/core';"
            )
  ...
  it('should be created', () => {{
    ...
    def _update_module(
        ...
        result.warnings.append(
            "Manual module update required - add component to appropriate module"
        )
    ...
    def _add_next_steps(
        ...
        if options.type == "component":
            result.next_steps.append("Add the component to your routing configuration")
            result.next_steps.append("Implement the component's functionality")
            result.next_steps.append("Add unit tests for the component")

        if not options.standalone:
            result.next_steps.append("Add the component to an Angular module")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            'Example: python generate_component.py /path/to/project \'{"name":"UserProfile","type":"component"}\''
        )
        sys.exit(1)
    ...
    try:
        ...
        error_output = {"success": False, "error": str(e), "project_root": project_root}
