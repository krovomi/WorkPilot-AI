...
class ContextAwareSnippetsRunner:
    ...
    def generate_snippet(
        self, snippet_type: str, description: str, target_language: str | None = None
        ...
        patterns = self._get_relevant_patterns(
            snippet_type, target_language, project_context
        )
    ...
    def _get_relevant_patterns(
        self, snippet_type: str, language: str, context: dict[str, Any]
    ) -> list[str]:
...
Generate the snippet now:"""
  ...
  "reasoning": "Claude was unavailable, so this is a basic placeholder"
    ...
    def _parse_snippet_result(
        self, content: str, language: str, description: str, patterns: list[str]
    ) -> ContextAwareSnippetResult:
        ...
        try:
            ...
            if content.strip().startswith("{"):
                ...
                return ContextAwareSnippetResult(
                    ...
                    reasoning=data.get(
                        "reasoning", "Generated based on project context"
                    ),
                )
...
def main():
    ...
    parser.add_argument(
        "--description", required=True, help="Description of the snippet"
    )
    ...
    if not _AVAILABLE:
        error_result = {
            "status": "error",
            "error": "Context-aware snippets dependencies not yet available. This feature is under development.",
            "snippet": "",
            "language": "",
