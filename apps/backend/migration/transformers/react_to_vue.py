...
class ReactToVueTransformer:
    ...
    def _is_react_component(self, content: str) -> bool:
        ...
        return bool(
            ...
            or re.search(
                r"function\s+\w+\s*\(.*\)\s*{.*return.*jsx", content, re.DOTALL
            )
        )
    ...
    def _transform_use_state(
        self, var_name: str, setter_name: str, initial_value: str
    ) -> str:
    ...
    def _transform_event_handlers(self, content: str) -> str:
        ...
        content = re.sub(r"@click=\{(\w+)\}", r'@click="\1"', content)

        # Transform inline handlers
        content = re.sub(r"@click=\{.*?\s*=>\s*([^}]*)\}", r'@click="\1"', content)
