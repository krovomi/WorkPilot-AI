...
class DatabaseTransformer:
    ...
    def _transform_create_table(self, sql: str) -> str:
        ...
        sql = re.sub(
            r"DEFAULT\s+CHARSET\s*=\s*\w+\s*[,;]?", "", sql, flags=re.IGNORECASE
        )
    ...
    def _transform_data_types(self, sql: str) -> str:
        ...
        sql = re.sub(r"UNSIGNED\s+INTEGER", "BIGINT", sql, flags=re.IGNORECASE)
    ...
    def _transform_syntax(self, sql: str) -> str:
        ...
        sql = re.sub(r"FULLTEXT\s+", "", sql, flags=re.IGNORECASE)
    ...
    def _transform_auto_increment(self, sql: str) -> str:
        ...
        # Replace AUTO_INCREMENT INT with SERIAL
        sql = re.sub(
            r"INT\s+NOT\s+NULL\s+AUTO_INCREMENT", "SERIAL", sql, flags=re.IGNORECASE
        )
    ...
    def transform_application_code(
    ...
    def _uses_database(self, content: str) -> bool:
        ...
        return bool(
            re.search(r"mysql", content, re.IGNORECASE)
            or re.search(r"mysql2", content, re.IGNORECASE)
            or re.search(r"database", content, re.IGNORECASE)
            or re.search(r"connection\.query", content, re.IGNORECASE)
            or re.search(r"db\.query", content, re.IGNORECASE)
        )
    ...
    def _transform_db_calls(self, content: str, file_path: str) -> str:
        ...
        if file_path.endswith(".js") or file_path.endswith(".ts"):
            ...
            code = re.sub(
                r'require\([\'"]mysql2[\'"](?:/promise)?\)', "require('pg')", code
            )
            code = re.sub(
        ...
        # Python transformations
            ...
            code = re.sub(r"import\s+MySQLdb", "import psycopg2", code)
            code = re.sub(r"from\s+mysql\s+import\s+connector", "import psycopg2", code)
    ...
    def _transform_mysql_connection(self, code: str) -> str:
        ...
        code = re.sub(r"connection\.connect\(\)", "// Pool auto-connects", code)
