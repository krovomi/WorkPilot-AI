...
class TestReactToVueTransformer:
    ...
    def test_transform_event_handlers(self, temp_project):
        ...
        Note: _transform_jsx_to_template converts onClick= -> @click=,
        and _transform_event_handlers then normalises @click={handler}
        to @click="handler". To test the full onClick -> @click= pipeline,
        call _transform_jsx_to_template directly.
        """
        transformer = ReactToVueTransformer(temp_project)
...
class TestJSToTypeScriptTransformer:
    ...
    def test_string_type_inference(self, temp_project):
        ...
        Note: _add_variable_types uses a regex that requires at least one space
        inside the quoted value to match as a string type. Use a multi-word
        string to trigger the pattern reliably.
        """
        transformer = JSToTypeScriptTransformer(temp_project)

        # Use a string with a space so the regex `[^'"]+ ` matches
