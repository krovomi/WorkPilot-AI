...
azure_devops_exceptions = import_module_from_file(
    "src.connectors.azure_devops.exceptions",
    connectors_dir / "azure_devops" / "exceptions.py",
)
sys.modules["src.connectors.azure_devops.exceptions"] = azure_devops_exceptions

# Import des modules principaux
BaseLLMProvider = import_module_from_file(
    "src.connectors.llm_base", connectors_dir / "llm_base.py"
).BaseLLMProvider
llm_config = import_module_from_file(
    "src.connectors.llm_config", connectors_dir / "llm_config.py"
)
...
(
    ...
    llm_config.save_provider_config,
    llm_config.load_provider_config,
    llm_config.delete_provider_config,
    llm_config.list_provider_configs,
)

llm_discovery = import_module_from_file(
    "src.connectors.llm_discovery", connectors_dir / "llm_discovery.py"
)
sys.modules["src.connectors.llm_discovery"] = llm_discovery
discover_llm_providers = llm_discovery.discover_llm_providers

ConfigurationError = import_module_from_file(
    "src.connectors.azure_devops.exceptions",
    connectors_dir / "azure_devops" / "exceptions.py",
).ConfigurationError
...
def test_provider_validation():
    ...
    provider2 = DummyProvider(
        key=None
    )  # Use None instead of "invalid" to trigger the error
