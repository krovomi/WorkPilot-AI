...
class TestSSRFProtection:
    ...
    def test_validate_url_private_ip_blocked(self, mock_getaddrinfo):
        ...
        with pytest.raises(
            ValueError, match="Resolved IP .* is in a non-routable / private range"
        ):
    ...
    def test_authorized_urls_completeness(self):
        ...
        for provider, url in AUTHORIZED_URLS.items():
            assert url.startswith("https://"), (
                f"Authorized URL for {provider} must use HTTPS"
            )
    ...
    def test_url_normalization(self):
        ...
        result = validate_url_ssrf(
            "anthropic", "https://api.anthropic.com/path?query=value#fragment"
        )
        assert result == "https://api.anthropic.com"
    ...
    def test_ipv6_private_ranges(self):
        ...
        ipv6_ranges = [
            r for r in PRIVATE_IP_RANGES if isinstance(r, ipaddress.IPv6Network)
        ]
        assert len(ipv6_ranges) > 0, "IPv6 private ranges should be included"

        # Test loopback
        loopback = ipaddress.IPv6Address("::1")
        assert any(loopback in r for r in ipv6_ranges), (
        ...
        assert any(link_local in r for r in ipv6_ranges), (
