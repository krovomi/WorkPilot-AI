...
- `save_pipelines()` — Sauvegarde dans `.workpilot/pipelines/` (et optionnellement dans le projet)
...
**Backend** (`apps/backend/spec/templates/`) :
- `models.py` — `SpecTemplate`, `TemplateCategory`, `AppliedTemplate` : modèles de données avec placeholders `{resource}`, `{fields}`, etc.
- `library.py` — `TemplateLibrary` : gestion des templates built-in + custom (persistance dans `.workpilot/spec_templates.json`)

**Templates built-in** (8 templates) :
- `crud-api` — CRUD REST complet (list, get, create, update, delete) avec pagination et validation
- `jwt-authentication` — Login/register/refresh JWT avec hashing et middleware
- `analytics-dashboard` — Dashboard avec KPI cards, time-series chart, export CSV
- `form-with-validation` — Formulaire multi-champs avec validation client+serveur et accessibilité
- `db-migration` — Migration de schéma avec rollback et zéro-downtime
- `code-refactoring` — Refactoring structuré avec couverture de tests et zéro régression
- `third-party-integration` — Intégration service externe avec retry, circuit breaker, mock
- `rest-endpoint` — Endpoint REST unique avec validation, auth et tests

**Intégration** (`apps/backend/spec/requirements.py`) :
- `apply_template_to_requirements()` — Enrichit un dict requirements avec le contenu du template (task description, workflow_type, keywords, qa_criteria_hints, services_involved)

**Usage** :
```python
from spec.templates import TemplateLibrary
library = TemplateLibrary(project_dir)

# Lister les templates
templates = library.list_templates(category=TemplateCategory.CRUD)

# Suggérer automatiquement selon la description
suggestions = library.suggest_templates("create a REST API for users")

# Appliquer un template aux requirements
patch = library.apply_template("crud-api", {"resource": "User", "resources": "users"})

# Créer un template custom
library.save_custom_template(SpecTemplate(id="my-template", ...))
```
...
**Backend** (`apps/backend/context/dependency_graph/`) :
- `models.py` — `DependencyNode`, `DependencyEdge`, `DependencyGraph` : graphe orienté avec fan-in/fan-out, traversée directionnelle `get_dependencies()` / `get_dependents()`
- `builder.py` — `DependencyGraphBuilder` : analyse AST Python + regex TypeScript/JavaScript, max 5 000 fichiers, résolution des chemins relatifs
- `analyzer.py` — `DependencyAnalyzer` + `GraphInsights` : détection de cycles (DFS), modules orphelins, hotspots de couplage (fan-in ≥ 15 ou fan-out ≥ 20), `get_related_files()` pour le contexte agent
- `cache.py` — `DependencyGraphCache` : cache JSON dans `.workpilot/dependency_graph.json` avec fingerprint basé sur les mtimes des fichiers source

**Intégration** (`apps/backend/context/builder.py`) :
- `_build_dependency_hints()` — Enrichit le `TaskContext` avec `dependency_hints` : fichiers structurellement reliés, cycles impliquant les fichiers cibles, warnings de couplage
- `TaskContext.dependency_hints` — Nouveau champ exposé aux agents

**Usage** :
```python
from context.dependency_graph import DependencyGraphCache, DependencyAnalyzer

# Build/load cached graph
cache = DependencyGraphCache(project_dir)
graph = cache.get_or_build()  # Rebuilt only if files changed

# Analyze
analyzer = DependencyAnalyzer(graph)
insights = analyzer.analyze()
print(f"Cycles: {len(insights.circular_dependencies)}")
print(f"Orphans: {insights.orphan_modules}")

# Get related files for agent context
related = analyzer.get_related_files(["src/api/users.py"], depth=2)
```
...
**Backend** (`apps/backend/security/qa_scanner.py`) :
- `QASecurityScanner` — Orchestre 2 types de scans :
  1. **Tool-based** via `VulnerabilityScanner` existant (Bandit, Semgrep, npm audit, pip-audit, secrets)
  2. **Pattern-based** — 8 règles regex intégrées couvrant : SQL injection (CWE-89), XSS/innerHTML/dangerouslySetInnerHTML (CWE-79), CSRF manquant (CWE-352), hardcoded credentials (CWE-798), command injection shell=True (CWE-78), eval/exec (CWE-95), path traversal (CWE-22)
- `SecurityScanSummary` — Résultat complet avec `to_markdown()` (section QA report) et `to_qa_issues()` (format implementation_plan.json)
- `run_qa_security_scan()` — Helper async qui écrit la section `## 🔐 Security Scan Results` dans `qa_report.md`

**Intégration** (`apps/backend/qa/loop.py`) :
- Scan lancé automatiquement après vérification que le build est complet, avant la première itération de review
- Findings critiques/high remontés via `TaskEventEmitter` (`SECURITY_SCAN_ISSUES`)
- Non-bloquant : en cas d'erreur du scanner, le pipeline QA continue normalement

**Comportement** :
- 🚨 Issues **CRITICAL** → bloquent le build (`should_block = True`)
- ⚠️ Issues **HIGH/MEDIUM** → warnings dans le rapport
- ✅ Aucune issue → confirmation dans les logs
- Les findings sont inclus dans `qa_report.md` pour que l'agent QA reviewer les prenne en compte
- L'agent QA fixer peut corriger automatiquement les issues critiques (SQL injection, secrets hardcodés)
...
Les épisodes importés sont stockés dans `.workpilot/team_sync/peers/{member_id}/imported_episodes.json`
...
Les fichiers sont créés dans `.workpilot/environment/` :
