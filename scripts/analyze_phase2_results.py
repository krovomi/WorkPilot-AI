...
class Phase2ResultsAnalyzer:
    ...
    def _analyze_full_evaluation(self):
        ...
        if "performance_impact" in analysis:
            ...
            print(
                f"  Performance Improvement: {perf['performance_improvement_percentage']:.1f}%"
            )
    ...
    def _extract_key_findings(self):
        ...
        if self.results["evaluation_type"] == "full":
            ...
            if "token_impact" in analysis:
                token = analysis["token_impact"]
                findings["token_reduction"] = (
                    f"{token['token_reduction_percentage']:.1f}%"
                )
            ...
            if "performance_impact" in analysis:
                ...
                findings["performance_improvement"] = (
                    f"{perf['performance_improvement_percentage']:.1f}%"
                )
            ...
            if "quality_impact" in analysis:
                quality = analysis["quality_impact"]
                findings["quality_maintenance"] = (
                    f"{quality['quality_change_percentage']:+.1f}%"
                )
                findings["success_rate_change"] = (
                    f"{quality['quality_change_percentage']:+.1f}%"
                )
...
def main():
    ...
    parser.add_argument("--input", type=str, help="Input file with evaluation results")
    ...
    # Create analyzer
    analyzer = Phase2ResultsAnalyzer(input_file=args.input, output_file=args.output)
