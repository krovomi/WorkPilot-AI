...
class Phase2EvaluationRunner:
    ...
    async def _run_baseline_evaluation(self):
        ...
        baseline_results["token_usage"] = await self.collector.collectors[
            "token_usage"
        ].collect_baseline_data(self.duration_hours)

        # Performance baseline
        print("    - Performance baseline...")
        baseline_results["performance"] = await self.collector.collectors[
            "performance"
        ].collect_baseline_data(self.duration_hours)

        # Quality baseline
        print("    - Quality baseline...")
        baseline_results["quality"] = await self.collector.collectors[
            "quality"
        ].collect_baseline_data(self.duration_hours)

        # Claude Code baseline
        print("    - Claude Code baseline...")
        baseline_results["claude_validation"] = await self.collector.collectors[
            "claude_validation"
        ].validate_claude_performance(self.duration_hours)
    ...
    async def _run_optimized_evaluation(self):
        ...
        optimized_results["token_usage"] = await self.collector.collectors[
            "token_usage"
        ].collect_optimized_data(self.duration_hours)

        # Performance optimized
        print("    - Performance optimized...")
        optimized_results["performance"] = await self.collector.collectors[
            "performance"
        ].collect_optimized_data(self.duration_hours)

        # Quality optimized
        print("    - Quality optimized...")
        optimized_results["quality"] = await self.collector.collectors[
            "quality"
        ].collect_optimized_data(self.duration_hours)

        # Claude Code during optimization
        print("    - Claude Code during optimization...")
        optimized_results["claude_validation"] = await self.collector.collectors[
            "claude_validation"
        ].validate_claude_performance(self.duration_hours)
    ...
    async def _run_full_evaluation(self):
        ...
        analysis_results["token_impact"] = self.collector._analyze_token_impact(
            baseline_results.get("token_usage"), optimized_results.get("token_usage")
        )

        print("  Analyzing performance impact...")
        analysis_results["performance_impact"] = (
            self.collector._analyze_performance_impact(
        ...
        analysis_results["quality_impact"] = self.collector._analyze_quality_impact(
            baseline_results.get("quality"), optimized_results.get("quality")
        )
        ...
        if results["evaluation_type"] == "full":
            ...
            if "analysis_results" in results:
                ...
                if "token_impact" in analysis:
                    ...
                    print(
                        f"  Token Reduction: {token.get('token_reduction_percentage', 0):.1f}%"
                    )
                ...
                if "performance_impact" in analysis:
                    ...
                    print(
                        f"  Performance Improvement: {perf.get('performance_improvement_percentage', 0):.1f}%"
                    )
                ...
                if "quality_impact" in analysis:
                    ...
                    print(
                        f"  Quality Change: {quality.get('quality_change_percentage', 0):.1f}%"
                    )
                ...
                if "claude_isolation" in analysis:
                    ...
                    print(
                        f"  Claude Code Impact: {claude.get('response_time_change', 0):.1f}%"
                    )
...
def main():
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Create sample data for testing"
    )
    ...
    runner = Phase2EvaluationRunner(
        duration_hours=args.duration, mode=args.mode, output_file=args.output
    )
