...
function parseVersion(version) {
	...
	if (!match) {
		error(
			`Invalid version format: ${version}. Expected format: x.y.z or x.y.z-prerelease`,
		);
	}
	return {
...
function checkChangelogEntry(version) {
	...
	if (!fs.existsSync(changelogPath)) {
		warning(
			"CHANGELOG.md not found - you will need to create it before releasing",
		);
		return false;
	}
...
function main() {
	...
	if (!bumpType) {
		error(
			"Please specify version bump type or version number.\n" +
				"Usage: node scripts/bump-version.js <major|minor|patch|x.y.z>",
		);
	}

	log("\n🚀 WorkPilot AI Version Bump\n", colors.cyan);
	...
	if (!hasChangelogEntry) {
		log(
			`   1. UPDATE CHANGELOG.md with release notes for ${newVersion}`,
			colors.red,
		);
		log(
			`   2. Commit the changelog: git add CHANGELOG.md && git commit --amend --no-edit`,
			colors.yellow,
		);
		log(
	...
	log(
		`   ${hasChangelogEntry ? "3" : "4"}. Create PR to main (or merge develop → main)`,
		colors.yellow,
	);
	log(
		`   ${hasChangelogEntry ? "4" : "5"}. When merged, GitHub Actions will automatically:`,
		colors.yellow,
	);
	log(
		`      - Validate CHANGELOG.md has entry for v${newVersion}`,
		colors.yellow,
	);
	...
	if (!hasChangelogEntry) {
		warning(
			"IMPORTANT: Update CHANGELOG.md before pushing or the release will fail!",
		);
	}
