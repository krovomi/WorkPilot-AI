...
class Phase2Monitor:
    ...
    def _display_dashboard(self, metrics):
        ...
        print("=" * 60)
        print(
            f"⏰ Time Elapsed: {metrics['elapsed_time_seconds'] // 60}m {metrics['elapsed_time_seconds'] % 60}s"
        )
        ...
        print(
            f"  Avg Tokens/Task: {metrics['global_stats']['average_tokens_per_task']:.1f}"
        )
        ...
        print(
            f"  Avg Tokens/Task: {metrics['runtime_stats']['average_tokens_per_task']:.1f}"
        )
        ...
        print(
            f"  Avg Tokens/Task: {metrics['client_stats']['average_tokens_per_task']:.1f}"
        )
        ...
        self._print_status_indicator(
            "Global Budget", metrics["global_stats"]["budget_utilization"]
        )
        self._print_status_indicator(
            "Runtime Budget", metrics["runtime_stats"]["budget_utilization"]
        )
        self._print_status_indicator(
            "Client Budget", metrics["client_stats"]["budget_utilization"]
        )
        self._print_status_indicator(
            "Global Success Rate", metrics["global_stats"]["success_rate"] * 100
        )
        self._print_status_indicator(
            "Runtime Efficiency", metrics["runtime_stats"]["efficiency_score"] * 100
        )
        self._print_status_indicator(
            "Client Efficiency", metrics["client_stats"]["efficiency_score"] * 100
        )
        print()
    ...
    def _display_trends(self):
        ...
        # Calculate trends
        global_tokens_trend = [
            point["global_stats"]["total_tokens"] for point in recent_points
        ]
        runtime_tokens_trend = [
            point["runtime_stats"]["total_tokens"] for point in recent_points
        ]
        client_tokens_trend = [
            point["client_stats"]["total_tokens"] for point in recent_points
        ]
        ...
        if len(self.data_points) >= 2:
            ...
            token_rate = (
                recent_point["global_stats"]["total_tokens"]
                - previous_point["global_stats"]["total_tokens"]
    ...
    def generate_report(self):
        ...
        total_tokens = sum(
            point["global_stats"]["total_tokens"] for point in self.data_points
        )
        avg_success_rate = sum(
            point["global_stats"]["success_rate"] for point in self.data_points
        ) / len(self.data_points)
        avg_efficiency = sum(
            point["runtime_stats"]["efficiency_score"] for point in self.data_points
        ) / len(self.data_points)
