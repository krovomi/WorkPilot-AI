            ...
            if f":{port}" in line and "LISTENING" in line:
                ...
                if len(parts) >= 5:
                    ...
                    try:
                        ...
                        processes.append(
                            {"pid": int(pid), "name": "Unknown", "cmdline": "Unknown"}
                        )
...
def kill_process(pid):
    ...
    try:
        ...
        print(
            f"Accès refusé pour tuer le processus {pid}. Essayez avec des privilèges administrateur."
        )
        return False
    except Exception as e:
...
def main():
    ...
    if not processes:
        print(
            f"Aucun processus trouvé utilisant le port {port}, mais le port est toujours occupé."
        )
