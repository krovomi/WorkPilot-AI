...
def ensure_grepai():
    ...
    subprocess.run(["python", "src/connectors/grepai/grepai_launcher.py"], check=True)
...
def handle_autofix_command(args):
    ...
    if args.dry_run:
        ...
        result = engine.apply_fixes(
            fixes, min_confidence=args.confidence, dry_run=False
        )
    else:
...
Exemples:
  ...
  %(prog)s grepai --query "votre requête"
    ...
    # Commande: score
    score_parser = subparsers.add_parser("score", help="Analyser la qualité du code")
    score_parser.add_argument(
        "--files", nargs="+", required=True, help="Fichiers à analyser"
    )
    score_parser.add_argument(
        "--project-dir", default=".", help="Répertoire du projet (défaut: .)"
    )
    score_parser.add_argument(
        "--format",
    ...
    score_parser.add_argument("--output", help="Fichier de sortie")

    # Commande: trends
    trends_parser = subparsers.add_parser(
        "trends", help="Afficher les tendances de qualité"
    )
    ...
    history_parser = subparsers.add_parser(
        "history", help="Afficher l'historique des scores"
    )
    history_parser.add_argument(
        "--project-dir", default=".", help="Répertoire du projet (défaut: .)"
    )
    history_parser.add_argument(
        "--limit", type=int, default=10, help="Nombre d'entrées à afficher (défaut: 10)"
    )

    # Commande: autofix
    autofix_parser = subparsers.add_parser(
        "autofix", help="Appliquer des fixes automatiques"
    )
    autofix_parser.add_argument(
        "--files", nargs="+", required=True, help="Fichiers à fixer"
    )
    autofix_parser.add_argument(
        "--project-dir", default=".", help="Répertoire du projet (défaut: .)"
    )
    autofix_parser.add_argument(
        "--dry-run", action="store_true", help="Preview sans appliquer les fixes"
    )
    autofix_parser.add_argument(
        "--apply", action="store_true", help="Appliquer les fixes"
    )
    autofix_parser.add_argument(
        "--preview", action="store_true", help="Afficher un preview détaillé"
    )
    autofix_parser.add_argument(
        "--confidence",
        type=float,
    ...
    clones_parser = subparsers.add_parser("clones", help="Détecter les clones de code")
    clones_parser.add_argument(
        "--project-dir", default=".", help="Répertoire du projet (défaut: .)"
    )
    clones_parser.add_argument(
        "--min-lines", type=int, default=6, help="Nombre minimum de lignes (défaut: 6)"
    )
    clones_parser.add_argument(
        "--min-similarity",
        type=float,
    ...
    clones_parser.add_argument("--output", help="Fichier de sortie pour markdown")

    # Commande: performance (NOUVEAU)
    performance_parser = subparsers.add_parser(
        "performance", help="Analyser les problèmes de performance"
    )
    ...
    rules_parser = subparsers.add_parser(
        "rules", help="Gérer les règles personnalisées"
    )
    ...
    rules_parser.add_argument(
        "--list", action="store_true", help="Lister les règles existantes"
    )

    # Commande: ml (NOUVEAU)
    ml_parser = subparsers.add_parser("ml", help="ML pattern detection")
    ml_parser.add_argument(
        "--project-dir", default=".", help="Répertoire du projet (défaut: .)"
    )
    ml_parser.add_argument(
        "--learn", action="store_true", help="Apprendre les patterns du projet"
    )
    ml_parser.add_argument(
        "--report", action="store_true", help="Générer un rapport des patterns appris"
    )
    ml_parser.add_argument("--output", help="Fichier de sortie pour le rapport")

    # Commande: coverage (NOUVEAU)
    coverage_parser = subparsers.add_parser(
        "coverage", help="Analyser la couverture de tests"
    )
    ...
    coverage_parser.add_argument("--output", help="Fichier de sortie pour markdown")

    # Commande: grepai (NOUVEAU)
    grepai_parser = subparsers.add_parser(
        "grepai", help="Effectuer une recherche via Grepai"
    )
    grepai_parser.add_argument("--query", required=True, help="La requête de recherche")

    # Commande: grepai-search (NOUVEAU)
    grepai_parser = subparsers.add_parser(
        "grepai-search", help="Recherche avancée via Grepai"
    )
    grepai_parser.add_argument(
        "--query", required=True, help="Texte à rechercher avec Grepai"
    )
